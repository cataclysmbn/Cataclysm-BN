...
- `RAMP_END` 경사로 오르내리기 보조 로직이 정렬된 경사로 끝을 식별하도록 표시합니다.
- `RAMP` 이동 보조 로직과 경로 탐색 지원을 위해 타일을 경사로처럼 표시합니다. 이것만으로는 자동 z-레벨 전환을 만들지 않습니다.
- `RAMP_DOWN` 이 타일에 들어가면 이동체는 같은 `x,y` 의 `z - 1` 로 이동합니다.
- `RAMP_UP` 이 타일에 들어가면 이동체는 같은 `x,y` 의 `z + 1` 로 이동합니다.

  [경사로 가이드](../guides/map/ramps.md)도 참고하십시오.
...
- `IMMOBILE` 움직이지 않으며 특수 공격이 아닌 공격도 사용하지 않습니다(예: 포탑).
- `STATIONARY` 움직이지는 않지만 반격은 합니다(예: 훈련용 인형).
