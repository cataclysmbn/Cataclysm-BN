...
{ // ファイル全体が一つのオブジェクトです
  ...
  "overlay_ordering": [
    {
      "id": "WINGS_BAT", // レガシー変異名。文字列、または文字列の配列で指定。
      "order": 1000 // 範囲は 0 - 9999。9999が最前面(一番上)に描画される。
    },
    {
      "id": ["PLANTSKIN", "BARK"], // レガシー変異名。単一の文字列、または文字列の配列のいずれかで記述します。
      "order": 3500 // (指定された)順序(order)は、配列内のすべての項目に適用されます。
    },
    {
      "id": "bio_armor_torso", // レガシーバイオニクス名
      "order": 500
    },
    {
      "id": "worn_backpack", // 完全なオーバーレイIDで装備・手持ちスプライトも並び替え可能
      "order": 500
    }
  ]
}
