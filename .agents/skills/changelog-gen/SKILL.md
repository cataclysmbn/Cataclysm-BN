...
- Preserve videos as `<video controls src="URL" title="CAPTION"></video>` inside `<details>` blocks; never render video/MP4 attachments with image Markdown and never delete valid videos solely because they are videos. Use a localized `media` summary label when a block contains both screenshots and videos.
