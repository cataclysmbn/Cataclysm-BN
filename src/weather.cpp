                            ...
                            const tripoint &location )
{
    ...
    for( time_point t = start; t < end; t += tick_size ) {
        ...
        data.wind_amount += get_local_windpower( weather.windspeed,
                            ...
                            get_overmapbuffer( get_avatar().get_dimension() ).ter( tripoint_abs_omt( ms_to_omt_copy( location ) ) ),
                            location,
...
std::string weather_forecast( const point_abs_sm &abs_sm_pos )
{
    ...
    // Local conditions
    const auto cref = get_overmapbuffer( get_avatar().get_dimension() ).closest_city( tripoint_abs_sm( abs_sm_pos, 0 ) );
