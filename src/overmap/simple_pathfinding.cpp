...
node_score::node_score(int node_cost, int estimated_dest_cost)
    : node_cost(node_cost),
...
struct point_node {
    ...
    point_node(point pos, om_direction::type dir, int priority = 0)
        : pos(pos),
          dir(dir),
          priority(priority) {}
    // Operator overload required by priority queue interface.
    bool operator<(const point_node& n) const { return priority > n.priority; }
};
...
directed_path<point> greedy_path(
    ...
    const auto map_index = [max](point p) { return p.y * max.x + p.x; };

    directed_path<point> res;
    if (source == dest) { return res; }
    if (!inbounds(source) || !inbounds(dest)) { return res; }
    const Node first_node(source, om_direction::type::invalid, 1000);
    if (scorer(directed_node<point>(first_node.pos, first_node.dir), std::nullopt).node_cost < 0) {
        return res;
    }
    ...
    while (!nodes[i].empty()) {
        ...
        for (om_direction::type dir : om_direction::all) {
            ...
            if (!inbounds(p) || closed[n]) { continue; }
            const node_score score =
                scorer(directed_node<point>(p, dir), directed_node<point>(mn.pos, mn.dir));
            if (score.node_cost < 0) { continue; }
            ...
            if (open[n] == 0 || open[n] > priority) {
                ...
                if (open[n] != 0) {
                    ...
                    nodes[i].pop();
                    if (nodes[i].size() > nodes[1 - i].size()) { i = 1 - i; }
                    while (!nodes[i].empty()) {
                        nodes[1 - i].push(nodes[i].top());
                        nodes[i].pop();
                    }
...
struct navigation_node {
    ...
    direction get_prev_dir() const { return static_cast<direction>(prev_dir); }
};

const std::vector<direction>& enumerate_directions(bool allow_z_change) {
    static const std::vector<direction> cardinal_dirs =
...
direction reverse_direction(direction dir) { return direction_from(-direction_to_tripoint(dir)); }
...
omt_score::omt_score(int node_cost, bool allow_z_change)
    : node_cost(node_cost),
      allow_z_change(allow_z_change) {}

simple_path<tripoint_abs_omt> find_overmap_path(
    const tripoint_abs_omt& source, const tripoint_abs_omt& dest, const int radius,
    omt_scoring_fn scorer, std::optional<int> max_cost) {
    constexpr size_t max_search_count = 100000;
    simple_path<tripoint_abs_omt> ret;
    bool meet = false;

    auto do_astar =
        [&](const tripoint_abs_omt& start,
            ...
            for (direction dir : enumerate_directions(cur_node.allow_z_change)) {
                ...
                const int cumulative_cost =
                    cur_node.cumulative_cost
                    + adjust_omt_cost(cur_node.node_cost, rev_dir, cur_node.get_prev_dir());
                ...
                if (iter != known_nodes.end()) {
                    ...
                    if (octile_dist(source.xy(), next_addr.xy()) > radius) { continue; }
                    ...
                    const int estimated_total_cost =
                        cumulative_cost + next_score.node_cost + xy_score + z_score;
                    if (max_cost && estimated_total_cost > *max_cost) { continue; }
    ...
    if (start_score.node_cost < 0 || end_score.node_cost < 0) { return ret; }
