...
auto strip_label_commands(std::string note_text) -> std::string {
    ...
    while (pos != std::string::npos) {
        ...
        while (end < note_text.size() && note_text[end] == ' ') { ++end; }
        while (end < note_text.size() && note_text[end] != ';') { ++end; }
        if (end < note_text.size() && note_text[end] == ';') { ++end; }
        note_text.erase(start, end - start);
        pos = note_text.find(label_prefix, start);
    }

    while (!note_text.empty() && note_text.back() == ';') { note_text.pop_back(); }
    while (!note_text.empty() && note_text.front() == ';') { note_text.erase(note_text.begin()); }
