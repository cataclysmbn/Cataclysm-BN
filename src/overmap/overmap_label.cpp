...
auto overmap_labels::set_label(const oter_type_str_id& id, const std::optional<std::string>& label)
    -> void {
    if (label.has_value()) {
        map_labels[id] = *label;
        return;
...
auto overmap_labels::get_label(const oter_type_str_id& id) -> std::optional<std::string> {
    ...
    if (iter == map_labels.end()) { return std::nullopt; }

    return iter->second;
}

auto overmap_labels::reset() -> void { map_labels.clear(); }
