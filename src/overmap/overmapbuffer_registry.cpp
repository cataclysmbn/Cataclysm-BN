...
public:
    ...
    auto unload_dimension(const dimension_id& dim_id) -> void { buffers_.erase(dim_id); }

    auto for_each(const std::function<void(const dimension_id&, overmapbuffer&)>& fn) -> void {
        for (auto& kv : buffers_) { fn(kv.first, *kv.second); }
    }
...
auto get_primary_overmapbuffer() -> overmapbuffer& { return registry().primary(); }

dimension_id g_active_dimension_id; // default "" = overworld (primary)

auto get_active_overmapbuffer() -> overmapbuffer& { return registry().get(g_active_dimension_id); }
