...
template <> std::string enum_to_string<overmap_connection_layout>(overmap_connection_layout data) {
    switch (data) {
        ...
        case overmap_connection_layout::city:
            return "city";
        case overmap_connection_layout::p2p:
            return "p2p";
        // *INDENT-ON*
...
bool overmap_connection::subtype::allows_terrain(const oter_id& oter) const {
    ...
    return std::ranges::any_of(locations, [&oter](const overmap_location_id& elem) {
...
overmap_connection::overmap_connection(const overmap_connection& other) { *this = other; }
...
const overmap_connection::subtype* overmap_connection::pick_subtype_for(
...

    {
        ...
        if (it != cached_subtypes.end()) { return it->second.value; }
    }

    // Used below in find_if iteration
    auto passes = [&ground](const subtype& elem) { return elem.allows_terrain(ground); };
    ...
    for (auto iter = std::find_if(subtypes.begin(), subtypes.end(), passes); iter != subtypes.end();
    ...
    if (idx != nullptr) { result = &subtypes[*idx]; }
...
void load(const JsonObject& jo, const std::string& src) { connections.load(jo, src); }
...
void check_consistency() { connections.check(); }
