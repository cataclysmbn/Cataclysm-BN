...
auto overmap_label_note::extract_label(const std::string& note) -> std::optional<std::string> {
    ...
    while (search_start < note.size()) {
        ...
        while (start < note.size() && note[start] == ' ') { start++; }

        if (start >= note.size()) { return std::nullopt; }

        auto end = start;
        while (end < note.size() && note[end] != ';') { end++; }

        while (end > start && note[end - 1] == ' ') { end--; }
...
auto overmap_label_note::is_label_only(const std::string& note) -> bool {
    ...
    if (note == make_note(*label)) { return true; }
