...
auto next_work_id() -> uint64_t {
    ...
    if (s_next_work_id == 0) { s_next_work_id = 1; }
    if (result == 0) { result = s_next_work_id++; }
    return result;
}

auto z_to_resident_index(const int zlev) -> int { return zlev + OVERMAP_DEPTH; }

auto resident_index_is_valid(const int idx) -> bool { return idx >= 0 && idx < s_lighting.z_count; }
...
auto reset_residency_for_shape(const int cache_x, const int cache_y, const int z_count) -> void {
    ...
    ++s_next_residency_id;
    if (s_next_residency_id == 0) { s_next_residency_id = 1; }
...
auto mark_seen_levels_valid(
    std::vector<int> const& levels, const int player_x, const int player_y,
    const bool force_invalidate) -> void {
    if (force_invalidate || !s_lighting.seen_origin_valid || s_lighting.seen_origin_x != player_x
        || s_lighting.seen_origin_y != player_y) {
        reset_seen_residency();
    }
...
auto all_levels() -> std::vector<int> {
    ...
    std::ranges::
        copy(std::views::iota(-OVERMAP_DEPTH, OVERMAP_HEIGHT + 1), std::back_inserter(result));
    return result;
}
...
auto sight_pair_levels(std::vector<sight_pair> const& pairs) -> std::vector<int> {
    ...
    for (const auto& pair : pairs) {
        ...
        std::ranges::copy(
            std::views::iota(min_z_idx, max_z_idx + 1) | std::views::filter([](const int z_idx) {
                return z_idx >= 0 && z_idx < OVERMAP_LAYERS;
            }) | std::views::transform([](const int z_idx) { return z_idx - OVERMAP_DEPTH; }),
...
auto choose_resident_level_update(choose_resident_level_update_params const& p)
    -> std::vector<int> {
    if (p.all_levels == nullptr) { return {}; }
    if (!p.dirty && p.resident_valid) { return {}; }
...
auto choose_transparency_update_levels(
    const bool dirty, std::vector<int> const* dirty_levels, std::vector<int> const& all_levels)
    -> std::vector<int> {
    auto result = std::vector<int>{};
    for (const auto zlev : all_levels) {
        if (!transparency_level_is_valid(zlev)) { result.push_back(zlev); }
    }
    if (dirty) {
        const auto& source =
...
auto copy_transparency_levels(
    map const& m, std::vector<int> const& levels, std::vector<float>& target) -> void {
    for (const auto zlev : levels) {
        const auto& lc = m.get_cache_ref(zlev);
        const auto offset = level_offset(zlev);
        std::ranges::
            copy(lc.transparency_cache, target.begin() + static_cast<std::ptrdiff_t>(offset));
        const auto z_idx = z_to_resident_index(zlev);
        if (resident_index_is_valid(z_idx)) {
...
auto copy_floor_levels(map const& m, std::vector<int> const& levels, std::vector<uint32_t>& target)
    -> void {
    for (const auto zlev : levels) {
        const auto& lc = m.get_cache_ref(zlev);
        const auto offset = level_offset(zlev);
        std::ranges::transform(
            lc.floor_cache, target.begin() + static_cast<std::ptrdiff_t>(offset),
            [](const char value) -> uint32_t { return value != '\0' ? 1U : 0U; });
    }
}

auto copy_vehicle_floor_levels(
    map const& m, std::vector<int> const& levels, std::vector<uint32_t>& target) -> void {
    for (const auto zlev : levels) {
        const auto& lc = m.get_cache_ref(zlev);
        const auto offset = level_offset(zlev);
        std::ranges::transform(
            lc.vehicle_floor_cache, target.begin() + static_cast<std::ptrdiff_t>(offset),
            [](const char value) -> uint32_t { return value != '\0' ? 1U : 0U; });
    }
}

auto copy_vehicle_obscured_levels(
    map const& m, std::vector<int> const& levels, std::vector<uint32_t>& target) -> void {
    for (const auto zlev : levels) {
        const auto& lc = m.get_cache_ref(zlev);
        const auto offset = level_offset(zlev);
        std::ranges::transform(
            lc.vehicle_obscured_cache, target.begin() + static_cast<std::ptrdiff_t>(offset),
...
auto copy_colored_light_to_level_caches(map const& m, std::vector<int> const& levels) -> void {
    for (const auto zlev : levels) {
        ...
        const auto offset = level_offset(zlev);
        const auto src = std::
            span{s_lighting.colored_light.data() + offset,
                 static_cast<std::size_t>(s_lighting.cache_x * s_lighting.cache_y)};
...
auto max_optic_radius(std::span<cata_gpu::GpuVehicleOptic const> optics) -> int {
    auto result = 0;
    for (const auto& optic : optics) { result = std::max(result, optic.range); }
    return result;
}
...
auto make_ambient_params(const int cache_x, const int cache_y, const int cache_xy)
    ...
    for (const auto z_idx : std::views::iota(0, OVERMAP_LAYERS)) {
        params.natural_light[z_idx / 4][z_idx % 4] = g->natural_light_level(z_idx - OVERMAP_DEPTH);
    }
    return params;
}
#endif
...
auto resident_lighting_ready_for_sight_pairs(resident_sight_pair_inputs_params const& p) -> bool {
    if (p.m == nullptr || p.pairs == nullptr || p.pairs->empty()) { return false; }
    ...
    return s_lighting.floor_valid && std::ranges::all_of(levels, [](const int zlev) {
...
auto prepare_lighting_transparency_output(prepare_lighting_transparency_output_params const& p)
    ...
    if (p.cache_x <= 0 || p.cache_y <= 0 || p.z_count <= 0) { return {}; }

    reset_residency_for_shape(p.cache_x, p.cache_y, p.z_count);
    const auto z_idx = z_to_resident_index(p.zlev);
    if (!resident_index_is_valid(z_idx)) { return {}; }
...
auto run_lighting(lighting_params const& p) -> bool {
    ...
    if (p.rebuild_seen_cache || p.download_seen_cache) { return false; }
    if (lightmap_levels.empty()) { return true; }
    ...
    if (!refresh_transparency_valid() || !s_lighting.floor_valid || !s_lighting.vehicle_floor_valid
        || !s_lighting.vehicle_obscured_valid) {
        return false;
    }
    ...
    if (!lightmap_levels.empty()) {
        ...
        if (p.direct_sunlight) {
            ...
            for (const auto pass : std::views::iota(0, daylight_diffusion_passes)) {
                ...
                target =
                    pass % 2 == 0
                        ? s_lighting.daylight_diffuse_b.data()
...
auto begin_lighting(lighting_params const& p) -> lighting_work {
    if (s_pending_lighting.active || !slang_cpu::run_lighting(p)) { return {}; }
...
auto run_visibility(visibility_params const& p) -> bool {
    ...
    auto download_levels =
        p.download_levels == nullptr ? all_levels() : sorted_unique(*p.download_levels);
...
auto begin_visibility(visibility_params const& p) -> visibility_work {
    if (s_pending_visibility.active || !slang_cpu::run_visibility(p)) { return {}; }
    const auto id = next_work_id();
    s_pending_visibility = {
        .active = true,
        .id = id,
...
auto begin_sight_pairs(begin_sight_pairs_params const& p) -> sight_pairs_work {
    if (s_pending_sight_pairs.active || p.pairs == nullptr || p.pairs->empty()) { return {}; }
    const auto id = next_work_id();
    s_pending_sight_pairs = {
        .active = true,
        .id = id,
