...
auto sight_pairs(sight_pairs_params const& params) -> bool {
    ...
    std::ranges::transform(
        params.pairs, std::back_inserter(shader_pairs),
        [](cata_gpu::GpuSightPair const& pair) { return copy_sight_pair<GpuSightPair_0>(pair); });
    ...
    globals.pairs_0 =
        readonly_buffer(shader_pairs.data(), static_cast<uint32_t>(shader_pairs.size()));
    globals.results_0 =
        writable_buffer(params.results->data(), static_cast<uint32_t>(params.results->size()));
