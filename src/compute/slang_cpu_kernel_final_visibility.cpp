...
auto final_visibility( final_visibility_params const &params ) -> bool
{
    ...
    if( params.transparency == nullptr || params.lightmap == nullptr || params.seen == nullptr ||
        ...
        params.z_count <= 0 || params.dispatch_z_count <= 0 ) {
        return false;
    }
    ...
    globals.lm_all_0 = StructuredBuffer<uint32_t> {
        .data = const_cast<uint32_t *>( params.lightmap ),
        .count = total_tiles,
    };
    ...
    globals.camera_all_0 = StructuredBuffer<uint32_t> {
        .data = const_cast<uint32_t *>( params.camera ),
        .count = total_tiles,
    };
    ...
    globals.visibility_all_0 = RWStructuredBuffer<uint32_t> {
