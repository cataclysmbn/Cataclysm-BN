...
auto dispatch_generated_kernel(Kernel&& kernel, Globals& globals, cpu_dispatch_range const& range)
    -> void {
...
auto source_window_is_valid(
    Sources const& sources, const uint32_t source_offset, const uint32_t source_count) -> bool {
...
template <typename Dst, typename Src> auto copy_light_source(Src const& source) -> Dst {
    auto result = Dst{};
    result.x_0 = source.x;
    result.y_0 = source.y;
...
template <typename Dst, typename Src> auto copy_colored_light_source(Src const& source) -> Dst {
    auto result = copy_light_source<Dst>(source);
    result.color_rgb_0 = source.color_rgb;
    return result;
}

template <typename Dst, typename Src> auto copy_vehicle_optic(Src const& source) -> Dst {
    auto result = Dst{};
    result.x_0 = source.x;
    result.y_0 = source.y;
    result.z_idx_0 = source.z_idx;
...
template <typename Dst, typename Src> auto copy_sight_pair(Src const& source) -> Dst {
    auto result = Dst{};
...
inline auto radius_group_side(const int32_t radius) -> uint32_t { return ray_group_side(radius); }
#endif

template <typename RunRange>
auto dispatch_independent(independent_dispatch_grid const& grid, RunRange&& run_range) -> void {
    if (grid.group_x == 0 || grid.group_y == 0 || grid.group_z == 0) { return; }
...
auto dispatch_independent_kernel(
    independent_dispatch_grid const& grid, Globals& globals, Kernel&& kernel) -> void {
...
inline auto make_accumulating_ranges(accumulating_dispatch_grid const& grid)
    ...
    if (grid.group_x == 0 || grid.group_y == 0 || grid.group_z == 0) { return ranges; }
    ...
    const auto push_full = [&]() { ranges.push_back(full_range); };
    ...
    if (grid.output_values > 0 && grid.max_scratch_bytes > 0) {
        const auto bytes_per_buffer =
            static_cast<uint64_t>(grid.output_values) * static_cast<uint64_t>(sizeof(uint32_t));
...
auto dispatch_accumulating_chunks(std::span<cpu_dispatch_range const> ranges, RunChunk&& run_chunk)
    -> void {
...
auto dispatch_accumulating_uint(
    accumulating_dispatch_grid const& grid, Globals globals, BindOutput&& bind_output,
    RunKernel&& run_kernel, MergeOutput&& merge_output) -> bool {
    const auto ranges = make_accumulating_ranges(grid);
    if (ranges.empty()) { return true; }

    if (ranges.size() == 1) {
        dispatch_accumulating_chunks(
            ranges, [&](const std::size_t, cpu_dispatch_range const& range) {
    ...
    dispatch_accumulating_chunks(
        ranges, [&](const std::size_t chunk_index, cpu_dispatch_range const& range) {
    ...
    for (auto& scratch : scratch_buffers) {
        if (!merge_output(scratch.data(), grid.output_values)) { return false; }
    }
    return true;
}
#endif
