...
auto raytrace(raytrace_params const& params) -> bool {
    ...
    if (params.transparency == nullptr || params.floor == nullptr || params.vehicle_floor == nullptr
        || params.sources.empty() || params.lightmap == nullptr || params.cache_x <= 0
    ...
    for (const auto& source : params.sources) {
        shader_sources.push_back(copy_light_source<GpuLightSource_0>(source));
    }
    ...
    globals.light_sources_0 =
        readonly_buffer(shader_sources.data(), static_cast<uint32_t>(shader_sources.size()));
    ...
    return dispatch_accumulating_uint(
        ...
        [](auto& chunk_globals, uint32_t* data, const uint32_t count) {
            chunk_globals.lm_all_0 = writable_buffer(data, count);
        },
        [](auto& chunk_globals, cpu_dispatch_range const& range) {
            dispatch_generated_kernel(cata_slang_lm_raytrace_cpu_main, chunk_globals, range);
        },
