...
auto ensure_buf(SDL_GPUDevice* const device, ZBuf& slot, Uint32 const size) -> void {
    ...
    SDL_GPUBufferCreateInfo const
        ci{.usage = SDL_GPU_BUFFERUSAGE_COMPUTE_STORAGE_READ, .size = size, .props = 0};
...
auto upload_char_cache_batch(
    SDL_GPUDevice* const device, std::array<ZBuf, OVERMAP_LAYERS>& slots, map const& m,
    int const minz, int const maxz, auto cache_accessor) -> void {
    ...
    SDL_GPUTransferBufferCreateInfo const
        tbuf_ci{.usage = SDL_GPU_TRANSFERBUFFERUSAGE_UPLOAD, .size = total_size, .props = 0};
...
auto upload_floor_caches(map const& m, int const minz, int const maxz) -> void {
    ...
    upload_char_cache_batch(
        device, s_floor_bufs, m, minz, maxz,
...
auto upload_vehicle_floor_caches(map const& m, int const minz, int const maxz) -> void {
    ...
    if (device == nullptr) { return; }
    upload_char_cache_batch(
        device, s_vehicle_floor_bufs, m, minz, maxz,
