...
public:
    ...
    std::function<void(const spell&, Creature&, const tripoint_abs_ms&)> effect;
...
public:
    ...
    bool in_aoe(const tripoint_abs_ms& source, const tripoint_abs_ms& target) const;
    ...
    void create_field(const tripoint_abs_ms& at) const;

    // makes a spell sound at the location
    void make_sound(const tripoint_abs_ms& target, Creature& caster) const;
    void make_sound(const tripoint_abs_ms& target, Creature& caster, int loudness) const;
    // heals the critter at the location, returns amount healed (Character heals each body part)
    int heal(const tripoint_abs_ms& target) const;

    // casts the spell effect. returns true if successful
    void cast_spell_effect(Creature& source, const tripoint_abs_ms& target) const;
    // goes through the spell effect and all of its internal spells
    void cast_all_effects(Creature& source, const tripoint_abs_ms& target) const;
    ...
    bool is_target_in_range(const Creature& caster, const tripoint_abs_ms& p) const;

    // is the target valid for this spell?
    bool is_valid_target(const Creature& caster, const tripoint_abs_ms& p) const;
    bool is_valid_target(valid_target t) const;
    bool is_valid_effect_target(valid_target t) const;
    bool target_by_monster_id(const tripoint_abs_ms& p) const;

    // picks a random valid tripoint from @area
    std::optional<tripoint_abs_ms> random_valid_target(
...
void teleport_random(const spell& sp, Creature& caster, const tripoint_abs_ms&);
void pain_split(const spell&, Creature&, const tripoint_abs_ms&);
void target_attack(const spell& sp, Creature& caster, const tripoint_abs_ms& epicenter);
void projectile_attack(const spell& sp, Creature& caster, const tripoint_abs_ms& target);
void cone_attack(const spell& sp, Creature& caster, const tripoint_abs_ms& target);
void line_attack(const spell& sp, Creature& caster, const tripoint_abs_ms& target);

void area_pull(const spell& sp, Creature& caster, const tripoint_abs_ms& center);
void area_push(const spell& sp, Creature& caster, const tripoint_abs_ms& center);
void directed_push(const spell& sp, Creature& caster, const tripoint_abs_ms& target);

std::set<tripoint_abs_ms> spell_effect_blast(
    const spell&, const tripoint_abs_ms&, const tripoint_abs_ms& target, int aoe_radius,
    bool ignore_walls);
std::set<tripoint_abs_ms> spell_effect_cone(
    const spell& sp, const tripoint_abs_ms& source, const tripoint_abs_ms& target, int aoe_radius,
    bool ignore_walls);
std::set<tripoint_abs_ms> spell_effect_line(
    const spell&, const tripoint_abs_ms& source, const tripoint_abs_ms& target, int aoe_radius,
    bool ignore_walls);

void spawn_ethereal_item(const spell& sp, Creature&, const tripoint_abs_ms&);
void recover_energy(const spell& sp, Creature&, const tripoint_abs_ms& target);
void spawn_summoned_monster(const spell& sp, Creature& caster, const tripoint_abs_ms& target);
void spawn_summoned_vehicle(const spell& sp, Creature& caster, const tripoint_abs_ms& target);
void translocate(const spell& sp, Creature& caster, const tripoint_abs_ms& target);
// adds a timed event to the caster only
void timed_event(const spell& sp, Creature& caster, const tripoint_abs_ms&);
void transform_blast(const spell& sp, Creature& caster, const tripoint_abs_ms& target);
void noise(const spell& sp, Creature&, const tripoint_abs_ms& target);
void vomit(const spell& sp, Creature& caster, const tripoint_abs_ms& target);
void explosion(const spell& sp, Creature& caster, const tripoint_abs_ms& target);
void flashbang(const spell& sp, Creature& caster, const tripoint_abs_ms& target);
void mod_moves(const spell& sp, Creature& caster, const tripoint_abs_ms& target);
void map_area(const spell& sp, Creature& caster, const tripoint_abs_ms&);
void morale(const spell& sp, Creature& caster, const tripoint_abs_ms& target);
void charm_monster(const spell& sp, Creature& caster, const tripoint_abs_ms& target);
void mutate(const spell& sp, Creature& caster, const tripoint_abs_ms& target);
void bash(const spell& sp, Creature& caster, const tripoint_abs_ms& target);
void dash(const spell& sp, Creature& caster, const tripoint_abs_ms& target);
void none(const spell& sp, Creature&, const tripoint_abs_ms& target);
void bash_area(const spell& sp, Creature& caster, const std::set<tripoint_abs_ms> area);
} // namespace spell_effect
...
struct area_expander {
    ...
    std::map<tripoint_abs_ms, int> area_search;
    ...
    int contains(const tripoint_abs_ms& pt) const;

    // Adds node to a search tree. Returns true if new node is allocated.
    bool enqueue(const tripoint_abs_ms& from, const tripoint_abs_ms& to, float cost);

    // Run wave propagation
    int run(const tripoint_abs_ms& center);
