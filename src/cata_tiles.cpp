...
static void get_pixel_rgba( SDL_Surface *surface, int x, int y,
...
static void set_pixel_rgba( SDL_Surface *surface, int x, int y,
                            Uint8 r, Uint8 g, Uint8 b, Uint8 a )
{
...
std::tuple<bool, SDL_Surface *, SDL_Rect> tileset::get_sprite_surface( int sprite_index ) const
{
    ...
    return tileset_atlas->readback_find( tex_it->second );
}
...
void tileset_loader::load_state_modifiers( const JsonObject &config )
{
    ...
    for( const JsonObject &mod_group : config.get_array( "state-modifiers" ) ) {
        ...
        // Check if a group with this ID already exists (from base tileset or earlier mod)
        // If so, replace it; otherwise add as new. This allows mod tilesets to override
        // state modifiers from the base tileset.
