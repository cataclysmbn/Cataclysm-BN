...
void iexamine::safe( player &p, const tripoint_bub_ms &examp )
{
    ...
    if( target_diff > 0 && prying_tool && !p.movement_mode_is( CMM_CROUCH ) && !p.movement_mode_is( CMM_PRONE ) ) {
...
void iexamine::locked_object( player &p, const tripoint_bub_ms &examp )
{
    ...
    if( lockpick_activity_actor::is_pickable( examp ) && ( p.movement_mode_is( CMM_CROUCH ) || p.movement_mode_is( CMM_PRONE ) ) ) {
