...
inline auto origin_lua_name( const coords::origin origin ) -> std::string_view
{
    switch( origin ) {
...
inline auto scale_lua_name( const coords::scale scale ) -> std::string_view
{
    switch( scale ) {
...
inline auto origin_type_name( const coords::origin origin ) -> std::string_view
{
    switch( origin ) {
        case coords::origin::relative:
            return "Rel";
...
inline auto scale_type_name( const coords::scale scale ) -> std::string_view
{
    switch( scale ) {
        case coords::scale::map_square:
            return "Ms";
...
inline auto parse_origin( const std::string_view name ) -> std::optional<coords::origin>
{
...
inline auto parse_scale( const std::string_view name ) -> std::optional<coords::scale>
{
...
inline auto is_registered_coord( const coords::origin origin, const coords::scale scale ) -> bool
{
    switch( origin ) {
        case coords::origin::relative:
            return true;
...
inline auto has_remainder_origin( const coords::scale scale ) -> bool
{
    switch( scale ) {
        case coords::scale::submap:
        case coords::scale::overmap_terrain:
...
inline auto exact_scale_conversion( const coords::scale source, const coords::scale result ) -> bool
{
...
inline auto can_project_to( const coords::origin origin, const coords::scale source,
...
inline auto can_project_remain( const coords::origin origin, const coords::scale source,
                                const coords::scale result ) -> bool
{
    const auto source_squares = coords::map_squares_per( source );
    const auto result_squares = coords::map_squares_per( result );
...
inline auto can_project_combine( const project_combine_check &check ) -> bool
{
...
inline auto coord_type_name( const bool is_tripoint, const coords::origin origin,
...
inline auto point_to_string( const lua_point_coord &coord ) -> std::string
{
    return string_format( "%s(%d,%d)", coord_type_name( false, coord.origin, coord.scale ),
                          coord.raw.x, coord.raw.y );
}

inline auto tripoint_to_string( const lua_tripoint_coord &coord ) -> std::string
{
    return string_format( "%s(%d,%d,%d)", coord_type_name( true, coord.origin, coord.scale ),
                          coord.raw.x, coord.raw.y, coord.raw.z );
}

inline auto make_point_coord( const coords::origin origin, const coords::scale scale,
...
inline auto make_tripoint_coord( const coords::origin origin, const coords::scale scale,
...
inline auto make_point_coord( const std::string &origin_name, const std::string &scale_name,
                              const point &raw ) -> lua_point_coord
{
...
inline auto make_tripoint_coord( const std::string &origin_name, const std::string &scale_name,
                                 const tripoint &raw ) -> lua_tripoint_coord
{
    const auto origin = parse_origin( origin_name );
    const auto scale = parse_scale( scale_name );
...
inline auto project_xy( const point &raw, const coords::scale source,
...
inline auto project_tripoint_raw( const tripoint &raw, const coords::scale source,
...
inline auto project_to( const lua_point_coord &coord,
...
inline auto project_to( const lua_tripoint_coord &coord,
...
inline auto same_coord_kind( const lua_point_coord &lhs, const lua_point_coord &rhs ) -> bool
{
    return lhs.origin == rhs.origin && lhs.scale == rhs.scale;
}

inline auto same_coord_kind( const lua_tripoint_coord &lhs,
