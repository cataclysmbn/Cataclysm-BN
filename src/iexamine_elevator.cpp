                   ...
                   const tripoint &sm_orig ) -> int
{
    ...
    for( int z = OVERMAP_HEIGHT; z >= -OVERMAP_DEPTH; z-- ) {
        ...
        const int turns = get_rot_turns( this_omt, that_omt, get_overmapbuffer( get_map().get_bound_dimension() ) );
        const tripoint zp =
        ...
        const std::string omt_name = get_overmapbuffer( get_map().get_bound_dimension() ).ter_existing( that_omt )->get_name();
