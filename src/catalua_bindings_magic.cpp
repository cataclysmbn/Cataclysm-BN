...
#define UT_CLASS fake_spell
    {
                          ...
                          sol::optional<int> level ) -> fake_spell
        {
            ...
            bool hit_self = avvy.abs_pos() == target;
...
#define UT_CLASS spell
    {
        ...
        SET_FX_N_T( cast_all_effects, "cast", void( Creature & source, const tripoint_abs_ms & target ) const );
        DOC( "Cast *only* this spell's main effects. Generally, cast() should be used instead." );
        SET_FX_N_T( cast_spell_effect, "cast_single_effect", void( Creature & source, const tripoint_abs_ms & target ) const );
    }
