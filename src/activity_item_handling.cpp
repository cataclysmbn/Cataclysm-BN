...
{
    return !con_id->group.is_empty() ? con_id->group->name() : _( "construction site" );
}
...
{
    const map_stack items = get_map().i_at( src_loc );
    return items.empty() ? std::string() : items.front()->display_name();
}
