...
auto source_for_ix(
    const std::vector<proc::candidate_source_entry>& sources,
    const proc::part_ix ix) -> const proc::candidate_source_entry* // *NOPAD*
{
    const auto iter =
        std::ranges::find_if(sources, [&](const proc::candidate_source_entry& source) {
...
auto proc::group_candidate_entries(const std::vector<candidate_label_entry>& entries)
    ...
    for (const auto& entry : entries) {
        const auto iter =
            std::find_if(ret.begin(), ret.end(), [&](const grouped_candidate_entry& grouped) {
...
auto proc::filter_grouped_candidates(
...
auto proc::grouped_candidate_label(const grouped_candidate_entry& entry) -> std::string {
    if (entry.total_count <= 1) { return entry.label; }
...
auto proc::first_grouped_candidate_ix(const grouped_candidate_entry& entry) -> part_ix {
    if (entry.ixs.empty()) { return invalid_part_ix; }
