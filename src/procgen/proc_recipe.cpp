...
auto stew_tool_requirements(const std::vector<proc::part_fact>& facts) -> requirement_data {
    if (!stew_requires_cutting(facts)) { return {}; }

    return requirement_data({}, {{quality_requirement(quality_id("CUT"), 1, 1)}}, {});
}
...
auto proc::recipe_requirements(const recipe& rec, const std::vector<part_fact>& facts)
    -> requirement_data {
    auto reqs = rec.simple_requirements();
    if (!rec.is_proc()) { return reqs; }
...
auto proc::recipe_preview_description(const recipe& rec) -> std::string {
    if (!rec.builder_desc().translated().empty()) { return rec.builder_desc().translated(); }
