...
auto build_candidates(const schema& sch, const std::vector<part_fact>& facts)
...
auto filter_available_candidates(const builder_state& state, const std::vector<part_ix>& candidates)
    -> std::vector<part_ix>;
auto slot_can_meet_minimum(const builder_state& state, const schema& sch, const slot_id& slot)
    -> bool;
...
auto preview_result_override(
    const schema& sch, const std::vector<part_fact>& facts,
    const std::vector<craft_pick>& picks) -> std::optional<itype_id>;
auto debug_part_fact(const schema& sch, const item& it, part_ix ix) -> std::optional<part_fact>;
auto fast_fp(const schema& sch, const fast_blob& blob, const std::vector<part_fact>& facts)
    -> std::string;
auto part_search_texts(const part_fact& fact, const part_search_options& opts)
    -> std::vector<std::string>;
auto part_matches_search(
    const part_fact& fact, const part_search_options& opts, const std::string& txt) -> bool;
