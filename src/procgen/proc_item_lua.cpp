...
auto scaled_food_servings(const item& it, const proc::fast_blob& blob) -> int {
    if (!it.count_by_charges()) { return 1; }
    ...
    const auto mass_servings =
        blob.mass_g > 0
            ? ceil_div(static_cast<int64_t>(blob.mass_g) * default_servings, default_mass_g)
            : default_servings;
    const auto volume_servings =
        blob.volume_ml > 0
            ? ceil_div(static_cast<int64_t>(blob.volume_ml) * default_servings, default_volume_ml)
            : default_servings;
...
auto find_lua_function(sol::state& lua, const std::string& path)
    ...
    if (current == sol::lua_nil) { return std::nullopt; }
...
auto resolve_lua_function(cata::lua_state* state, const std::string& path)
    -> std::optional<lua_function_ref> {
...
auto fact_table(sol::state& lua, const proc::part_fact& fact) -> sol::table {
    ...
    tbl["chg"] = fact.chg;
    tbl["proc"] =
        fact.proc.empty() ? sol::make_object(lua, sol::lua_nil) : sol::make_object(lua, fact.proc);
    ...
    auto idx = int{1};
    std::ranges::for_each(fact.tag, [&](const std::string& entry) { tag_tbl[idx++] = entry; });
    idx = 1;
    std::ranges::for_each(fact.flag, [&](const flag_id& entry) { flag_tbl[idx++] = entry.str(); });
    idx = 1;
...
auto make_lua_params(sol::state& lua, const lua_params_options& opts) -> sol::table {
    ...
    idx = int{1};
    const auto picks = opts.picks == nullptr ? std::vector<proc::craft_pick>{} : *opts.picks;
...
auto call_lua_blob(
    const std::string& path, const proc::schema& sch, const std::vector<proc::part_fact>& facts,
    const std::vector<proc::craft_pick>& picks, const proc::fast_blob& blob, cata::lua_state* state,
    ...
    const auto params = make_lua_params(
        lua,
        {
            .sch = sch,
            .facts = facts,
    ...
    if (!res.valid() || res.return_count() == 0) { return std::nullopt; }
    const auto obj = res.get<sol::object>();
    if (obj == sol::lua_nil || !obj.is<sol::table>()) { return std::nullopt; }
    return obj.as<sol::table>();
}

auto call_lua_validate(
    const std::string& path, const proc::schema& sch, const std::vector<proc::part_fact>& facts,
    const proc::fast_blob& blob, cata::lua_state* state) -> std::optional<lua_validate_result> {
    const auto ref = resolve_lua_function(state, path);
    if (!ref) { return std::nullopt; }
    ...
    auto res = ref->fn(params);
    check_func_result(res);
    if (!res.valid() || res.return_count() == 0) { return lua_validate_result{}; }
    const auto obj = res.get<sol::object>();
    if (obj == sol::lua_nil) { return lua_validate_result{}; }
    if (obj.is<bool>()) { return lua_validate_result{.ok = obj.as<bool>(), .reason = ""}; }
    if (obj.is<std::string>()) {
        const auto reason = obj.as<std::string>();
        return lua_validate_result{.ok = reason.empty(), .reason = reason};
    }
    if (!obj.is<sol::table>()) { return lua_validate_result{}; }
    ...
    if (ok_obj != sol::lua_nil && ok_obj.is<bool>()) { out.ok = ok_obj.as<bool>(); }
    ...
    if (!out.ok && out.reason.empty()) { out.reason = "invalid selection"; }
    return out;
}
...
auto spawn_fact_item(const proc::part_fact& fact) -> detached_ptr<item> {
    ...
    if (spawned->max_damage() > 0) { spawned->set_damage(damage_from_hp(*spawned, fact.hp)); }
    if (spawned->count_by_charges() && fact.chg > 0) { spawned->charges = fact.chg; }
    if (!fact.proc.empty()) { proc::set_payload_from_json(*spawned, fact.proc); }
    return spawned;
}
...
auto proc::run_full(
    ...
    if (const auto tbl =
            call_lua_blob(sch.lua.name, sch, facts, opts.picks, out.data, opts.state)) {
        parse_blob_into(out.data, *tbl);
    }
    return out;
}

auto proc::validate_selection(
    const schema& sch, const std::vector<part_fact>& facts, const fast_blob& blob,
    const validate_opts& opts) -> std::expected<void, std::string> {
    if (sch.lua.validate.empty()) { return {}; }
    const auto result = call_lua_validate(sch.lua.validate, sch, facts, blob, opts.state);
    if (!result.has_value() || result->ok) { return {}; }
    if (result->reason.empty()) { return std::unexpected(std::string("invalid selection")); }
    return std::unexpected(result->reason);
}

auto proc::make_item(const schema& sch, const std::vector<part_fact>& facts, const make_opts& opts)
    -> detached_ptr<item> {
    ...
    if (const auto tbl = call_lua_blob(
            sch.lua.make, sch, facts, lua_picks, full.data, opts.state, result_override)) {
        parse_blob_into(full.data, *tbl);
        const auto result_id = tbl->get_or<std::string>("result", "");
        if (!result_id.empty()) { result->convert(itype_id(result_id)); }
        const auto mode_name = tbl->get_or<std::string>("mode", "");
        if (!mode_name.empty()) { mode = io::string_to_enum<proc::hist>(mode_name); }
    }
    ...
    out_payload.servings =
        result->is_comestible() && result->count_by_charges() ? std::max(result->charges, 1) : 0;
    out_payload.fp = fast_fp(sch, full.data, facts);
    if (mode == hist::compact) {
        out_payload.parts =
            !opts.slots.empty()
                ? make_compact_parts(facts, opts.slots)
    ...
    if (mode == hist::full) {
        ...
        if (!opts.used.empty()) {
            std::ranges::for_each(opts.used, [&](const item* used) {
                if (used != nullptr) { components.push_back(item::spawn(*used)); }
            });
        } else {
            std::ranges::for_each(facts, [&](const part_fact& fact) {
