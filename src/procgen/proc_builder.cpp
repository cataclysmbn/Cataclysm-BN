...
auto split_atom(const std::string& atom) -> std::pair<std::string, std::string> {
    ...
    if (pos == std::string::npos) { return {atom, ""}; }
...
auto quality_match(const proc::part_fact& fact, const std::string& rhs) -> bool {
    ...
    if (cmp_pos == std::string::npos || cmp_pos == 0) { return false; }
    const auto qual = quality_id(rhs.substr(0, cmp_pos));
    const auto level_txt = rhs.substr(cmp_pos + 2);
    if (level_txt.empty()) { return false; }
    auto level = 0;
    const auto parse =
        std::from_chars(level_txt.data(), level_txt.data() + level_txt.size(), level);
    ...
    return std::ranges::any_of(fact.qual, [&](const std::pair<const quality_id, int>& entry) {
        return to_lower_case(entry.first.str()) == to_lower_case(qual.str())
            && entry.second >= level;
    });
}
...
auto split_search_terms(const std::string& query) -> std::vector<std::string> {
    ...
    for (auto term = std::string{}; input >> term;) { ret.push_back(term); }
    return ret;
}

auto join_none(const std::vector<std::string>& values) -> std::string {
    auto ret = std::string{};
    std::ranges::for_each(values, [&](const std::string& value) { ret += value; });
    return ret;
}
...
template <typename Func> auto average_material_stat(const proc::part_fact& fact, Func fn) -> int {
    if (fact.mat.empty()) { return 0; }
    auto total = 0;
    std::ranges::for_each(fact.mat, [&](const material_id& mat) {
        if (mat.is_valid()) { total += fn(mat.obj()); }
    });
    return total / static_cast<int>(fact.mat.size());
}

auto fact_density(const proc::part_fact& fact) -> int {
    return average_material_stat(fact, [](const material_type& mat) { return mat.density(); });
}

auto fact_bash_resist(const proc::part_fact& fact) -> int {
    return average_material_stat(fact, [](const material_type& mat) { return mat.bash_resist(); });
}

auto fact_cut_resist(const proc::part_fact& fact) -> int {
    return average_material_stat(fact, [](const material_type& mat) { return mat.cut_resist(); });
}

auto fact_chip_resist(const proc::part_fact& fact) -> int {
    return average_material_stat(fact, [](const material_type& mat) { return mat.chip_resist(); });
}
...
auto fact_soft_penalty(const proc::part_fact& fact) -> int {
    return average_material_stat(fact, [](const material_type& mat) { return mat.soft() ? 8 : 0; });
}
...
auto blade_material_rank(const proc::part_fact& fact) -> int {
    ...
    if (std::ranges::find(fact.mat, material_id("bone")) != fact.mat.end()) { return 2; }
    if (std::ranges::find(fact.mat, material_id("wood")) != fact.mat.end()) { return 1; }
    return 0;
}
...
auto weapon_role_score(const std::string& role, const proc::part_fact& fact) -> int {
    ...
    return bash_resist * 2 + cut_resist * 2 + chip_resist * 2 + density / 16 + reinforce_bonus * 2
         - soft_penalty * 2 + fact.mass_g / 200;
}

auto build_candidate_sort_entry(const proc::slot_data& slot, const proc::part_fact& fact)
    -> candidate_sort_entry {
...
auto sort_weapon_slot_candidates(
    const proc::slot_data& slot, const std::vector<proc::part_fact>& facts,
    ...
    std::ranges::sort(ranked, [&](const candidate_sort_entry& lhs, const candidate_sort_entry& rhs) {
        ...
        if (lhs.mass_g != rhs.mass_g) { return lhs.mass_g > rhs.mass_g; }
        if (lhs.volume_ml != rhs.volume_ml) { return lhs.volume_ml > rhs.volume_ml; }
    ...
    std::ranges::
        transform(ranked, std::back_inserter(slot_facts), [](const candidate_sort_entry& entry) {
            return entry.ix;
        });
}
...
auto has_itype(const std::vector<proc::part_fact>& facts, const itype_id& id) -> bool {
    return std::ranges::any_of(facts, [&](const proc::part_fact& fact) { return fact.id == id; });
}
...
auto primary_material_name(const std::vector<proc::part_fact>& facts) -> std::string {
    if (std::ranges::any_of(facts, [&](const proc::part_fact& fact) {
            return has_material(fact, material_id("steel"))
                || has_material(fact, material_id("iron"));
        })) {
        return "metal";
    }
    if (std::ranges::any_of(facts, [&](const proc::part_fact& fact) {
...
auto sword_base_description(const std::string& name) -> std::string {
    if (name == "nail sword") { return "A rough wooden sword stiffened with driven nails."; }
    if (name == "crude sword") {
    ...
    if (name == "bone sword") { return "A rough sword built around a sharpened bone blade."; }
    if (name == "2-by-sword") { return "A club-like sword carved from a sturdy length of wood."; }
...
auto sword_guard_phrase(const std::vector<proc::part_fact>& guard_facts) -> std::string {
    if (guard_facts.empty()) { return "no guard"; }
    const auto material = primary_material_name(guard_facts);
    if (material == "metal") { return "a metal guard"; }
    if (material == "bone") { return "a bone guard"; }
    if (material == "wooden") { return "a wooden guard"; }
    return "a simple guard";
}

auto sword_grip_phrase(const std::vector<proc::part_fact>& grip_facts) -> std::string {
    if (grip_facts.empty()) { return {}; }
    if (has_itype(grip_facts, itype_id("leather"))
        || std::ranges::any_of(grip_facts, [&](const proc::part_fact& fact) {
               return has_material(fact, material_id("leather"));
           })) {
        return "a leather-wrapped grip";
    }
    if (has_itype(grip_facts, itype_id("rag"))
        || std::ranges::any_of(grip_facts, [&](const proc::part_fact& fact) {
...
auto sword_hilt_sentence(
    const std::vector<proc::part_fact>& guard_facts,
    ...
    if (grip_phrase.empty()) { return string_format("The hilt uses %s.", guard_phrase); }
...
auto sword_reinforcement_sentence(const std::vector<proc::part_fact>& reinforcement_facts)
    ...
    if (has_nails) { return "Driven nails add stiffness and a little puncturing power."; }
    if (has_scrap) { return "Scrap reinforcement adds weight and stiffness."; }
    return {};
}
...
auto matching_slot_uses(const proc::schema& sch, const proc::part_fact& fact) -> int {
    ...
    std::ranges::for_each(sch.slots, [&](const proc::slot_data& slot) {
        if (proc::matches_slot(slot, fact)) { uses += std::max(slot.max, 1); }
    });
    return uses;
}

auto picked_facts_for_role(
    const proc::schema& sch, const std::vector<proc::part_fact>& facts,
    ...
    std::ranges::for_each(picks, [&](const proc::craft_pick& pick) {
        ...
        if (slot == nullptr || fact == nullptr || slot->role != role) { return; }
        ret.push_back(*fact);
    });
    return ret;
}
...
auto sword_description(
    const proc::schema& sch, const std::vector<proc::part_fact>& facts,
    const std::vector<proc::craft_pick>& picks, const std::string& name) -> std::string {
...
auto basic_preview(
    ...
    std::ranges::for_each(sch.slots, [&](const proc::slot_data& slot) {
        ...
        if (count > 0) { role_counts.push_back(role_count_name(slot.role, count)); }
    });
...
auto sword_preview(
    ...
    std::ranges::for_each(picks, [&](const proc::craft_pick& pick) {
        const auto* slot = slot_by_id(sch, pick.slot);
        const auto* fact = find_fact(facts, pick.ix);
        if (slot == nullptr || fact == nullptr) { return; }
        const auto density = fact_density(*fact);
        const auto bash_resist = fact_bash_resist(*fact);
        const auto cut_resist = fact_cut_resist(*fact);
        const auto chip_resist = fact_chip_resist(*fact);
...
auto proc::builder_state::picks_for(const slot_id& slot) const
    -> const std::vector<part_ix>& // *NOPAD*
{
...
auto proc::matches_atom(const part_fact& fact, const std::string& atom) -> bool {
    ...
    if (lhs == "itype") { return to_lower_case(fact.id.str()) == normalized_rhs; }
    if (lhs == "qual") { return quality_match(fact, normalized_rhs); }
    return false;
}

auto proc::matches_slot(const slot_data& slot, const part_fact& fact) -> bool {
    const auto allow_ok =
        slot.ok.empty() || std::ranges::any_of(slot.ok, [&](const std::string& atom) {
            return matches_atom(fact, atom);
        });
    if (!allow_ok) { return false; }
    return !std::ranges::any_of(slot.no, [&](const std::string& atom) {
        return matches_atom(fact, atom);
    });
}

auto proc::build_candidates(const schema& sch, const std::vector<part_fact>& facts)
    -> std::map<slot_id, std::vector<part_ix>> {
    ...
    std::ranges::for_each(sch.slots, [&](const slot_data& slot) {
        ...
        std::ranges::for_each(facts, [&](const part_fact& fact) {
            if (matches_slot(slot, fact)) { slot_facts.push_back(fact.ix); }
        });
...
auto proc::pick_count(const builder_state& state, const part_ix ix) -> int {
    auto total = 0;
    std::ranges::
        for_each(state.chosen, [&](const std::pair<const slot_id, std::vector<part_ix>>& entry) {
...
auto proc::remaining_uses(const builder_state& state, const part_ix ix) -> int {
    ...
    if (fact == nullptr) { return 0; }
    return std::max(fact->uses - pick_count(state, ix), 0);
}

auto proc::filter_available_candidates(
    const builder_state& state, const std::vector<part_ix>& candidates) -> std::vector<part_ix> {
    auto ret = std::vector<part_ix>{};
    ret.reserve(candidates.size());
    std::ranges::for_each(candidates, [&](const part_ix ix) {
        if (remaining_uses(state, ix) > 0) { ret.push_back(ix); }
    });
    return ret;
}

auto proc::slot_can_meet_minimum(const builder_state& state, const schema& sch, const slot_id& slot)
    -> bool {
    const auto* slot_data = slot_by_id(sch, slot);
    if (slot_data == nullptr) { return false; }
    if (slot_data->min <= 0) { return true; }
    const auto iter = state.cand.find(slot);
    if (iter == state.cand.end()) { return false; }
...
auto proc::slot_complete(const builder_state& state, const schema& sch, const slot_id& slot)
    -> bool {
    const auto* slot_data = slot_by_id(sch, slot);
    if (slot_data == nullptr) { return false; }
...
auto proc::selected_picks(const builder_state& state, const schema& sch)
    -> std::vector<craft_pick> {
...
auto proc::selected_facts(const builder_state& state) -> std::vector<part_fact> {
    ...
    if (!state.sch.id.is_null()) {
        ...
        std::ranges::for_each(picks, [&](const craft_pick& pick) {
            if (const auto* fact = find_fact(state.facts, pick.ix)) { ret.push_back(*fact); }
        });
        return ret;
    }
    ...
    std::ranges::
        for_each(state.chosen, [&](const std::pair<const slot_id, std::vector<part_ix>>& entry) {
            std::ranges::for_each(entry.second, [&](const part_ix ix) {
                if (const auto* fact = find_fact(state.facts, ix)) { ret.push_back(*fact); }
            });
        });
    return ret;
}
...
auto proc::rebuild_fast(const builder_state& state) -> fast_blob {
    ...
    if (state.sch.id.is_null()) { return fast_blob{}; }
    return preview_blob(state.sch, facts, selected_picks(state, state.sch));
}

auto proc::preview_result_override(
    const schema& sch, const std::vector<part_fact>& facts,
...
auto proc::debug_part_fact(const schema& sch, const item& it, const part_ix ix)
    -> std::optional<part_fact> {
    auto fact = normalize_part_fact(
        it,
        {
            .ix = ix,
...
auto proc::add_pick(builder_state& state, const schema& sch, const slot_id& slot, const part_ix ix)
    -> bool {
    const auto* slot_data = slot_by_id(sch, slot);
    if (slot_data == nullptr) { return false; }
    ...
    const auto* fact = find_fact(state.facts, ix);
    if (fact == nullptr || pick_count(state, ix) >= fact->uses) { return false; }

    auto& picked = state.chosen[slot];
    if (!slot_data->rep && std::ranges::find(picked, ix) != picked.end()) { return false; }
    if (slot_data->max > 0 && static_cast<int>(picked.size()) >= slot_data->max) { return false; }
...
auto proc::remove_pick(builder_state& state, const slot_id& slot, const part_ix ix) -> bool {
    ...
    if (iter == state.chosen.end()) { return false; }
    const auto erase_iter = std::ranges::find(iter->second, ix);
    if (erase_iter == iter->second.end()) { return false; }
...
auto proc::remove_last_pick(builder_state& state, const slot_id& slot) -> bool {
    const auto iter = state.chosen.find(slot);
    if (iter == state.chosen.end() || iter->second.empty()) { return false; }
    iter->second.pop_back();
    state.fast = rebuild_fast(state);
    return true;
}

auto proc::fast_fp(const schema& sch, const fast_blob& blob, const std::vector<part_fact>& facts)
    -> std::string {
    auto ids = std::vector<std::string>{};
    ids.reserve(facts.size());
    std::ranges::for_each(facts, [&](const part_fact& fact) {
        ids.push_back(
            string_format("%s:%d:%d", fact.id.str(), fact.chg, static_cast<int>(fact.hp * 1000)));
    });
    std::ranges::sort(ids);
    const auto joined = join_none(ids);
    const auto hash = std::hash<std::string>{}(string_format(
        "%s|%d|%d|%d|%d|%d|%d|%d|%d|%s", sch.id.str(), blob.mass_g, blob.volume_ml, blob.kcal,
        blob.melee.bash, blob.melee.cut, blob.melee.stab, blob.melee.to_hit, blob.melee.dur,
        blob.melee.moves, joined));
    return string_format("%s:%08x", sch.id.str(), static_cast<unsigned int>(hash & 0xffffffffU));
}

auto proc::part_search_texts(const part_fact& fact, const proc::part_search_options& opts)
    -> std::vector<std::string> {
    auto ret = std::vector<std::string>{
        opts.name, fact.id.str(), string_format("itype:%s", fact.id.str()), opts.where};
...
auto proc::part_matches_search(
    const part_fact& fact, const proc::part_search_options& opts, const std::string& txt) -> bool {
    ...
    return std::ranges::all_of(terms, [&](const std::string& term) {
        if (is_search_atom(term)) {
            return matches_atom(fact, term)
                || std::ranges::any_of(texts, [&](const std::string& entry) {
...
auto proc::recipe_matches_search(const recipe& rec, const std::string& txt) -> bool {
    auto needle = txt;
    if (needle.starts_with("c:")) { needle = needle.substr(2); }
