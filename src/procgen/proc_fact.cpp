...
auto default_tags(const item& it) -> std::vector<std::string> { return proc::food_tags(it); }
...
auto proc::normalize_part_fact(const item& it, const proc::normalize_options& opts) -> part_fact {
    ...
    const auto selected_charges =
        it.count_by_charges() ? std::clamp(opts.charges > 0 ? opts.charges : 1, 1, full_charges) : 0;
    const auto uses = std::max(opts.uses, 1);
    const auto scale =
        it.count_by_charges()
            ? static_cast<double>(selected_charges) / static_cast<double>(full_charges)
            : 1.0;
    ...
    fact.mass_g = static_cast<int>(
        std::lround(static_cast<double>(units::to_gram(it.weight())) * scale));
    fact.volume_ml = static_cast<int>(
        std::lround(static_cast<double>(units::to_milliliter(it.volume())) * scale));
    fact.kcal =
        it.is_comestible()
            ? it.get_comestible()->default_nutrition.kcal * std::max(selected_charges, 1)
            : 0;
    ...
    if (const auto payload = proc::read_payload(it)) { fact.proc = proc::payload_json(*payload); }
    return fact;
}
