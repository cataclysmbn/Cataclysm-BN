...
auto trim_copy(const std::string& text) -> std::string {
    ...
    const auto last =
        std::ranges::find_if_not(text.rbegin(), text.rend(), [](const unsigned char ch) {
            return std::isspace(ch) != 0;
        }).base();
...
auto proc::grouped_label_summary(
    const std::vector<std::string>& labels, const std::string& empty_label) -> std::string {
    const auto grouped = group_duplicate_labels(labels);
    if (grouped.empty()) { return empty_label; }
...
auto proc::compact_requirement_text(const std::string& text) -> std::string {
    ...
    for (const char ch : text) {
        if (ch == '\n') {
            ...
            if (line.ends_with(':')) {
                if (!header.empty() && header != line) { flush_section(); }
                header = line;
                continue;
            }
            if (header.empty()) {
    ...
    if (!last_line.empty()) {
        if (last_line.ends_with(':')) {
            if (!header.empty() && header != last_line) { flush_section(); }
            header = last_line;
