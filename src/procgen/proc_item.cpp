...
auto default_stack_servings(const itype_id& id) -> int {
    ...
    if (const auto iter = cache.find(id); iter != cache.end()) { return iter->second; }
...
auto payload_servings(const item& it, const proc::payload& data) -> int {
    if (data.servings > 0) { return data.servings; }
...
auto scaled_payload_total(const item& it, const proc::payload& data, const int total) -> int {
    if (!it.count_by_charges()) { return total; }

    const auto servings = payload_servings(it, data);
    if (servings <= 0) { return total; }

    return static_cast<int>(std::lround(
        static_cast<double>(total) * static_cast<double>(std::max(it.charges, 0))
...
auto per_serving_payload_value(const item& it, const proc::payload& data, const int total) -> int {
    ...
    return static_cast<int>(
        std::lround(static_cast<double>(total) / static_cast<double>(servings)));
}

auto default_food_blob(const item& it, const itype_id& source_id) -> proc::fast_blob {
    ...
    blob.mass_g = static_cast<int>(std::lround(
        static_cast<double>(units::to_gram(sample->weight())) * static_cast<double>(current_charges)
        / static_cast<double>(sample_charges)));
    blob.volume_ml = static_cast<int>(std::lround(
        static_cast<double>(units::to_milliliter(sample->volume()))
        * static_cast<double>(current_charges) / static_cast<double>(sample_charges)));
    blob.name = sample->type_name();
    blob.description = sample->type->description.translated();
    if (!sample->is_comestible()) { return blob; }
...
auto damage_from_hp(const item& it, const float hp) -> int {
    if (it.max_damage() <= 0) { return 0; }
    const auto clamped = std::clamp(hp, 0.0f, 1.0f);
    return std::clamp(static_cast<int>((1.0f - clamped) * static_cast<float>(it.max_damage())), 0,
                      it.max_damage());
}

auto make_legacy_weapon_fact(const legacy_weapon_part_spec& spec, const proc::part_ix ix)
    -> proc::part_fact {
    const auto sample = item(spec.id, calendar::turn);
    return proc::normalize_part_fact(
        sample,
        {
            .ix = ix,
        });
...
auto legacy_sandwich_specs(const itype_id& id) -> std::vector<legacy_sandwich_part_spec> {
    ...
    if (id == itype_id("sandwich_veggy")) {
        return {legacy_sandwich_part_spec{.role = "bread", .id = itype_id("bread"), .count = 2},
...
auto legacy_weapon_specs(const itype_id& id) -> std::optional<legacy_weapon_spec> {
    ...
    if (id == itype_id("sword_bone")) {
        return legacy_weapon_spec{
            .schema = proc::schema_id("sword"),
            .fp_prefix = "sword",
            .parts = {legacy_weapon_part_spec{.role = "blade", .id = itype_id("bone")},
                      legacy_weapon_part_spec{.role = "handle", .id = itype_id("stick_long")},
                      legacy_weapon_part_spec{.role = "grip", .id = itype_id("leather")}}};
    }
    if (id == itype_id("hand_axe")) {
        return legacy_weapon_spec{
            ...
            .fp_prefix = "axe",
            .parts = {legacy_weapon_part_spec{.role = "head", .id = itype_id("rock")},
                      legacy_weapon_part_spec{.role = "handle", .id = itype_id("stick_long")}}};
    }
    if (id == itype_id("makeshift_axe")) {
        return legacy_weapon_spec{
            .schema = proc::schema_id("axe"),
            .fp_prefix = "axe",
            .parts = {legacy_weapon_part_spec{.role = "head", .id = itype_id("steel_chunk")},
                      legacy_weapon_part_spec{.role = "handle", .id = itype_id("stick_long")},
    ...
    if (id == itype_id("spear_spike")) {
        return legacy_weapon_spec{
            ...
            .fp_prefix = "spear",
            .parts = {legacy_weapon_part_spec{.role = "tip", .id = itype_id("spike")},
    ...
    if (id == itype_id("spear_knife")) {
        return legacy_weapon_spec{
            .schema = proc::schema_id("spear"),
            .fp_prefix = "spear",
            .parts = {legacy_weapon_part_spec{.role = "tip", .id = itype_id("knife_hunting")},
                      legacy_weapon_part_spec{.role = "shaft", .id = itype_id("stick_long")},
                      legacy_weapon_part_spec{.role = "binding", .id = itype_id("string_36")}}};
    }
    ...
    if (id == itype_id("spear_stone")) {
        return legacy_weapon_spec{
            .schema = proc::schema_id("spear"),
            .fp_prefix = "spear",
            .parts = {legacy_weapon_part_spec{.role = "tip", .id = itype_id("rock")},
                      legacy_weapon_part_spec{.role = "shaft", .id = itype_id("stick_long")},
                      legacy_weapon_part_spec{.role = "binding", .id = itype_id("string_36")}}};
    }
    if (id == itype_id("spear_steel")) {
        return legacy_weapon_spec{
            .schema = proc::schema_id("spear"),
            .fp_prefix = "spear",
            .parts = {legacy_weapon_part_spec{.role = "tip", .id = itype_id("steel_chunk")},
                      legacy_weapon_part_spec{.role = "shaft", .id = itype_id("stick_long")},
                      legacy_weapon_part_spec{.role = "binding", .id = itype_id("string_36")}}};
    }
    return std::nullopt;
...
auto to_json(JsonOut& jsout, const proc::part_fact& fact) -> void {
    ...
    jsout.member("flag");
    jsout.start_array();
    std::ranges::for_each(fact.flag, [&](const flag_id& entry) { jsout.write(entry); });
    jsout.end_array();
    jsout.member("qual");
    jsout.start_object();
    ...
    jsout.member("mat");
    jsout.start_array();
    std::ranges::for_each(fact.mat, [&](const material_id& entry) { jsout.write(entry); });
    jsout.end_array();
    jsout.member("vit");
    jsout.start_object();
    ...
    if (!fact.proc.empty()) { jsout.member("proc", fact.proc); }
    jsout.end_object();
}
...
auto from_json(JsonObject& jo, proc::part_fact& fact) -> void {
    ...
    if (jo.has_array("flag")) {
        ...
        while (arr.has_more()) { fact.flag.push_back(flag_id(arr.next_string())); }
    }
    ...
    if (jo.has_array("mat")) {
        ...
        while (arr.has_more()) { fact.mat.push_back(material_id(arr.next_string())); }
    }
...
auto proc::to_json(JsonOut& jsout, const compact_part& part) -> void {
    ...
    jsout.member("mat");
    jsout.start_array();
    std::ranges::for_each(part.mat, [&](const material_id& entry) { jsout.write(entry); });
    jsout.end_array();
    if (!part.proc.empty()) { jsout.member("proc", part.proc); }
    jsout.end_object();
}
...
auto proc::from_json(JsonIn& jsin, compact_part& part) -> void {
    ...
    if (jo.has_array("mat")) {
        auto arr = jo.get_array("mat");
        while (arr.has_more()) { part.mat.push_back(material_id(arr.next_string())); }
    }
...
auto proc::to_json(JsonOut& jsout, const payload& data) -> void {
    ...
    if (!data.blob.description.empty()) { jsout.member("description", data.blob.description); }
    jsout.member("melee");
    jsout.start_object();
    ...
    jsout.member("parts");
    jsout.start_array();
    std::ranges::for_each(data.parts, [&](const compact_part& entry) { to_json(jsout, entry); });
    jsout.end_array();
    jsout.end_object();
}
...
auto proc::from_json(JsonIn& jsin, payload& data) -> void {
    ...
    data.mode =
        jo.has_member("mode")
            ? io::string_to_enum<proc::hist>(jo.get_string("mode"))
            : proc::hist::none;
    jo.read("fp", data.fp);
...
auto proc::to_json(JsonOut& jsout, const craft_plan& data) -> void {
    ...
    jsout.member("slots");
    jsout.start_array();
    std::ranges::for_each(data.slots, [&](const slot_id& slot) { jsout.write(slot); });
    jsout.end_array();
    jsout.member("facts");
    jsout.start_array();
    std::ranges::for_each(data.facts, [&](const part_fact& fact) { ::to_json(jsout, fact); });
    jsout.end_array();
    jsout.end_object();
}
...
auto proc::from_json(JsonIn& jsin, craft_plan& data) -> void {
    ...
    if (jo.has_array("slots")) {
        ...
        while (arr.has_more()) { data.slots.push_back(slot_id(arr.next_string())); }
    }
...
auto proc::payload_json(const payload& data) -> std::string {
    return serialize_wrapper([&](JsonOut& jsout) { to_json(jsout, data); });
}

auto proc::read_payload(const item& it) -> std::optional<payload> {
    if (!it.has_var(std::string(proc_var_key))) { return std::nullopt; }

    auto ret = payload{};
    try {
        deserialize_wrapper(
            [&](JsonIn& jsin) { from_json(jsin, ret); }, it.get_var(std::string(proc_var_key)));
...
auto proc::read_craft_plan(const item& it) -> std::optional<craft_plan> {
    if (!it.has_var(std::string(proc_craft_var_key))) { return std::nullopt; }

    auto ret = craft_plan{};
    try {
        deserialize_wrapper(
            [&](JsonIn& jsin) { from_json(jsin, ret); },
...
auto proc::write_payload(item& it, const payload& data) -> void {
    ...
    if (!data.blob.name.empty()) { it.set_var("name", data.blob.name); }
    if (!data.blob.description.empty()) { it.set_var("description", data.blob.description); }
    if (it.is_comestible() && !data.blob.empty()) { it.set_flag(flag_NUTRIENT_OVERRIDE); }
}

auto proc::write_craft_plan(item& it, const craft_plan& data) -> void {
    it.set_var(std::string(proc_craft_var_key),
               serialize_wrapper([&](JsonOut& jsout) { to_json(jsout, data); }));
}
...
auto legacy_sandwich_payload(const item& it, const itype_id& legacy_id) -> std::optional<payload> {
    ...
    std::ranges::
        transform(specs, std::back_inserter(out.parts), [&](const legacy_sandwich_part_spec& spec) {
            return make_legacy_sandwich_part(spec);
        });
    return out;
}

auto legacy_weapon_payload(const item&, const itype_id& legacy_id) -> std::optional<payload> {
    ...
    std::ranges::for_each(std::views::iota(size_t{0}, spec->parts.size()), [&](const size_t idx) {
...
auto proc::clear_payload(item& it) -> void { it.erase_var(std::string(proc_var_key)); }

auto proc::make_compact_parts(const std::vector<part_fact>& facts, const schema& sch)
    ...
    std::ranges::for_each(sch.slots, [&](const slot_data& slot) {
        std::ranges::for_each(facts, [&](const part_fact& fact) {
            if (!matches_slot(slot, fact)) { return; }
            auto part = compact_part{};
            part.role = slot.role;
            part.id = fact.id;
...
auto proc::make_compact_parts(
    const std::vector<part_fact>& facts,
...
auto proc::restore_parts(const payload& data) -> std::vector<detached_ptr<item>> {
    ...
    std::ranges::for_each(data.parts, [&](const compact_part& part) {
        ...
        if (spawned->count_by_charges() && part.chg > 0) { spawned->charges = part.chg; }
        if (!part.proc.empty()) { set_payload_from_json(*spawned, part.proc); }
...
auto proc::apply_on_damage(item& it, const int qty) -> void {
    ...
    if (!payload || payload->mode != hist::compact) { return; }
    ...
    std::ranges::for_each(updated.parts, [&](compact_part& part) {
        part.dmg += qty;
        part.hp =
            std::max(0.0f, part.hp - static_cast<float>(qty) / static_cast<float>(max_damage));
    });
...
auto proc::blob_kcal(const item& it) -> std::optional<int> {
    const auto payload = read_payload(it);
    return payload ? std::optional<int>(per_serving_payload_value(it, *payload, payload->blob.kcal))
                   : std::nullopt;
}
...
auto proc::blob_volume(const item& it) -> std::optional<int> {
    const auto payload = read_payload(it);
    return payload ? std::optional<int>(scaled_payload_total(it, *payload, payload->blob.volume_ml))
                   : std::nullopt;
}
