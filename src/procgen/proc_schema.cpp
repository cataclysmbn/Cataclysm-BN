...
auto proc::slot_data::load(const JsonObject& jo) -> void {
    ...
    if (jo.has_array("ok")) {
        ok.clear();
        auto ok_arr = jo.get_array("ok");
        while (ok_arr.has_more()) { ok.push_back(ok_arr.next_string()); }
    }
    if (jo.has_array("no")) {
        no.clear();
        auto no_arr = jo.get_array("no");
        while (no_arr.has_more()) { no.push_back(no_arr.next_string()); }
    }
}
...
auto proc::schema::load(const JsonObject& jo, const std::string&) -> void {
    ...
    if (jo.has_object("hist")) { hist.load(jo.get_object("hist")); }
    if (jo.has_object("lua")) { lua.load(jo.get_object("lua")); }
...
auto proc::schema::check() const -> void {
    ...
    std::ranges::for_each(slots, [&](const slot_data& slot) {
        if (slot.id.is_null()) { debugmsg("proc schema %s has slot with null id", id.c_str()); }
...
auto proc::check() -> void { proc_schema_factory.check(); }

auto proc::reset() -> void { proc_schema_factory.reset(); }
...
auto proc::has(const schema_id& id) -> bool { return proc_schema_factory.is_valid(id); }
