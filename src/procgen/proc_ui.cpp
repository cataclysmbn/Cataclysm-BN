...
auto make_source_entry(const source_options& opts) -> source_entry {
    return source_entry{
        ...
        .prefix = opts.prefix,
        .where = opts.where,
        .fact = proc::normalize_part_fact(
            *opts.src, {.ix = opts.ix, .charges = opts.charges, .uses = opts.uses})};
}

auto current_recipe_requirement_status(
    Character& who, const recipe& rec,
    const std::vector<proc::part_fact>& facts) -> recipe_requirement_status {
    const auto reqs = proc::recipe_requirements(rec, facts);
    const auto ready = reqs.can_make_with_inventory(
        who.crafting_inventory(), rec.get_component_filter(), 1, cost_adjustment::start_only);
    const auto missing = ready ? std::string{} : reqs.list_missing();
...
{
    if (ix < 0 || static_cast<size_t>(ix) >= sources.size()) { return nullptr; }
...
auto candidate_line(const source_entry& source, const proc::schema& sch) -> std::string {
    return string_format(
        "%s %s  [%s]", source.prefix, source.src->tname(), candidate_meta(source, sch));
}

auto filtered_candidates(
    const proc::builder_state& state, const proc::slot_id& slot, const proc::schema& sch,
    const std::vector<source_entry>& sources,
    ...
    std::ranges::
        transform(sources, std::back_inserter(candidate_sources), [&](const source_entry& source) {
...
auto slot_summary(
    const proc::builder_state& state, const proc::slot_data& slot,
...
auto candidate_meta(const source_entry& source, const proc::schema& sch) -> std::string {
    if (sch.cat == "food") { return string_format("+%d kcal", source.fact.kcal); }
    if (sch.cat == "weapon") { return string_format("%d g", source.fact.mass_g); }
...
auto highlighted_preview(
    proc::builder_state state, const proc::schema& sch, const proc::slot_id& slot,
    ...
    if (chosen == proc::invalid_part_ix) { return state.fast; }
    if (const auto slot_data = std::ranges::
            find_if(sch.slots, [&](const proc::slot_data& entry) { return entry.id == slot; });
        slot_data != sch.slots.end() && slot_data->max == 1 && !state.picks_for(slot).empty()) {
        clear_slot(state, slot);
    }
    if (!proc::add_pick(state, sch, slot, chosen)) { return state.fast; }
    return state.fast;
}

auto diff_line(
    const std::string& label, const int current, const int preview,
    const std::string& suffix) -> std::string {
    if (current == preview) { return string_format("%-10s %d%s", label, current, suffix); }
    if (current == 0) { return string_format("%-10s %d%s", label, preview, suffix); }
    return string_format("%-10s %d%s -> [ %d%s ]", label, current, suffix, preview, suffix);
}

auto selected_ingredient_lines(
    const proc::builder_state& state, const proc::schema& sch,
...
auto gather_inventory_sources(Character& who, const proc::schema& sch) -> source_pool {
    ...
    std::ranges::for_each(inv.const_slice(), [&](const std::vector<item*>* stack) {
        ...
        std::ranges::for_each(*stack, [&](item* it) {
            if (it == nullptr || it->is_craft()) { return; }
...
auto missing_required_slots(const proc::schema& sch, const proc::builder_state& state)
    -> std::vector<std::string> {
    auto ret = std::vector<std::string>{};
    std::ranges::for_each(sch.slots, [&](const proc::slot_data& slot) {
        if (!proc::slot_can_meet_minimum(state, sch, slot.id)) { ret.push_back(slot.role); }
    });
    return ret;
}
...
auto preview_lines(
    const proc::schema& sch, const proc::builder_state& state, const proc::fast_blob& preview_blob,
    const std::vector<source_entry>& sources) -> std::vector<std::string> {
    auto lines = std::vector<std::string>{};
    lines.push_back(string_format(
        _("Result: %s"), preview_blob.name.empty() ? sch.res.str() : preview_blob.name));
    ...
    if (!preview_blob.vit.empty()) {
        ...
        lines.push_back(string_format(
            _("Vit: %s"), enumerate_as_string(vitamins, enumeration_conjunction::none)));
    }
    if (!preview_blob.melee.empty()) {
        ...
        lines.push_back(
            diff_line(_("To-hit:"), state.fast.melee.to_hit, preview_blob.melee.to_hit, ""));
...
auto preview_slots(const proc::builder_state& state, const proc::schema& sch)
    -> std::vector<proc::slot_id> {
    auto slots = std::vector<proc::slot_id>{};
    const auto picks = proc::selected_picks(state, sch);
    slots.reserve(picks.size());
    std::ranges::for_each(picks, [&](const proc::craft_pick& pick) { slots.push_back(pick.slot); });
    return slots;
}

auto preview_item_from_state(
    const proc::builder_state& state, const proc::schema& sch,
    ...
    if (facts.empty()) {
        ...
        fallback->set_var(
            "name",
            rec.builder_name().translated().empty()
                ? rec.result_name()
                : rec.builder_name().translated());
        fallback->set_var(
            "description",
            rec.builder_desc().translated().empty()
                ? _("Select parts to preview the crafted item.")
                : rec.builder_desc().translated());
        return fallback;
    }
    return proc::make_item(
        sch, facts,
        {.mode = proc::hist::none, .rec = &rec, .used = {}, .slots = preview_slots(state, sch)});
}

auto preview_item_info(
    const proc::schema& sch, const proc::builder_state& state, const proc::fast_blob& preview_blob,
    const std::vector<source_entry>&, const recipe& rec, int& preview_scroll) -> item_info_data {
    ...
    auto data = item_info_data(
        display_item->tname(), std::string{}, display_item->info(), compare_info, preview_scroll);
...
auto draw_slot_indicator(
    const catacurses::window& w, const point& pos, const proc::slot_data& slot, const int picked,
...
auto proc::open_builder(Character& who, const recipe& rec) -> std::optional<ui_result> {
    if (!rec.is_proc() || !proc::has(rec.proc_id())) { return std::nullopt; }
    ...
    auto source_data =
        who.has_trait(trait_DEBUG_HS)
            ? gather_debug_sources(sch)
            : gather_inventory_sources(who, sch);

    const auto facts =
        source_data.entries
        | std::views::transform([](const source_entry& entry) { return entry.fact; })
    ...
    ui.on_redraw([&](ui_adaptor&) {
        ...
        auto candidates =
            filtered_candidates(state, slot.id, sch, source_data.entries, search_query);
        ...
        trim_and_print(
            w, point(left_width + 3, content_top - 1), middle_width - 2,
            focus != panel_focus::slots ? c_yellow : c_light_gray,
            string_format(
                _("[2. Candidates: %s ]"), search_query.empty() ? slot.role : search_query));
        trim_and_print(w, point(left_width + middle_width + 5, content_top - 1), right_width - 1,
                       c_light_gray, _("[3. Preview ]"));
        trim_and_print(w, point(left_width + 3, search_row), middle_width - 2, c_light_gray,
                       string_format(_("Search: /%s"), search_query));

        auto slot_start = 0;
        calcStartPos(slot_start, slot_cursor, content_height, static_cast<int>(sch.slots.size()));
        const auto slot_end =
            std::min(static_cast<int>(sch.slots.size()), slot_start + content_height);
        std::ranges::for_each(std::views::iota(slot_start, slot_end), [&](const int row) {
            ...
            const auto color =
                selected ? c_yellow
                : proc::slot_complete(state, sch, slot_entry.id)
                    ? c_light_green
                    : c_white;
            ...
            const auto indicator_width = draw_slot_indicator(
                w, point(2 + utf8_width(prefix), y), slot_entry, picked, selected, color);
            trim_and_print(
                w, point(2 + utf8_width(prefix) + indicator_width, y),
        ...
        const auto cand_end =
            std::min(static_cast<int>(candidates.size()), cand_start + content_height);
        std::ranges::for_each(std::views::iota(cand_start, cand_end), [&](const int row) {
            const auto color =
                row == cand_cursor && focus != panel_focus::slots ? c_yellow : c_white;
            trim_and_print(
                w, point(left_width + 3, list_top + row - cand_start), middle_width - 2, color,
                proc::grouped_candidate_label(candidates[static_cast<size_t>(row)]));
        });

        const auto recipe_requirements =
            current_recipe_requirement_status(who, rec, proc::selected_facts(state));

        const auto readiness =
            !proc::complete(state, sch) ? proc::builder_readiness::missing_required_slots
            : recipe_requirements.ready
                ? proc::builder_readiness::ready_to_craft
                : proc::builder_readiness::missing_recipe_requirements;
        const auto ready_color =
            readiness == proc::builder_readiness::ready_to_craft ? c_light_green : c_light_red;
        trim_and_print(w, point(2, height - 3), width - 4, ready_color,
                       proc::builder_readiness_label(readiness));
        const auto idle_status =
            readiness == proc::builder_readiness::missing_recipe_requirements
                ? proc::compact_requirement_text(recipe_requirements.missing)
                : _("[Arrows] Navigate  [/] Search  [Enter] Add  [r] Remove  [c] Clear  [f] Craft  "
                    "[Esc] Cancel");
        trim_and_print(w, point(2, height - 2), width - 4, c_light_gray,
                       status.empty() ? idle_status : status);
        wnoutrefresh(w);

        auto preview_data =
            preview_item_info(sch, state, preview_blob, source_data.entries, rec, preview_scroll);
    ...
    while (true) {
        ...
        if (action == "QUIT") { return std::nullopt; }
        ...
        if (ch == 'r' || ch == KEY_BACKSPACE || ch == KEY_DC) {
            status =
                proc::remove_last_pick(state, slot.id)
                    ? _("Removed last pick.")
        ...
        if (ch == 'f') {
            ...
            if (const auto valid =
                    proc::validate_selection(sch, proc::selected_facts(state), state.fast);
                !valid) {
                status = valid.error();
                continue;
            }
            ...
            std::ranges::for_each(picks, [&](const proc::craft_pick& pick) {
                if (const auto* source = source_for_ix(source_data.entries, pick.ix)) {
                    result.picks.push_back(
                        ui_pick{.slot = pick.slot, .src = source->src, .fact = source->fact});
                }
            });
            return result;
        }
        if (action == "CONFIRM") {
            ...
            const auto picked_ix = proc::first_grouped_candidate_ix(
                candidates[static_cast<size_t>(cand_cursor)]);
            if (picked_ix == proc::invalid_part_ix) {
                status = _("No candidates for this slot.");
                continue;
            }
            if (slot.max == 1 && !state.picks_for(slot.id).empty()
                && std::ranges::find(state.picks_for(slot.id), picked_ix)
                       == state.picks_for(slot.id).end()) {
                clear_slot(state, slot.id);
            }
            ...
            status =
                proc::remove_pick(state, slot.id, picked_ix)
                    ? _("Removed selection.")
