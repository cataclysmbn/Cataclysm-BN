...
auto is_search_navigation_action(const std::string& action) -> bool {
    constexpr auto search_navigation_actions =
        std::array<std::string_view, 6>{"UP", "DOWN", "PAGE_UP", "PAGE_DOWN", "HOME", "END"};
...
auto handle_builder_search_input(const builder_input_options& opts) -> builder_input_result {
    ...
    if (opts.focus == builder_focus::search) {
        if (opts.ch == KEY_BACKSPACE || opts.ch == KEY_DC) {
            if (!result.search_query.empty()) { result.search_query.pop_back(); }
            result.handled = true;
            return result;
        }
        if (opts.action == "LEFT") {
...
auto handle_builder_slot_navigation(const builder_slot_navigation_options& opts)
    ...
    if (opts.focus != builder_focus::slots || opts.slot_count <= 0) { return result; }
    ...
    if (next_cursor != opts.slot_cursor) { result.search_query.clear(); }
    return result;
}
...
auto handle_builder_candidate_navigation(const builder_candidate_navigation_options& opts)
    ...
    if (opts.focus != builder_focus::candidates || opts.candidate_count <= 0) { return result; }
    ...
    if (opts.action == "PAGE_DOWN") {
        ...
        result.candidate_cursor =
            std::min(opts.candidate_cursor + opts.page_size, opts.candidate_count - 1);
        return result;
    }
    if (opts.action == "HOME") {
        result.handled = true;
