...
void talk_function::buy_10_logs( npc &p )
{
    ...
    std::vector<tripoint_abs_omt> places = get_overmapbuffer( get_player_character().get_dimension() ).find_all(
    ...
    for( const tripoint_abs_omt &i : places ) {
        if( &cur_om == get_overmapbuffer( get_player_character().get_dimension() ).get_existing_om_global( i ).om ) {
...
void talk_function::buy_100_logs( npc &p )
{
    ...
    std::vector<tripoint_abs_omt> places =
        get_overmapbuffer( get_player_character().get_dimension() ).find_all( get_player_character().global_omt_location(), find_params );
    if( places.empty() ) {
        debugmsg( "Couldn't find %s", "ranch_camp_67" );
        return;
    }
