                           ...
                           const std::string &female_msg )
{
    ...
    const oter_id &cur_ter = get_overmapbuffer( get_avatar().get_dimension() ).ter( g->u.global_omt_location() );
...
void memorial_logger::write( std::ostream &file, const std::string &epitaph ) const
{
    ...
    const std::string locdesc =
        get_overmapbuffer( get_avatar().get_dimension() ).get_description_at( tripoint_abs_sm( u.global_sm_location() ) );
