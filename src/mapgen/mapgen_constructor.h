...
public:
    ...
    auto generate(
        const tripoint_abs_omt& omt_pos, const time_point& when,
    ...
    auto points_in_rectangle(const point_omt_ms& from, const point_omt_ms& to) const
        -> point_range<point_omt_ms>;
    auto points_in_radius(const point_omt_ms& center, size_t radius) const
        -> point_range<point_omt_ms>;
    ...
    auto add_field(
        const point_omt_ms& p, const field_type_id& type_id, int intensity = INT_MAX,
    ...
    auto add_computer(const point_omt_ms& p, const std::string& name, int security) -> computer*;
    ...
    auto add_item_or_charges(
        const point_omt_ms& pos, detached_ptr<item>&& obj, bool overflow = true)
        -> detached_ptr<item>;
    auto spawn_an_item(
        const point_omt_ms& p, detached_ptr<item>&& new_item, int charges, int damlevel)
        -> detached_ptr<item>;
    auto spawn_items(const point_omt_ms& p, std::vector<detached_ptr<item>> new_items)
        -> std::vector<detached_ptr<item>>;
    auto spawn_item(
        const point_omt_ms& p, const itype_id& type_id, unsigned quantity = 1, int charges = 0,
        const time_point& birthday = calendar::start_of_cataclysm, int damlevel = 0) -> void;
    auto put_items_from_loc(
        const item_group_id& loc, const point_omt_ms& p,
        const time_point& turn = calendar::start_of_cataclysm) -> std::vector<item*>;
    auto place_items(
        const item_group_id& loc, int chance, const point_omt_ms& p1, const point_omt_ms& p2,
    ...
    auto draw_line_ter(const ter_id& type, const point_omt_ms& p1, const point_omt_ms& p2) -> void;
    auto draw_line_furn(const furn_id& type, const point_omt_ms& p1, const point_omt_ms& p2)
        -> void;
    ...
    auto draw_square_ter(const ter_id& type, const point_omt_ms& p1, const point_omt_ms& p2)
        -> void;
    auto draw_square_ter(ter_id (*f)(), const point_omt_ms& p1, const point_omt_ms& p2) -> void;
    auto draw_square_ter(
        const weighted_int_list<ter_id>& f, const point_omt_ms& p1, const point_omt_ms& p2) -> void;
    auto draw_square_furn(const furn_id& type, const point_omt_ms& p1, const point_omt_ms& p2)
        -> void;
    auto draw_rough_circle_ter(const ter_id& type, const point_omt_ms& p, int rad) -> void;
    auto draw_rough_circle_furn(const furn_id& type, const point_omt_ms& p, int rad) -> void;
    ...
    auto place_spawns(
        const mongroup_id& group, int chance, const point_omt_ms& p1, const point_omt_ms& p2,
        float density, bool individual = false, bool friendly = false,
        const std::string& name = "NONE", int mission_id = -1) -> void;
    auto place_gas_pump(const point_omt_ms& p, int charges, const itype_id& fuel_type) -> void;
    auto place_gas_pump(const point_omt_ms& p, int charges) -> void;
    auto place_toilet(const point_omt_ms& p, int charges = 6 * 4) -> void;
    auto place_vending(const point_omt_ms& p, const item_group_id& type, bool reinforced = false)
        -> void;
    auto place_npc(const point_omt_ms& p, const string_id<npc_template>& type, bool force = false)
        -> character_id;
    auto apply_faction_ownership(
        const point_omt_ms& p1, const point_omt_ms& p2, const faction_id& id) -> void;
    auto add_spawn(
        const mtype_id& type, int count, const point_omt_ms& p, bool friendly = false,
        int faction_id = -1, int mission_id = -1, const std::string& name = "NONE") const -> void;
    auto add_spawn(
        const mtype_id& type, int count, const point_omt_ms& p, spawn_disposition disposition,
        int faction_id = -1, int mission_id = -1, const std::string& name = "NONE") const -> void;
    auto add_vehicle(
        const std::variant<vgroup_id, vproto_id>& type_, const point_omt_ms& p, units::angle dir,
    ...
    auto add_vehicle(std::unique_ptr<vehicle> veh, const bool merge_wrecks)
    ...
    auto spawn_natural_artifact(const point_omt_ms& p, artifact_natural_property prop) -> void;
    auto make_rubble(
    ...
    auto bash(
        const point_omt_ms& p, int str, bool destroy = false, bool bash_floor = false,
    ...
    auto create_anomaly(
        const point_omt_ms& p, artifact_natural_property prop, bool create_rubble = true) -> void;
