...
auto omt_tile_max() -> point_omt_ms { return point_omt_ms(SEEX * 2 - 1, SEEY * 2 - 1); }
...
auto has_owned_submaps(const std::vector<std::unique_ptr<submap>>& submaps) -> bool {
    return std::ranges::any_of(submaps, [](const auto& sm) { return sm != nullptr; });
}
...
auto max_items_for_tile(const submap& sm, const point_sm_ms& local) -> units::volume {
    ...
    if (furniture != f_null) { return furniture->max_volume; }
    return sm.get_ter(local)->max_volume;
}

} // namespace

mapgen_constructor::mapgen_constructor(mapbuffer& buffer): buffer_(buffer) {}
...
auto mapgen_constructor::get_abs_omt() const -> tripoint_abs_omt { return abs_offset_; }

auto mapgen_constructor::get_bound_dimension() const -> const dimension_id& {
    return buffer_.get_dimension_id();
}

auto mapgen_constructor::get_buffer() const -> mapbuffer& { return buffer_; }
...
auto mapgen_constructor::load(const tripoint_abs_omt& omt_pos) -> bool {
    ...
    abs_offset_ = omt_pos;
    return std::ranges::all_of(all_submap_offsets(), [&](const point_omt_sm& offset) {
        return buffer_.get_submap(project_combine(abs_offset_, offset), load_or_generate_lookup)
            != nullptr;
    });
}

auto mapgen_constructor::clear() -> void { owned_submaps_.clear(); }
...
auto mapgen_constructor::for_each_submap(const std::function<void(submap&)>& func) const -> void {
    std::ranges::for_each(all_submap_offsets(), [&](const point_omt_sm& offset) {
        ...
        if (sm != nullptr) { func(*sm); }
    });
}

auto mapgen_constructor::clear_spawns() -> void {
    for_each_submap([](submap& sm) { sm.spawns.clear(); });
}

auto mapgen_constructor::clear_traps() -> void {
    for_each_submap([](submap& sm) {
        for (const auto p : submap_tiles()) { sm.set_trap(p, tr_null); }
    });
}

auto mapgen_constructor::reset_scratch_omt(
    const tripoint_abs_omt& omt_pos, const ter_id& terrain, const furn_id& furniture,
    const trap_id& trap) -> void {
    clear();
    abs_offset_ = omt_pos;
...
auto mapgen_constructor::ensure_omt_submaps(const time_point& when) -> bool {
    ...
    std::ranges::for_each(all_submap_offsets(), [&](const point_omt_sm& offset) {
        ...
        if (!index) { return; }
        const auto abs = project_combine(abs_offset_, offset);
        if (buffer_.lookup_submap_in_memory(abs) != nullptr) { return; }
        auto sm = std::make_unique<submap>(abs, buffer_.get_dimension_id());
...
auto mapgen_constructor::commit_generated_submaps() -> bool {
    if (!has_owned_submaps(owned_submaps_)) {
        return std::ranges::all_of(all_submap_offsets(), [&](const point_omt_sm& offset) {
            const auto* const sm = buffer_.lookup_submap_in_memory(
                project_combine(abs_offset_, offset));
    ...
    for (const point_omt_sm& offset : all_submap_offsets()) {
        ...
        if (!buffer_.add_submap(abs, owned_submaps_[*index])) { return false; }
    }
    owned_submaps_.clear();
    return true;
}

auto mapgen_constructor::discard_generated_submaps() -> void { owned_submaps_.clear(); }

auto mapgen_constructor::points_on_zlevel() const -> point_range<point_omt_ms> {
    return {point_omt_ms::zero(), omt_tile_max()};
}

auto mapgen_constructor::points_in_rectangle(const point_omt_ms& from, const point_omt_ms& to) const
    -> point_range<point_omt_ms> {
...
auto mapgen_constructor::points_in_radius(const point_omt_ms& center, const size_t radius) const
    -> point_range<point_omt_ms> {
...
auto mapgen_constructor::tr_at(const point_omt_ms& p) const -> const trap& {
    const auto [sm, local] = tile_at(p);
    if (sm == nullptr) { return tr_null.obj(); }
    if (sm->get_ter(local).obj().trap != tr_null) { return sm->get_ter(local).obj().trap.obj(); }
    return sm->get_trap(local).obj();
}

auto mapgen_constructor::has_furn(const point_omt_ms& p) const -> bool { return furn(p) != f_null; }
...
auto mapgen_constructor::has_flag_ter(const std::string& flag, const point_omt_ms& p) const
    -> bool {
    const auto [sm, local] = tile_at(p);
    return sm != nullptr && sm->get_ter(local).obj().has_flag(flag);
}

auto mapgen_constructor::has_flag_ter(const ter_bitflags flag, const point_omt_ms& p) const
    -> bool {
    const auto [sm, local] = tile_at(p);
    return sm != nullptr && sm->get_ter(local).obj().has_flag(flag);
}

auto mapgen_constructor::has_flag_furn(const std::string& flag, const point_omt_ms& p) const
    -> bool {
    const auto [sm, local] = tile_at(p);
    return sm != nullptr && sm->get_furn(local).obj().has_flag(flag);
}

auto mapgen_constructor::has_flag_furn(const ter_bitflags flag, const point_omt_ms& p) const
    -> bool {
    const auto [sm, local] = tile_at(p);
    return sm != nullptr && sm->get_furn(local).obj().has_flag(flag);
}
...
auto mapgen_constructor::ter_vars(const point_omt_ms& p) const -> data_vars::data_set* {
    ...
    if (sm != nullptr) { return &sm->get_ter_vars(local); }
...
auto mapgen_constructor::furn_vars(const point_omt_ms& p) const -> data_vars::data_set* {
    ...
    if (sm != nullptr) { return &sm->get_furn_vars(local); }

    return nullptr;
}

auto mapgen_constructor::passable(const point_omt_ms& p) const -> bool { return move_cost(p) > 0; }

auto mapgen_constructor::impassable(const point_omt_ms& p) const -> bool { return !passable(p); }

auto mapgen_constructor::move_cost(const point_omt_ms& p, const vehicle* ignored_vehicle) const
    -> int {
    const furn_t& furniture = furn(p).obj();
    const ter_t& terrain = ter(p).obj();
    const optional_vpart_position vp = veh_at(p);
    const vehicle* const veh =
        (!vp || &vp->vehicle() == ignored_vehicle) ? nullptr : &vp->vehicle();
    ...
    if (veh != nullptr) {
        ...
        if (veh_part.obstacle_at_part()) { return 0; }
        if (veh_part.part_with_feature(VPFLAG_AISLE, true)) { return 2; }
        return 8;
    }
...
auto mapgen_constructor::veh_at(const point_omt_ms& p) const -> optional_vpart_position {
    ...
    if (!has_owned_submaps(owned_submaps_)) { return buffer_.veh_at(abs_p, resident_lookup); }
    for (const vehicle* const veh : get_vehicles()) {
        const int part = veh->part_at(abs_p - veh->abs_ms_location());
        if (part >= 0) {
            return optional_vpart_position(
                vpart_position(*const_cast<vehicle*>(veh), static_cast<size_t>(part)));
        }
    }
...
auto mapgen_constructor::get_vehicles() const -> std::vector<vehicle*> {
    ...
    std::ranges::for_each(all_submap_offsets(), [&](const point_omt_sm& offset) {
        ...
        std::ranges::for_each(sm->vehicles, [&](const auto& veh) { result.push_back(veh.get()); });
    });
    return result;
}
...
auto mapgen_constructor::trap_set(const point_omt_ms& p, const trap_id& trap) -> void {
    const auto [sm, local] = tile_at(p);
    if (sm == nullptr || sm->get_ter(local).obj().trap != tr_null) { return; }
    sm->set_trap(local, trap);
}

auto mapgen_constructor::remove_trap(const point_omt_ms& p) -> void { trap_set(p, tr_null); }

auto mapgen_constructor::set_radiation(const point_omt_ms& p, const int value) -> void {
    const auto [sm, local] = tile_at(p);
    if (sm != nullptr) { sm->set_radiation(local, value); }
}

auto mapgen_constructor::adjust_radiation(const point_omt_ms& p, const int delta) -> void {
    const auto [sm, local] = tile_at(p);
    if (sm != nullptr) { sm->set_radiation(local, sm->get_radiation(local) + delta); }
}
...
auto mapgen_constructor::set_temperature(const point_omt_ms& p, const int temperature) -> void {
    ...
    if (sm != nullptr) { sm->set_temperature(temperature); }
}

auto mapgen_constructor::add_field(
    const point_omt_ms& p, const field_type_id& type_id, const int intensity,
    const time_duration& age, const bool /*hit_player*/) -> bool {
    const auto [sm, local] = tile_at(p);
    if (sm == nullptr || !type_id) { return false; }
    const auto field_intensity = std::min(intensity, type_id->get_max_intensity());
    if (field_intensity <= 0) { return false; }
    const auto added = sm->get_field(local).add_field(type_id, field_intensity, age);
    sm->is_uniform = false;
    if (added) {
        sm->field_count++;
...
auto mapgen_constructor::remove_field(const point_omt_ms& p, const field_type_id& field_to_remove)
    -> void {
    const auto [sm, local] = tile_at(p);
    if (sm != nullptr && sm->get_field(local).remove_field(field_to_remove)) { --sm->field_count; }
}
...
auto mapgen_constructor::add_splatter_trail(
    const field_type_id& type, const point_omt_ms& from, const point_omt_ms& to) -> void {
    std::ranges::for_each(line_to(from, to), [&](const point_omt_ms& p) {
        add_field(p, type, 1, 0_turns, false);
    });
}

auto mapgen_constructor::add_computer(
    const point_omt_ms& p, const std::string& name, const int security) -> computer* {
    ter_set(p, t_console);
    const auto [sm, local] = tile_at(p);
...
auto mapgen_constructor::set_signage(const point_omt_ms& p, const std::string& message) const
    -> void {
    const auto [sm, local] = tile_at(p);
    if (sm != nullptr) { sm->set_signage(local, message); }
}

auto mapgen_constructor::set_graffiti(const point_omt_ms& p, const std::string& contents) -> void {
    const auto [sm, local] = tile_at(p);
    if (sm != nullptr) { sm->set_graffiti(local, contents); }
}

auto mapgen_constructor::delete_graffiti(const point_omt_ms& p) -> void {
    const auto [sm, local] = tile_at(p);
    if (sm != nullptr) { sm->delete_graffiti(local); }
}

auto mapgen_constructor::i_at(const point_omt_ms& p) -> map_stack {
    const auto [sm, local] = tile_at(p);
    auto* items = sm != nullptr ? &sm->get_items(local) : nullptr;
    if (items == nullptr) { items = &null_items(); }
    return map_stack({
        .stack = items,
...
auto mapgen_constructor::add_item_or_charges(
    const point_omt_ms& pos, detached_ptr<item>&& obj, const bool overflow) -> detached_ptr<item> {
    if (!obj || obj->is_null() || obj->has_flag(flag_NO_DROP)) { return std::move(obj); }
    const auto try_place = [&](const point_omt_ms& p, const bool reject_noitem) -> bool {
        ...
        if (reject_noitem && (has_flag("NOITEM", p) || has_flag("SEALED", p))) { return false; }
        auto& items = sm->get_items(local);
        if (obj->count_by_charges()) {
            for (auto& existing : items) {
                if (existing->merge_charges(std::move(obj))) { return true; }
            }
        }
    ...
    if (try_place(pos, false)) { return std::move(obj); }
    if (overflow) {
        for (const auto& candidate : points_in_radius(pos, 2)) {
            if (candidate != pos && try_place(candidate, true)) { return std::move(obj); }
        }
    }
    return std::move(obj);
}

auto mapgen_constructor::spawn_an_item(
    const point_omt_ms& p, detached_ptr<item>&& new_item, const int charges, const int damlevel)
    -> detached_ptr<item> {
    if (!new_item) { return detached_ptr<item>(); }
    if (one_in(3) && new_item->has_flag(flag_VARSIZE)) { new_item->set_flag(flag_FIT); }
    if (charges && new_item->charges > 0) { new_item->charges = charges; }
...
auto mapgen_constructor::spawn_items(
    const point_omt_ms& p, std::vector<detached_ptr<item>> new_items)
...
auto mapgen_constructor::spawn_item(
    const point_omt_ms& p, const itype_id& type_id, const unsigned quantity, const int charges,
...
auto mapgen_constructor::put_items_from_loc(
    const item_group_id& loc, const point_omt_ms& p, const time_point& turn) -> std::vector<item*> {
    auto items = item_group::items_from(loc, turn);
    auto ret = std::vector<item*>{};
    ret.reserve(items.size());
    std::ranges::transform(items, std::back_inserter(ret), [](const detached_ptr<item>& it) {
        return &*it;
    });
...
auto mapgen_constructor::place_items(
    const item_group_id& loc, const int chance, const point_omt_ms& p1, const point_omt_ms& p2,
    ...
    std::ranges::for_each(std::views::iota(0, spawn_count), [&](const auto) {
        ...
        if (is_type) {
            ...
            if (placed) { res.push_back(&*placed); }
            return;
        }
    ...
    std::ranges::for_each(res, [&](item* const e) {
        if (e == nullptr || (!e->is_tool() && !e->is_gun() && !e->is_magazine())) { return; }
...
auto mapgen_constructor::draw_line_ter(
    const ter_id& type, const point_omt_ms& p1, const point_omt_ms& p2) -> void {
    draw_line([this, type](const point& p) { ter_set(point_omt_ms(p), type); }, p1.raw(), p2.raw());
}

auto mapgen_constructor::draw_line_furn(
    const furn_id& type, const point_omt_ms& p1, const point_omt_ms& p2) -> void {
    draw_line([this, type](const point& p) { furn_set(point_omt_ms(p), type); }, p1.raw(), p2.raw());
}
...
auto mapgen_constructor::draw_square_ter(
    const ter_id& type, const point_omt_ms& p1, const point_omt_ms& p2) -> void {
    draw_square([this, type](const point& p) { ter_set(point_omt_ms(p), type); }, p1.raw(),
                p2.raw());
}

auto mapgen_constructor::draw_square_ter(
    ter_id (*f)(), const point_omt_ms& p1, const point_omt_ms& p2) -> void {
    draw_square([this, f](const point& p) { ter_set(point_omt_ms(p), f()); }, p1.raw(), p2.raw());
}

auto mapgen_constructor::draw_square_ter(
    const weighted_int_list<ter_id>& f, const point_omt_ms& p1, const point_omt_ms& p2) -> void {
    draw_square(
...
auto mapgen_constructor::draw_square_furn(
    const furn_id& type, const point_omt_ms& p1, const point_omt_ms& p2) -> void {
    draw_square([this, type](const point& p) { furn_set(point_omt_ms(p), type); }, p1.raw(),
                p2.raw());
}

auto mapgen_constructor::draw_rough_circle_ter(
    const ter_id& type, const point_omt_ms& p, const int rad) -> void {
    draw_rough_circle(
        [this, type](const point& loc) { ter_set(point_omt_ms(loc), type); }, p.raw(), rad);
}

auto mapgen_constructor::draw_rough_circle_furn(
    const furn_id& type, const point_omt_ms& p, const int rad) -> void {
    draw_rough_circle(
        [this, type](const point& loc) { furn_set(point_omt_ms(loc), type); }, p.raw(), rad);
}

auto mapgen_constructor::draw_circle_ter(const ter_id& type, const rl_vec2d& p, const double rad)
    -> void {
    draw_circle([this, type](const point& loc) { ter_set(point_omt_ms(loc), type); }, p, rad);
}

auto mapgen_constructor::draw_circle_ter(const ter_id& type, const point_omt_ms& p, const int rad)
    -> void {
    draw_circle([this, type](const point& loc) { ter_set(point_omt_ms(loc), type); }, p.raw(), rad);
}

auto mapgen_constructor::draw_circle_furn(const furn_id& type, const point_omt_ms& p, const int rad)
    -> void {
    draw_circle([this, type](const point& loc) { furn_set(point_omt_ms(loc), type); }, p.raw(), rad);
}
...
auto mapgen_constructor::rotate(int turns, const bool setpos_safe) -> void {
    ...
    std::ranges::for_each(npcs, [&](const shared_ptr_fast<npc>& i) {
        ...
        if (proj.quotient_tripoint != abs_omt) { return; }
        const auto new_pos =
    ...
    const auto swap_submaps = [&](const point_omt_sm& p1, const point_omt_sm& p2) {
        ...
        if (sm1 != nullptr && sm2 != nullptr) { submap::swap(*sm1, *sm2); }
    };
...
auto mapgen_constructor::place_spawns(
    const mongroup_id& group, const int chance, const point_omt_ms& p1, const point_omt_ms& p2,
    ...
    const auto spawn_density =
        MonsterGroupManager::is_animal(group)
            ? get_option<float>("SPAWN_ANIMAL_DENSITY")
            : get_option<float>("SPAWN_DENSITY");
    auto num = roll_remainder(
        individual ? 1.0f : density * spawn_density * rng_float(10.0f, 50.0f));
    while (num > 0) {
        auto tries = 10;
        auto p = point_omt_ms::zero();
        do {
        ...
        add_spawn(spawn_details.name, spawn_details.pack_size, p, friendly, -1, mission_id, name);
    }
}

auto mapgen_constructor::place_gas_pump(
    const point_omt_ms& p, const int charges, const itype_id& fuel_type) -> void {
...
auto mapgen_constructor::place_vending(
    const point_omt_ms& p, const item_group_id& type, const bool reinforced) -> void {
    if (reinforced) {
    ...
    if (one_in(2)) {
        ...
        std::ranges::for_each(points_in_radius(p, 1), [&](const point_omt_ms& loc) {
            if (one_in(4)) { spawn_item(loc, itype_id("glass_shard"), rng(1, 2)); }
        });
        return;
    }
...
auto mapgen_constructor::place_npc(
    const point_omt_ms& p, const string_id<npc_template>& type, const bool force) -> character_id {
...
auto mapgen_constructor::apply_faction_ownership(
    const point_omt_ms& p1, const point_omt_ms& p2, const faction_id& id) -> void {
    std::ranges::for_each(points_in_rectangle(p1, p2), [&](const point_omt_ms& p) {
        auto items = i_at(p);
        std::ranges::for_each(items, [&](item* const elem) { elem->set_owner(id); });
        const auto vp = veh_at(p);
        if (vp && !vp->vehicle().has_owner()) { vp->vehicle().set_owner(id); }
    });
}

auto mapgen_constructor::add_spawn(
    const mtype_id& type, const int count, const point_omt_ms& p, const bool friendly,
...
auto mapgen_constructor::add_spawn(
    const mtype_id& type, const int count, const point_omt_ms& p,
    const spawn_disposition disposition, const int faction_id, const int mission_id,
...
auto mapgen_constructor::add_vehicle(
    const std::variant<vgroup_id, vproto_id>& type_, const point_omt_ms& p, const units::angle dir,
    const int veh_fuel, const int veh_status, const bool /*merge_wrecks*/,
    std::optional<bool> locked, std::optional<bool> has_keys, bool place_beyond_bounds)
    -> vehicle* {
    const auto type = std::visit(
        [](const auto& v) -> vproto_id {
    ...
    auto* const place_on_submap = submap_at_grid(
        project_remain<coords::omt>(veh->abs_sm_pos).remainder);
    if (place_on_submap == nullptr) { return nullptr; }
...
auto mapgen_constructor::add_vehicle(std::unique_ptr<vehicle> veh, const bool merge_wrecks)
    ...
    for (const int pt : frame_indices) {
        ...
        if (has_flag_ter(TFLAG_DEEP_WATER, omt_pos) && !can_float) { return nullptr; }
        // Don't spawn shopping carts on top of another vehicle or other obstacle.
        if (veh->type == vproto_id("shopping_cart")) {
            if (veh_at(omt_pos) || impassable(omt_pos)) { return nullptr; }
        }
        ...
        if (other_veh != nullptr && other_veh->type != vproto_id("shopping_cart")) {
            ...
            if (impassable(omt_pos)) { return nullptr; }
...
auto mapgen_constructor::destroy_vehicle(vehicle* veh) -> void {
    ...
    if (iter != sm->vehicles.end()) { sm->vehicles.erase(iter); }
}
...
auto mapgen_constructor::add_corpse(const point_omt_ms& p) -> void {
    ...
    if (revives && body) { body->set_flag(flag_REVIVE_SPECIAL); }
...
auto mapgen_constructor::spawn_natural_artifact(
    const point_omt_ms& p, const artifact_natural_property prop) -> void {
    add_item_or_charges(p, item::spawn(new_natural_artifact(prop), calendar::start_of_cataclysm));
}

auto mapgen_constructor::make_rubble(
    const point_omt_ms& p, const furn_id& rubble_type, const ter_id& floor_type,
    ...
    if (has_furn(p) && is_bashable(p)) { furn_set(p, f_null); }
    if (impassable(p) && is_bashable(p)) { destroy(p); }
    if (impassable(p)) { ter_set(p, floor_type); }

    furn_set(p, rubble_type);
}

auto mapgen_constructor::make_rubble(const point_omt_ms& p, const furn_id& rubble_type) -> void {
    make_rubble(p, rubble_type, t_dirt, false);
}

auto mapgen_constructor::bash(
    const point_omt_ms& p, const int str, const bool destroy, const bool /*bash_floor*/,
    ...
    if (!bashed_sealed) { bash_items(p); }

    const vehicle* veh = veh_pointer_or_null(veh_at(p));
    if (veh != nullptr) {
        bash_vehicle(p, str);
    } else if (!bashed_sealed) {
        // If we still didn't bash anything solid (a vehicle) or a tile with SEALED flag, bash
        // ter/furn
        bash_ter_furn(p, destroy);
    }
...
void mapgen_constructor::bash_field(const point_omt_ms& p) { remove_field(p, fd_web); }
...
auto mapgen_constructor::create_anomaly(
    const point_omt_ms& cp, const artifact_natural_property prop, const bool create_rubble)
    -> void {
    if (create_rubble) {
    ...
    const auto around = [&](const int radius) {
        return points_in_rectangle(
            cp + point_rel_ms(-radius, -radius), cp + point_rel_ms(radius, radius));
    };
    ...
    switch (prop) {
        ...
        case ARTPROP_MOVING:
            std::ranges::for_each(around(5), [&](const point_omt_ms& p) {
                if (furn(p) == f_rubble) { add_field(p, fd_push_items, 1); }
            });
            break;
        ...
        case ARTPROP_GLITTERING:
            std::ranges::for_each(around(5), [&](const point_omt_ms& p) {
                if (furn(p) == f_rubble && one_in(2)) { trap_set(p, tr_glow); }
            });
            break;
        ...
        case ARTPROP_RATTLING:
            std::ranges::for_each(around(5), [&](const point_omt_ms& p) {
                if (furn(p) == f_rubble && one_in(2)) { trap_set(p, tr_hum); }
            });
            break;
        ...
        case ARTPROP_ENGRAVED:
            std::ranges::for_each(around(5), [&](const point_omt_ms& p) {
                if (furn(p) == f_rubble && one_in(3)) { trap_set(p, tr_shadow); }
            });
            break;
        ...
        case ARTPROP_DEAD:
            std::ranges::for_each(around(5), [&](const point_omt_ms& p) {
                if (furn(p) == f_rubble) { trap_set(p, tr_drain); }
            });
            break;

        case ARTPROP_ITCHY:
            std::ranges::for_each(around(5), [&](const point_omt_ms& p) {
                if (furn(p) == f_rubble) { set_radiation(p, rng(0, 10)); }
            });
            break;
        ...
        case ARTPROP_WARM:
            std::ranges::for_each(around(5), [&](const point_omt_ms& p) {
                if (furn(p) == f_rubble) { add_field(p, fd_fire_vent, 1 + (rl_dist(cp, p) % 3)); }
            });
            break;

        case ARTPROP_SCALED:
            std::ranges::for_each(around(5), [&](const point_omt_ms& p) {
                if (furn(p) == f_rubble) { trap_set(p, tr_snake); }
            });
            break;
...
auto random_point(
    const point_range<point_omt_ms>& range,
    ...
    -> std::optional<point_omt_ms> // *NOPAD*
{
    for (const auto unused : std::views::iota(0, 10)) {
        (void)unused;
        const auto p = point_omt_ms(
            rng(range.min().x(), range.max().x()), rng(range.min().y(), range.max().y()));
        if (predicate(p)) { return p; }
    }
    auto suitable = std::vector<point_omt_ms>{};
    std::ranges::copy_if(range, std::back_inserter(suitable), predicate);
    if (suitable.empty()) { return std::nullopt; }
    return random_entry(suitable);
}

auto random_point(
    const mapgen_constructor& m,
    const std::function<bool(const point_omt_ms&)>& predicate)
    -> std::optional<point_omt_ms> // *NOPAD*
{
