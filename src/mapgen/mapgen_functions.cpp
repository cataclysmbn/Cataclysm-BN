...
building_gen_pointer get_mapgen_cfunction(const std::string& ident) {
    static const std::map<std::string, building_gen_pointer> pointers = {{
        ...
        // New rock function - should be default, but isn't yet for compatibility reasons (old
        // overmaps)
...
void mapgen_crater(mapgendata& dat) {
    ...
    for (int i = 0; i < 4; i++) {
        if (dat.t_nesw[i] != "crater") { dat.set_dir(i, 6); }
    }
    ...
    m->place_items(item_group_id("wreckage"), 83, point_omt_ms::zero(),
...
/*
 * Default above ground non forested 'blank' area; typically a grassy field with a scattering of
 * shrubs, but changes according to dat->region
 */
void mapgen_field(mapgendata& dat) {
    ...
    for (int i = 0; i < SEEX * 2; i++) {
        for (int j = 0; j < SEEY * 2; j++) {
            ...
            if (mpercent_bush > rng(0, 1000000)) {
                if (boosted_vegetation
                    && dat.region.field_coverage.boosted_other_mpercent > rng(0, 1000000)) {
    ...
    m->place_items(item_group_id("field"), 60, point_omt_ms::zero(),
                   point_omt_ms(SEEX * 2 - 1, SEEY * 2 - 1), true, dat.when());
}
...
void mapgen_hive(mapgendata& dat) {
    ...
    const bool is_center =
        dat.t_nesw[0] == "hive" && dat.t_nesw[1] == "hive" && dat.t_nesw[2] == "hive"
        && dat.t_nesw[3] == "hive";
    for (int j = 5; j < SEEY * 2 - 5; j += 6) {
        for (int i = (j == 5 || j == 17 ? 3 : 6); i < SEEX * 2 - 5; i += 6) {
            if (!one_in(8)) {
                ...
                if (skip1 == 1 || skip2 == 1) { m->ter_set(point_omt_ms(i, j - 4), t_floor_wax); }
                if (skip1 == 2 || skip2 == 2) {
                    m->ter_set(point_omt_ms(i + 1, j - 4), t_floor_wax);
                }
                if (skip1 == 3 || skip2 == 3) {
                ...
                if (skip1 == 12 || skip2 == 12) { m->ter_set(point_omt_ms(i - 3, j), t_floor_wax); }
                ...
                if (skip1 == 15 || skip2 == 15) { m->ter_set(point_omt_ms(i - 3, j), t_floor_wax); }
                ...
                if (skip1 == 22 || skip2 == 22) { m->ter_set(point_omt_ms(i, j + 4), t_floor_wax); }
                ...
                if (is_center) {
                    m->place_items(item_group_id("hive_center"), 90, point_omt_ms(i - 2, j - 2),
                                   point_omt_ms(i + 2, j + 2), false, dat.when());
                } else {
                    m->place_items(item_group_id("hive"), 80, point_omt_ms(i - 2, j - 2),
                                   point_omt_ms(i + 2, j + 2), false, dat.when());
                }
            }
        }
...
void mapgen_road(mapgendata& dat) {
    ...
    for (int dir = 0; dir < 4; dir++) {
        if (!roads_nesw[dir] || dat.t_nesw[dir]->get_type_id().str() != "road") { continue; }
        ...
        if (n_num_dirs == 2 && n_roads_nesw[(dir + 2) % 4]) {
            ...
            if (n_roads_nesw[(dir - 1 + 4) % 4]) { curvedir_nesw[dir]--; }
            // our road curves clockwise
            if (n_roads_nesw[(dir + 1) % 4]) { curvedir_nesw[dir]++; }
        }
    }
    ...
    switch (num_dirs) {
        case 4:
            ...
            plaza_dir =
                ...
                : compare_neswx(fourways_neswx, {0, 1, 1, 1, 0, 1, 1, 0})
                    ? 0
                    : -1;
    ...
    if (diag) {
        ...
        if (dead_end_extension > 0 && sidewalks_neswx[2]) {
            square(m, t_sidewalk, point_omt_ms(0, SEEY + dead_end_extension),
                   point_omt_ms(SEEX * 2 - 1, SEEY + dead_end_extension + 4));
        }
        ...
        if (cul_de_sac) { circle(m, t_pavement, double(SEEX) - 0.5, double(SEEY) - 0.5, 11.0); }
        ...
        if (plaza_dir > -1) {
            if (plaza_dir == 8) {
                ...
                // plaza side
                square(m, t_pavement, point_omt_ms(0, SEEY - 10),
                       point_omt_ms(SEEX * 2 - 1, SEEY - 1));
                square(m, t_sidewalk, point_omt_ms(0, SEEY - 2),
                       point_omt_ms(SEEX * 2 - 1, SEEY * 2 - 1));
                if (one_in(3)) {
                ...
                if (one_in(3)) { circle(m, t_water_sh, point_omt_ms(4, SEEY * 2 - 5), 3); }
            }
        }
    }
    ...
    // spawn some vehicles
    if (plaza_dir != 8) {
        vspawn_id(neighbor_sidewalks ? "default_city" : "default_country")
            .obj()
            .apply(*m,
    ...
    if (neighbor_sidewalks) {
        m->place_spawns(GROUP_ZOMBIE, 2, point_omt_ms::zero(),
                        point_omt_ms(SEEX * 2 - 1, SEEX * 2 - 1), dat.monster_density());
        // 1 per 10 overmaps
    ...
    m->place_items(item_group_id(plaza ? "trash" : "road"), 5, point_omt_ms::zero(),
                   point_omt_ms(SEEX * 2 - 1, SEEX * 2 - 1), plaza, dat.when());
...
void mapgen_subway(mapgendata& dat) {
    ...
    for (int dir = 0; dir < 4; dir++) {
        ...
        if (n_num_dirs == 2 && n_subway_nesw[(dir + 2) % 4]) {
            ...
            if (n_subway_nesw[(dir - 1 + 4) % 4]) { curvedir_nesw[dir]--; }
            // our subway curves clockwise
            if (n_subway_nesw[(dir + 1) % 4]) { curvedir_nesw[dir]++; }
        }
    }
    ...
    switch (num_dirs) {
        case 4:
            ...
            mapf::formatted_set_simple(
                ...
                "..^DX^^DX^....^XD^^XD^..\n"
                "...^X^^^X^....^X^^^X^...",
                mapf::ter_bind(". # ^ D X", t_rock_floor, t_rock, t_railroad_rubble,
                               t_railroad_track_d, t_railroad_track),
                mapf::furn_bind(". # ^ D X", f_null, f_null, f_null, f_null, f_null));
            break;
        case 3:
            // tee
            mapf::formatted_set_simple(
                ...
                "........................",
                mapf::ter_bind(
                    ". # ^ | X x D", t_rock_floor, t_rock, t_railroad_rubble, t_railroad_tie,
                    t_railroad_track, t_railroad_track_on_tie, t_railroad_track_d),
                mapf::furn_bind(
                    ". # ^ | X x D", f_null, f_null, f_null, f_null, f_null, f_null, f_null));
            break;
        case 2:
            // straight or diagonal
            if (diag) { // diagonal subway get drawn differently from all other types
                mapf::formatted_set_simple(
                    ...
                    "###################.....",
                    mapf::ter_bind(
                        ". # ^ D", t_rock_floor, t_rock, t_railroad_rubble, t_railroad_track_d),
                    mapf::furn_bind(". # ^ D", f_null, f_null, f_null, f_null));
                ...
                mapf::formatted_set_simple(
                    ...
                    "...-x---x-....-x---x-...\n"
                    "...^X^^^X^....^X^^^X^...",
                    mapf::ter_bind(". # ^ - X x", t_rock_floor, t_rock, t_railroad_rubble,
        ...
        case 1:
            ...
            mapf::formatted_set_simple(
                ...
                "########################",
                mapf::ter_bind(
                    ". # S ^ - D X x", t_rock_floor, t_rock, t_buffer_stop, t_railroad_rubble,
                    t_railroad_tie, t_railroad_track_d, t_railroad_track, t_railroad_track_on_tie),
                mapf::furn_bind(". # S ^ - D X x", f_null, f_null, f_null, f_null, f_null, f_null,
                                f_null, f_null));
    ...
    for (int i = 0; i < SEEX * 2; i++) {
        for (int j = 0; j < SEEY * 2; j++) {
            if (one_in(12) && m->ter(point_omt_ms(i, j)) == t_rock_floor) {
                m->ter_set(point_omt_ms(i, j), t_moss_underground);
                // Some of that moss has mushrooms too.
                if (one_in(15)) { m->furn_set(point_omt_ms(i, j), f_cave_mushrooms); }
            }
        }
    }
    ...
    for (int i = 0; i < SEEX * 2; i++) {
        for (int j = 0; j < SEEY * 2; j++) {
            ...
            if (j >= SEEY - 2 && j <= SEEY + 1) {
                if (i <= SEEX - 2) {
                    if (sewer_nesw[3]) { fill = false; }
                } else if (i >= SEEX + 1) {
                    if (sewer_nesw[1]) { fill = false; }
                } else {
                ...
                if (j <= SEEY - 2) {
                    if (sewer_nesw[0]) { fill = false; }
                } else {
                    if (sewer_nesw[2]) { fill = false; }
                }
            }
...
void mapgen_highway(mapgendata& dat) {
    ...
    if (dat.terrain_type() == "hiway_ew") { m->rotate(1); }
    m->place_items(item_group_id("road"), 8, point_omt_ms::zero(),
                   point_omt_ms(SEEX * 2 - 1, SEEX * 2 - 1), false, dat.when());
}
    ...
    switch (num_dirs) {
        case 4:
            ...
            mapf::formatted_set_simple(
                ...
                "DD^^DD^..........^DD^^DD\n"
                ".DD^^DD^........^DD^^DD.",
                mapf::ter_bind(". ^ D", t_dirt, t_railroad_rubble, t_railroad_track_d),
                mapf::furn_bind(". ^ D", f_null, f_null, f_null));
            break;
        case 3:
            // tee
            mapf::formatted_set_simple(
                ...
                "........................",
                mapf::ter_bind(
                    ". ^ | X x / D", t_dirt, t_railroad_rubble, t_railroad_tie, t_railroad_track,
                    t_railroad_track_on_tie, t_railroad_tie_d, t_railroad_track_d),
                mapf::furn_bind(
                    ". ^ | X x / D", f_null, f_null, f_null, f_null, f_null, f_null, f_null));
            break;
        case 2:
            ...
            if (diag) {
                ...
                mapf::formatted_set_simple(
                    ...
                    ".-x---x-........-x---x-.\n"
                    ".^X^^^X^........^X^^^X^.",
                    mapf::ter_bind(". ^ - X x", t_dirt, t_railroad_rubble, t_railroad_tie,
                                   t_railroad_track, t_railroad_track_on_tie),
                    mapf::furn_bind(". ^ - X x", f_null, f_null, f_null, f_null, f_null));
            }
            break;
        case 1:
            // dead end
            mapf::formatted_set_simple(
                ...
                "........................\n"
                "........................",
                mapf::ter_bind(". ^ S - X x", t_dirt, t_railroad_rubble, t_buffer_stop,
                               t_railroad_tie, t_railroad_track, t_railroad_track_on_tie),
                mapf::furn_bind(". ^ S - X x", f_null, f_null, f_null, f_null, f_null, f_null));
            break;
    }
    // finally, unrotate the map
    m->rotate(rot);
...
void mapgen_railroad_bridge(mapgendata& dat) {
    ...
    mapf::formatted_set_simple(
        ...
        "r-x---x-________-x---x-r\n"
        "r^X^^^X^________^X^^^X^r",
        mapf::ter_bind(". _ r ^ - X x", t_dirt, t_concrete, t_railing, t_railroad_rubble,
                       t_railroad_tie, t_railroad_track, t_railroad_track_on_tie),
        mapf::furn_bind(". _ r ^ - X x", f_null, f_null, f_null, f_null, f_null, f_null, f_null));
    m->rotate(static_cast<int>(dat.terrain_type()->get_dir()));
}

void mapgen_river_center(mapgendata& dat) { fill_background(&dat.m, t_water_moving_dp); }
...
void mapgen_river_curved_not(mapgendata& dat) {
    ...
    if (dat.terrain_type() == "river_c_not_se") { m->rotate(1); }
    if (dat.terrain_type() == "river_c_not_sw") { m->rotate(2); }
    if (dat.terrain_type() == "river_c_not_nw") { m->rotate(3); }
}
...
void mapgen_river_straight(mapgendata& dat) {
    ...
    for (int x = 0; x < SEEX * 2; x++) {
        ...
        if (one_in(20)) { m->ter_set(point_omt_ms(x, ++ground_edge), clay_or_sand()); }
        line(m, t_water_moving_sh, point_omt_ms(x, ++ground_edge), point_omt_ms(x, shallow_edge));
    }

    if (dat.terrain_type() == "river_east") { m->rotate(1); }
    if (dat.terrain_type() == "river_south") { m->rotate(2); }
    if (dat.terrain_type() == "river_west") { m->rotate(3); }
}
...
void mapgen_river_curved(mapgendata& dat) {
    ...
    for (int y = 0; y < SEEY * 2; y++) {
        ...
        if (one_in(20)) { m->ter_set(point_omt_ms(--ground_edge, y), clay_or_sand()); }
        line(m, t_water_moving_sh, point_omt_ms(shallow_edge, y), point_omt_ms(--ground_edge, y));
    }

    if (dat.terrain_type() == "river_se") { m->rotate(1); }
    if (dat.terrain_type() == "river_sw") { m->rotate(2); }
    if (dat.terrain_type() == "river_nw") { m->rotate(3); }
}
...
void mapgen_river_shore(mapgendata& dat) {
    ...
    for (int dir = 0; dir < 8; dir++) {
        ground_neswx[dir] = !(
            dat.t_nesw[dir]->is_river() || dat.t_nesw[dir]->is_lake()
    ...
    for (int dir = 0; dir < 4; dir++) {
        if (ground_neswx[dir]) {
            ...
            for (int x = 0; x < SEEX * 2; x++) {
                ...
                line(m, grass_or_dirt(), point_omt_ms(x, 0), point_omt_ms(x, ground_edge));
                if (one_in(20)) { m->ter_set(point_omt_ms(x, ++ground_edge), clay_or_sand()); }
                line(m, t_water_moving_sh, point_omt_ms(x, ++ground_edge),
                     point_omt_ms(x, shallow_edge));
            }
            m->rotate(dir);
        }
    }
...
void mapgen_parking_lot(mapgendata& dat) {
    ...
    m->place_items(item_group_id("road"), 8, point_omt_ms::zero(),
                   point_omt_ms(SEEX * 2 - 1, SEEY * 2 - 1), false, dat.when());
    for (int i = 1; i < 4; i++) {
        const std::string& id = dat.t_nesw[i].id().str();
        if (id.size() > 5 && id.starts_with("road_")) { m->rotate(i); }
    }
}
...
void mapgen_cavern(mapgendata& dat) {
    ...
    for (int n = 0; n < rn; n++) {
        ...
        for (int i = p.x() - 1; i <= p.x() + 1; i++) {
            for (int j = p.y() - 1; j <= p.y() + 1; j++) { m->ter_set(point_omt_ms(i, j), t_rock); }
        }
    }

    if (connects_to(dat.north(), 2)) {
        for (int i = SEEX - 2; i <= SEEX + 3; i++) {
            for (int j = 0; j <= SEEY; j++) { m->ter_set(point_omt_ms(i, j), t_rock_floor); }
        }
    }
    ...
    m->place_items(item_group_id("cavern"), 60, point_omt_ms::zero(),
                   point_omt_ms(SEEX * 2 - 1, SEEY * 2 - 1), false, dat.when());
    if (one_in(6)) { // Miner remains
        ...
        if (!one_in(3)) { m->spawn_item(p2, itype_jackhammer); }
        if (one_in(3)) { m->spawn_item(p2, itype_mask_dust); }
        if (one_in(2)) { m->spawn_item(p2, itype_hat_hard); }
        while (!one_in(3)) {
            for (int i = 0; i < 3; ++i) {
...
void mapgen_rock(mapgendata& dat) { fill_background(&dat.m, t_rock); }

void mapgen_open_air(mapgendata& dat) { fill_background(&dat.m, t_open_air); }

void mapgen_pd_border(mapgendata& dat) { fill_background(&dat.m, ter_id("t_pd_border")); }
...
void mapgen_rift(mapgendata& dat) {
    ...
    for (int i = 0; i < SEEX * 2; i++) {
        for (int j = 0; j < SEEY * 2; j++) {
            if ((dat.n_fac < 0 && j < dat.n_fac * -1)
                || (dat.s_fac < 0 && j >= SEEY * 2 - dat.s_fac)
                || (dat.w_fac < 0 && i < dat.w_fac * -1)
                || (dat.e_fac < 0 && i >= SEEX * 2 - dat.e_fac)) {
                m->ter_set(point_omt_ms(i, j), t_rock_floor);
            } else if (j < dat.n_fac || j >= SEEY * 2 - dat.s_fac || i < dat.w_fac
                       || i >= SEEX * 2 - dat.e_fac) {
                m->ter_set(point_omt_ms(i, j), t_rock);
            } else {
...
void mapgen_hellmouth(mapgendata& dat) {
    ...
    // .. seriously, though...
    for (int i = 0; i < 4; i++) {
        if (dat.t_nesw[i] != "rift" && dat.t_nesw[i] != "hellmouth") { dat.dir(i) = 6; }
    }

    for (int i = 0; i < SEEX * 2; i++) {
        for (int j = 0; j < SEEY * 2; j++) {
            if (j < dat.n_fac || j >= SEEY * 2 - dat.s_fac || i < dat.w_fac
                || i >= SEEX * 2 - dat.e_fac
...
void mapgen_forest(mapgendata& dat) {
    ...
    const auto get_feature_for_neighbor =
        [&dat, &no_ter_furn](
            const std::map<oter_id, ter_furn_id>& biome_features, const om_direction::type dir) {
    ...
    const auto get_max_sparseness_adjacency_factor = [&dat]() {
        if (dat.region.forest_composition.biomes.empty()) { return 0; }
    ...
    const auto get_blended_feature =
        [&no_ter_furn, &max_factor, &factor, &get_feature_for_neighbor,
         &dat](const point_omt_ms& p) {
            // Pick one random feature from each biome according to the biome defs and save it into
            // a lookup. We'll blend these features together below based on the current and adjacent
            // terrains.
            ...
            const ter_furn_id east_feature =
                get_feature_for_neighbor(biome_features, om_direction::type::east);
            const ter_furn_id west_feature =
                get_feature_for_neighbor(biome_features, om_direction::type::west);
            const ter_furn_id north_feature =
                get_feature_for_neighbor(biome_features, om_direction::type::north);
            const ter_furn_id south_feature =
                get_feature_for_neighbor(biome_features, om_direction::type::south);
            ...
            if (p.x() < margin_x) {
                ...
                if (p.y() < margin_y) {
                    feature_pool
                        .add(no_ter_furn, 3 * max_factor - (dat.n_fac + dat.w_fac + factor * 2));
                ...
                // SW corner - blend S, W, and self
                    ...
                    feature_pool
                        .add(no_ter_furn, 3 * max_factor - (dat.s_fac + dat.w_fac + factor * 2));
            ...
            // E sections
                ...
                if (p.y() < margin_y) {
                    feature_pool
                        .add(no_ter_furn, 3 * max_factor - (dat.n_fac + dat.e_fac + factor * 2));
                    feature_pool.add(self_feature, factor);
                ...
                // SE corner - blend S, E, and self
                    ...
                    feature_pool
                        .add(no_ter_furn, 3 * max_factor - (dat.s_fac + dat.e_fac + factor * 2));
                    feature_pool.add(self_feature, factor);
                    feature_pool.add(east_feature, east_weight);
                    feature_pool.add(south_feature, south_weight);
                }
    ...
    const auto set_terrain_dependent_furniture =
        [&current_biome_def, &m](const ter_id& tid, point_omt_ms p) {
            const auto terrain_dependent_furniture_it =
                current_biome_def.terrain_dependent_furniture.find(tid);
            if (terrain_dependent_furniture_it
                == current_biome_def.terrain_dependent_furniture.end()) {
                // No terrain dependent furnitures for this terrain, so bail.
                return;
            }

            const forest_biome_terrain_dependent_furniture tdf =
                terrain_dependent_furniture_it->second;
    ...
    for (int i = 0; i < current_biome_def.item_spawn_iterations; i++) {
        m->place_items(
            current_biome_def.item_group, current_biome_def.item_group_chance, point_omt_ms::zero(),
            point_omt_ms(SEEX * 2 - 1, SEEY * 2 - 1), true, dat.when());
    }
}
...
void mapgen_forest_trail_curved(mapgendata& dat) {
    ...
    if (dat.terrain_type() == "forest_trail_es") { m->rotate(1); }
    if (dat.terrain_type() == "forest_trail_sw") { m->rotate(2); }
    if (dat.terrain_type() == "forest_trail_wn") { m->rotate(3); }
...
void mapgen_forest_trail_tee(mapgendata& dat) {
    ...
    if (dat.terrain_type() == "forest_trail_esw") { m->rotate(1); }
    if (dat.terrain_type() == "forest_trail_nsw") { m->rotate(2); }
    if (dat.terrain_type() == "forest_trail_new") { m->rotate(3); }

    m->place_items(item_group_id("forest_trail"), 75, center + point(-2, -2), center + point(2, 2),
                   true, dat.when());
}
...
void mapgen_lake_shore(mapgendata& dat) {
    ...
    if (!dat.region.overmap_lake.shore_extendable_overmap_terrain.empty()) {
        ...
        for (oter_id& adjacent : dat.t_nesw) {
            ...
            // Check if this terrain has an alias to something we actually will extend, and if so,
            // use it.
        ...
        if (!adjacent_type_count.empty()) {
            const auto most_common_adjacent = std::ranges::max_element(
                adjacent_type_count,
                [](const std::pair<oter_id, int>& p1, const std::pair<oter_id, int>& p2) {
                    return p1.second < p2.second;
                });

            mapgendata forest_mapgen_dat(dat, most_common_adjacent->first);
            mapgen_forest(forest_mapgen_dat);
            did_extend_adjacent_terrain =
                run_mapgen_func(most_common_adjacent->first.id().str(), forest_mapgen_dat);

            // One fun side effect of running another mapgen here is that it may have placed items
            // in locations that we're later going to turn into water. Let's just remove all items.
            if (did_extend_adjacent_terrain) {
                for (int x = 0; x < SEEX * 2; x++) {
                    for (int y = 0; y < SEEY * 2; y++) { m->i_clear(point_omt_ms(x, y)); }
                }
            }
        }
    }
    ...
    enum class n_type { lake, shore, river, solid };
    n_type neighbours[8];
    ...
    // We need to detect whether river bank is touching this map with its side, or corner, and
    // where. Checking our surrounding can give just enough information for that
    if (n_river_bank) {
        if (nw_water && (!ne_water || !e_water)) { slots[0] = ne_corner - river_beach_x; }
        if (ne_water && (!nw_water || !w_water)) { slots[1] = nw_corner + river_beach_x; }
    }

    if (w_river_bank) {
        if (sw_water && (!nw_water || !n_water)) { slots[2] = nw_corner + river_beach_y; }
        if (nw_water && (!sw_water || !s_water)) { slots[3] = sw_corner - river_beach_y; }
    }

    if (s_river_bank) {
        if (se_water && (!sw_water || !w_water)) { slots[4] = sw_corner + river_beach_x; }
        if (sw_water && (!se_water || !e_water)) { slots[5] = se_corner - river_beach_x; }
    }

    if (e_river_bank) {
        if (ne_water && (!se_water || !s_water)) { slots[6] = se_corner - river_beach_y; }
        if (se_water && (!ne_water || !n_water)) { slots[7] = ne_corner + river_beach_y; }
    }
    ...
    if (n_shore) {
        if (nw_lake || w_lake) { slots[0] = ne_corner - lake_beach_x; }
        if (ne_lake || e_lake) { slots[1] = nw_corner + lake_beach_x; }
    }

    if (w_shore) {
        if (sw_lake || s_lake) { slots[2] = nw_corner + lake_beach_y; }
        if (nw_lake || n_lake) { slots[3] = sw_corner - lake_beach_y; }
    }

    if (s_shore) {
        if (se_lake || e_lake) { slots[4] = sw_corner + lake_beach_x; }
        if (sw_lake || w_lake) { slots[5] = se_corner - lake_beach_x; }
    }

    if (e_shore) {
        if (ne_lake || n_lake) { slots[6] = se_corner - lake_beach_y; }
        if (se_lake || s_lake) { slots[7] = ne_corner + lake_beach_y; }
    }
    ...
    for (int i = 0; i < 8; i++) {
        ...
        if (pair == i) {
            ...
            bool water[8] =
                {n_water, n_water, w_water, w_water, s_water, s_water, e_water, e_water};
            int fallback = i;
            for (int j = modulo(i + dir, 8); j != i; j = modulo(j + dir, 8)) {
                if (!water[j]) {
                    fallback = j;
    ...
    const auto draw_shallow_water = [&](point_omt_ms from, point_omt_ms to) {
        ...
        for (auto& p : points) {
            for (point_omt_ms bp : closest_points_first(p, 1)) {
                if (!map_boundaries.contains(bp)) { continue; }
    ...
    const auto jittered_midpoint = [&](const point_omt_ms& from, const point_omt_ms& to) {
        ...
        const point_omt_ms
            midpoint((from.x() + to.x()) / 2 + roll(prng), (from.y() + to.y()) / 2 + roll(prng));
        return midpoint;
    };

    ter_id edge_tile = dat.zlevel() >= 0 ? t_water_sh : t_rock;
    ter_id water_tile =
        dat.zlevel() >= 0 ? t_water_dp
        : dat.zlevel() == dat.region.overmap_lake.lake_depth
            ? t_lake_bed
            : t_water_cube;
    ...
    if (n_lake || nw_lake || w_lake) { fill_deep_water(nw_corner); }

    if (s_lake || sw_lake || w_lake) { fill_deep_water(sw_corner); }

    if (n_lake || ne_lake || e_lake) { fill_deep_water(ne_corner); }

    if (s_lake || se_lake || e_lake) { fill_deep_water(se_corner); }

    // We previously placed our shallow water but actually did a t_null instead to make sure that we
    // didn't pick up shallow water from our extended terrain. Now turn those nulls into t_water_sh.
    m->translate(t_null, edge_tile);
}

void mremove_trap(mapgen_constructor* m, const point_omt_ms& p) { m->remove_trap(p); }

void mtrap_set(mapgen_constructor* m, const point_omt_ms& p, trap_id type) { m->trap_set(p, type); }
