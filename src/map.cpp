...
static auto get_sound_volume( const map_bash_info &bash, const bash_params &params ) -> int
{
    ...
    return units::to_decibel( bash.sound_vol.value_or( units::from_decibel( std::clamp(
                                  minvol + impact_strength, minvol, maxvol ) ) ) );
}
...
bash_results map::bash_ter_furn( const tripoint_bub_ms &p, const bash_params &params )
{
    ...
    if( !result.success ) {
        ...
        const auto sound_volume = std::min( 120,
                                            units::to_decibel( bash->sound_fail_vol.value_or( 70_dB ) ) );
