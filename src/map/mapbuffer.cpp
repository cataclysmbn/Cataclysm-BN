...
mapbuffer_abs_tile_view::mapbuffer_abs_tile_view(
    const tripoint_abs_sm& abs_sm, const point_sm_ms& local, const submap& sm)
    : abs_sm_(abs_sm),
      local_(local),
      sm_(&sm) {}

mapbuffer_abs_tile_view::operator bool() const { return sm_ != nullptr; }

auto mapbuffer_abs_tile_view::abs_pos() const -> tripoint_abs_ms {
    return project_combine(abs_sm_, local_);
}

auto mapbuffer_abs_tile_view::abs_submap_pos() const -> tripoint_abs_sm { return abs_sm_; }

auto mapbuffer_abs_tile_view::submap_pos() const -> point_sm_ms { return local_; }

auto mapbuffer_abs_tile_view::get_ter() const -> ter_id { return sm_->get_ter(local_); }

auto mapbuffer_abs_tile_view::get_furn() const -> furn_id { return sm_->get_furn(local_); }

auto mapbuffer_abs_tile_view::get_trap() const -> trap_id { return sm_->get_trap(local_); }

auto mapbuffer_abs_tile_view::get_ter_t() const -> const ter_t& { return get_ter().obj(); }

auto mapbuffer_abs_tile_view::get_furn_t() const -> const furn_t& { return get_furn().obj(); }

auto mapbuffer_abs_tile_view::get_trap_t() const -> const trap& { return get_trap().obj(); }

auto mapbuffer_abs_tile_view::get_field() const -> const field& { return sm_->get_field(local_); }
...
auto mapbuffer_abs_tile_view::get_radiation() const -> int { return sm_->get_radiation(local_); }

auto mapbuffer_abs_tile_view::get_lum() const -> std::uint8_t { return sm_->get_lum(local_); }
...
mapbuffer_abs_tile_with_vehicle_view::operator bool() const { return static_cast<bool>(tile_); }
...
auto mapbuffer_abs_tile_with_vehicle_view::passable() const -> bool { return move_cost() != 0; }

mapbuffer_abs_submap_view::mapbuffer_abs_submap_view(
    const tripoint_abs_sm& abs_sm, const submap& sm)
    : abs_sm_(abs_sm),
      sm_(&sm) {}

mapbuffer_abs_submap_view::operator bool() const { return sm_ != nullptr; }

auto mapbuffer_abs_submap_view::abs_pos() const -> tripoint_abs_sm { return abs_sm_; }

auto mapbuffer_abs_submap_view::get_submap() const -> const submap& { return *sm_; }
...
auto mapbuffer_abs_submap_view::tiles() const -> point_range<point_sm_ms> { return submap_tiles(); }

mapbuffer_abs_omt_view::mapbuffer_abs_omt_view(
    const tripoint_abs_omt& abs_omt, const std::array<const submap*, 4>& submaps)
    : abs_omt_(abs_omt),
      submaps_(submaps) {}

mapbuffer_abs_omt_view::operator bool() const { return has_any_submap(); }

auto mapbuffer_abs_omt_view::has_any_submap() const -> bool {
    return std::ranges::any_of(submaps_, [](const auto* sm) { return sm != nullptr; });
}

auto mapbuffer_abs_omt_view::abs_pos() const -> tripoint_abs_omt { return abs_omt_; }

auto mapbuffer_abs_omt_view::is_complete() const -> bool {
    return std::ranges::all_of(submaps_, [](const auto* sm) { return sm != nullptr; });
}
...
auto mapbuffer_abs_omt_view::get_submap_view(const point_omt_sm& local) const
    ...
    if (!index || submaps_[*index] == nullptr) { return std::nullopt; }
...
mapbuffer_abs_tile_reader::mapbuffer_abs_tile_reader(
    mapbuffer& buffer, const mapbuffer_lookup_options options)
    : buffer_(&buffer),
      options_(options) {}
...
void mapbuffer::unload_omt(const tripoint_abs_omt& omt_addr, bool save) {
    ...
    if (save) {
        ...
        const auto base = project_to<coords::sm>(omt_addr);
        for (const auto& off :
             {point_rel_sm::zero(), point_rel_sm::south(), point_rel_sm::east(),
              point_rel_sm::south_east()}) {
...
auto mapbuffer::simulated_submap_views(const int zlev) -> std::vector<mapbuffer_abs_submap_view> {
    ...
    const auto horizontal_positions = submap_loader.simulated_submaps(dimension_id_);
    result.reserve(
        horizontal_positions.empty() ? loaded_submap_count() : horizontal_positions.size());
    const auto options = mapbuffer_lookup_options{.mode = mapbuffer_lookup_mode::resident_only};
    for_each_simulated_submap_position(
...
auto mapbuffer::get_ter(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> std::optional<ter_id> {
...
auto mapbuffer::get_furn(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
...
auto mapbuffer::veh_at(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> optional_vpart_position {
    if (const auto local = active_reality_bubble_local(p)) { return g->m.veh_at(*local); }
...
auto mapbuffer::move_cost(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> std::optional<int> {
...
auto mapbuffer::passable(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> std::optional<bool> {
    const auto tile = get_abs_tile_with_vehicle(p, options);
    if (!tile) { return std::nullopt; }
...
auto mapbuffer::has_flag_furn_or_vpart(
    const std::string& flag, const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> bool {
    const auto tile = get_abs_tile(p, options);
    if (!tile) { return false; }
    if (tile->get_furn_t().has_flag(flag)) { return true; }
...
auto mapbuffer::has_flag(
    const ter_bitflags flag, const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> bool {
    return has_flag_ter_or_furn(flag, p, options);
}
...
auto mapbuffer::creature_on_trap(Creature& critter, const bool may_avoid) -> void {
    ...
    if (trap_here == tr_null) { trap_here = tile->get_trap(); }
    ...
    if (may_avoid && critter.avoid_trap(bubble_pos, tr)) { return; }
...
auto mapbuffer::water_from(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> detached_ptr<item> {
    const auto tile = lookup_tile(*this, p, options);
...
auto mapbuffer::add_item_or_charges(
    ...
    auto valid_tile = [&](const tripoint_abs_ms& target) -> std::optional<mapbuffer_tile_lookup> {
        ...
        if (!tile) { return std::nullopt; }
        if (tile_has_flag(*tile, "DESTROY_ITEM")) { return std::nullopt; }
        if (new_item->made_of(LIQUID) && tile_has_flag(*tile, "SWIMMABLE")) { return std::nullopt; }
        return tile;
    };

    auto valid_limits = [&](const mapbuffer_tile_lookup& tile) {
        const auto max_volume =
            tile.sm->get_furn(tile.local) != f_null
    ...
    auto route_allows_overflow = [&](const tripoint_abs_ms& target) {
        ...
        if (!source_local || !target_local) { return false; }
        const auto max_dist = 2;
        const auto max_path_length = 4 * max_dist;
        const auto setting =
            pathfinding_settings(0, max_dist, max_path_length, 0, false, true, false, false, false);
    ...
    auto place_item = [&](const tripoint_abs_ms& target, mapbuffer_tile_lookup& tile) {
        ...
        if (const auto local = active_reality_bubble_local(target)) { g->m.support_dirty(*local); }
    ...
    auto try_place = [&](const tripoint_abs_ms& target, const bool reject_noitem,
        ...
        if (call_drop_hook_first && call_active_drop_hook(target)) { return true; }
...
auto mapbuffer::run_omt_pillar_post_pass(const point_abs_omt& omt_pos) -> void {
    ...
    for (const auto zlev : std::views::iota(-OVERMAP_DEPTH, OVERMAP_HEIGHT + 1)) {
        ...
        for (const auto& offset : offsets) {
            ...
            if (changed) { mark_post_pass_changed(*sub_here); }
        }
    }
}
