...
#include "map/map_mutation_hooks.h"
...
auto calculate_vehicle_submap_footprints(
    const vehicle& veh, const tripoint_abs_ms& base, const std::size_t precalc_index)
    ...
    for (const vpart_reference& vpr : veh.get_all_parts()) {
        ...
        const auto part_pos =
            base
            + tripoint_rel_ms(
                part.precalc[precalc_index], part.mount.z() + part.z_terrain[precalc_index]);
        const auto part_sm = project_to<coords::sm>(part_pos);
        if (part_sm.z() < -OVERMAP_DEPTH || part_sm.z() > OVERMAP_HEIGHT) { continue; }
...
auto vehicle_submap_footprint_overlaps_active_bubble(
    const mapbuffer& buffer, const vehicle_submap_footprint& footprint) -> bool {
    if (g == nullptr || g->m.get_bound_dimension() != buffer.get_dimension_id()) { return false; }

    const auto bubble_min = g->m.get_abs_sub();
    const auto bubble_max = bubble_min + point_rel_sm(g->m.getmapsize() - 1, g->m.getmapsize() - 1);
...
auto invalidate_active_vehicle_footprints(
    const mapbuffer& buffer, const vehicle_submap_footprints& footprints) -> bool {
    if (g == nullptr || g->m.get_bound_dimension() != buffer.get_dimension_id()) { return false; }
    ...
    for (const auto& footprint : footprints) {
        ...
        invalidated = true;
        g->m.on_vehicle_moved(
            abs_to_map_local(g->m, footprint->min), abs_to_map_local(g->m, footprint->max),
            footprint->min.z());
    }
    return invalidated;
}
...
auto lookup_tile(
    mapbuffer& buffer, const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> std::optional<mapbuffer_tile_lookup> {
    if (buffer.is_outside_pocket_dimension_bounds(p)) { return std::nullopt; }
...
auto ensure_roof_above_cache(mapbuffer& buffer, submap& sm, const mapbuffer_lookup_options options)
    -> void {
    if (!sm.roof_above_dirty) { return; }
    auto* const above = buffer.get_submap(sm.position() + tripoint_above, options);
    sm.rebuild_roof_above_cache(above);
}

auto uniform_terrain_for_omt(const dimension_id& dimension_id, const tripoint_abs_omt& omt_addr)
    -> std::optional<ter_id> {
    ...
    if (terrain_type == air) { return t_open_air; }
    if (terrain_type == rock) { return t_rock; }
    return std::nullopt;
}

auto add_uniform_omt(mapbuffer& dest, const tripoint_abs_sm& base, const ter_id& terrain_type)
    -> bool {
    static constexpr auto offsets = std::array{
        point_rel_sm::zero(), point_rel_sm::east(), point_rel_sm::south(),
...
auto trap_at_tile(const submap& sm, const point_sm_ms& local) -> const trap& {
    ...
    if (terrain_trap != tr_null) { return terrain_trap.obj(); }
...
auto add_item_to_actualized_tile(const actualize_tile_options& options, detached_ptr<item>&& obj)
    -> void {
    options.buffer.add_item_or_charges(
        options.abs_pos, std::move(obj),
        {
...
auto add_spawn_to_submap(const add_submap_spawn_options& options) -> void {
    if (!options.type || MonsterGroupManager::monster_is_blacklisted(options.type)) { return; }
...
auto handle_decayed_corpse(const actualize_tile_options& options, const item& corpse) -> void {
    ...
    for (const auto& entry : dead_monster->harvest.obj()) {
        ...
        for (const auto ignored : std::views::iota(0, roll)) {
            ...
            if (!harvest->rotten()) { add_item_to_actualized_tile(options, item::spawn(*harvest)); }
        }
    }
...
auto rotten_item_spawn(const actualize_tile_options& options, const item& source) -> void {
    if (options.buffer.creature_at(options.abs_pos) != nullptr) { return; }

    const auto& comestible = source.get_comestible();
    if (!comestible || !comestible->rot_spawn) { return; }

    const auto chance = static_cast<int>(
        comestible->rot_spawn_chance * get_option<float>("CARRION_SPAWNRATE"));
    if (rng(0, 100) >= chance) { return; }

    const auto spawn_details = MonsterGroupManager::GetResultFromGroup(comestible->rot_spawn);
    const auto disposition =
        source.has_own_flag(flag_SPAWN_FRIENDLY)
    ...
    if (!g->u.sees(options.abs_pos)) { return; }
...
auto remove_rotten_items(const actualize_tile_options& options, location_vector<item>& items)
    -> void {
    ...
    items.remove_with([&](detached_ptr<item>&& it) {
        ...
        auto removed_snapshot =
            can_spawn_rot || can_decay_corpse ? item::spawn(*it) : detached_ptr<item>();
        it = item::actualize_rot(
            std::move(it),
            {
    ...
    for (const auto& corpse : decayed_corpses) { handle_decayed_corpse(options, *corpse); }
}
...
auto fill_funnels(const actualize_tile_options& options) -> void {
    ...
    if (biggest_container != items.end()) {
        retroactively_fill_from_funnel(
            **biggest_container, tr, options.last_touched, calendar::turn, options.abs_pos);
    }
}

auto remove_fertilizer(const actualize_tile_options& options, location_vector<item>& items)
    -> void {
...
auto grow_plant(const actualize_tile_options& options) -> void {
    ...
    if (seed_it == items.end()) {
        const auto overmap_terrain =
            get_overmapbuffer(options.buffer.get_dimension_id())
                .ter(project_to<coords::omt>(options.abs_pos));
    ...
    if (seed->age() < plant_epoch * 2) {
        if (furn.has_flag("GROWTH_SEEDLING")) { return; }
        remove_fertilizer(options, items);
        rotten_item_spawn(options, *seed);
        options.buffer
            .set_furn(options.abs_pos, furn_str_id(furn.plant->transform), options.lookup);
        return;
    }

    if (seed->age() < plant_epoch * 3 * furn.plant->growth_multiplier) {
        if (furn.has_flag("GROWTH_MATURE")) { return; }
        remove_fertilizer(options, items);
        rotten_item_spawn(options, *seed);
        if (!furn.has_flag("GROWTH_SEEDLING")) { rotten_item_spawn(options, *seed); }
        options.buffer
            .set_furn(options.abs_pos, furn_str_id(furn.plant->transform), options.lookup);
        return;
    }
...
auto restock_fruits(const actualize_tile_options& options) -> void {
    ...
    const auto& furn = options.sm.get_furn(options.local).obj();
    if (!ter.has_flag(TFLAG_HARVESTED) && !furn.has_flag(TFLAG_HARVESTED)) { return; }
...
auto produce_sap(const actualize_tile_options& options) -> void {
    ...
    if (options.elapsed >= calendar::year_length()) {
        ...
        if (last_producing && current_producing) {
            time_producing =
                options.elapsed < non_producing_length
                    ? options.elapsed
                    : options.elapsed - non_producing_length;
    ...
    auto& items = options.sm.get_items(options.local);
    for (auto& it : items) {
        if (!it->is_bucket() && !it->is_watertight_container()) { continue; }
...
auto rad_scorch(const actualize_tile_options& options) -> void {
    ...
    if (!x_in_y(1.0 * rads * rads * options.elapsed, 91_days)) { return; }
...
auto decay_cosmetic_fields(const actualize_tile_options& options) -> void {
    ...
    for (auto& pr : fields) {
        ...
        if (!fd.decays_on_actualize() || half_life <= 0_turns) { continue; }
...
auto tile_allows_item_despite_noitem_flag(const item& target, const mapbuffer_tile_lookup& tile)
    -> bool {
    return target.made_of(LIQUID) && tile_has_flag(tile, "LIQUIDCONT");
}

auto tile_allows_item_despite_noitem_flag(const item& target, const abs_tile_handle& tile) -> bool {
    return target.made_of(LIQUID) && tile.has_flag(TFLAG_LIQUIDCONT);
}

auto move_cost_from_tile_parts(
    const ter_id& terrain_id, const furn_id& furniture_id, const optional_vpart_position& vp)
    -> int {
    ...
    if (vp) {
        if (vp.obstacle_at_part()) { return 0; }
        if (vp.part_with_feature(VPFLAG_AISLE, true)) { return 2; }
        return 8;
    }
...
auto omt_submap_index(const point_omt_sm& local) -> std::optional<std::size_t> {
    if (local.x() < 0 || local.x() > 1 || local.y() < 0 || local.y() > 1) { return std::nullopt; }
...
auto vertical_transition_target_below(const ter_id& source) -> std::optional<ter_id> {
    ...
    if (source == t_stairs_down || source == t_stairs_down_no_roof) { return t_stairs_up; }
    if (source == t_stairs_down_underwater) { return t_stairs_up_underwater; }
    if (source == t_wood_stairs_down) { return t_wood_stairs_up; }
    if (source == t_ladder_down || source == t_ladder_up_down) { return t_ladder_up; }
    if (source == t_slope_down || source == t_slope_down_underground) { return t_slope_up; }
    if (source == t_triffid_slope_down) { return t_triffid_slope_up; }
    return std::nullopt;
}
...
auto vertical_transition_target_above(const ter_id& source) -> std::optional<ter_id> {
    ...
    if (source == t_stairs_up) { return t_stairs_down; }
    if (source == t_stairs_up_underwater) { return t_stairs_down_underwater; }
    if (source == t_wood_stairs_up) { return t_wood_stairs_down; }
    if (source == t_ladder_up || source == t_ladder_up_down) { return t_ladder_down; }
    if (source == t_slope_up) { return t_slope_down; }
    if (source == t_triffid_slope_up) { return t_triffid_slope_down; }
    return std::nullopt;
}

auto terrain_has_vertical_transition_direction(const ter_t& terrain, const ter_id& desired)
    -> bool {
...
auto can_replace_with_vertical_transition(
    const submap& sm, const point_sm_ms& local, const ter_id& desired) -> bool {
    const auto current = sm.get_ter(local);
    const auto& current_terrain = current.obj();
    if (terrain_has_vertical_transition_direction(current_terrain, desired)) { return false; }
    if (current_terrain.has_flag(TFLAG_GOES_UP) || current_terrain.has_flag(TFLAG_GOES_DOWN)) {
        return false;
    }
    ...
    if (current_terrain.has_flag(TFLAG_WALL) || current_terrain.has_flag(TFLAG_LIQUID)) {
        return false;
    }
    return current_terrain.has_flag(TFLAG_NO_FLOOR) || current_terrain.has_flag(TFLAG_SUPPORTS_ROOF)
        || current_terrain.has_flag(TFLAG_FLAT) || current_terrain.has_flag("ROOF");
}
...
abs_tile_handle::abs_tile_handle(const submap& sm, tripoint_abs_sm abs_sm, point_sm_ms local)
    : sm_(&sm),
      abs_sm_(abs_sm),
      local_(local),
      veh_part_(std::optional<vpart_position>()) {}

abs_tile_handle::abs_tile_handle(
    const submap& sm, tripoint_abs_sm abs_sm, point_sm_ms local, optional_vpart_position veh_part)
    : sm_(&sm),
      abs_sm_(abs_sm),
      local_(local),
      veh_part_(std::move(veh_part)) {}

abs_tile_handle::operator bool() const { return sm_ != nullptr; }

auto abs_tile_handle::abs_pos() const -> tripoint_abs_ms {
    return project_combine(abs_sm_, local_);
}

auto abs_tile_handle::abs_submap_pos() const -> tripoint_abs_sm { return abs_sm_; }

auto abs_tile_handle::submap_pos() const -> point_sm_ms { return local_; }

auto abs_tile_handle::ter() const -> ter_id { return sm_ ? sm_->get_ter(local_) : t_null; }

auto abs_tile_handle::furn() const -> furn_id {
    if (!sm_) { return f_null; }
    return sm_->get_furn(local_);
}

::trap_id abs_tile_handle::trap_id() const { return sm_ ? sm_->get_trap(local_) : tr_null; }
...
auto abs_tile_handle::radiation() const -> int { return sm_ ? sm_->get_radiation(local_) : 0; }

auto abs_tile_handle::lum() const -> std::uint8_t { return sm_ ? sm_->get_lum(local_) : 0; }

auto abs_tile_handle::move_cost(const vehicle* ignored_vehicle) const -> int {
    return move_cost_from_tile_parts(
        ter(), furn(),
        veh_part_ && ignored_vehicle && &veh_part_->vehicle() == ignored_vehicle
            ? optional_vpart_position{}
            : veh_part_);
}

auto abs_tile_handle::passable() const -> bool { return move_cost() != 0; }

auto abs_tile_handle::vehicle_part() const -> const optional_vpart_position& { return veh_part_; }
...
auto abs_tile_handle::has_flag_furn_or_vpart(const std::string& flag) const -> bool {
    return furn_obj().has_flag(flag) || static_cast<bool>(veh_part_.part_with_feature(flag, true));
}
...
auto abs_tile_handle::is_bashable(const bool allow_floor) const -> bool {
    if (veh_part_ && veh_part_->obstacle_at_part()) { return true; }
...
auto abs_tile_handle::is_bashable_ter(const bool allow_floor) const -> bool {
    ...
    return ter_bash.str_max != -1
        && ((!ter_bash.bash_below && !ter_obj().has_flag("VEH_TREAT_AS_BASH_BELOW"))
            || allow_floor);
}
...
auto abs_tile_handle::bash_rating(const int str, const bool allow_floor) const -> int {
    ...
    if (veh_part_ && veh_part_->obstacle_at_part()) { return 2; }
...
auto abs_tile_handle::is_divable() const -> bool {
    if (veh_part_.part_with_feature(VPFLAG_BOARDABLE, true)) { return false; }
    return ter_obj().has_flag("SWIMMABLE") && ter_obj().has_flag(TFLAG_DEEP_WATER);
}

auto abs_tile_handle::is_water_shallow_current() const -> bool {
    return ter_obj().has_flag("CURRENT") && !ter_obj().has_flag(TFLAG_DEEP_WATER);
}

auto abs_tile_handle::has_items() const -> bool { return !items().empty(); }

auto abs_tile_handle::has_field_at() const -> bool { return sm_ ? sm_->field_count > 0 : false; }
...
auto abs_tile_handle::has_signage() const -> bool { return sm_ ? sm_->has_signage(local_) : false; }
...
auto abs_tile_handle::impassable() const -> bool { return !passable(); }

auto abs_tile_handle::impassable_ter_furn() const -> bool { return !passable_ter_furn(); }

auto abs_tile_handle::passable_ter_furn() const -> bool { return move_cost_ter_furn() != 0; }
...
auto abs_tile_handle::is_harvestable() const -> bool {
    ...
    if (furn_id != f_null && furn_id.obj().examine != iexamine::none) {
        ...
        return !furniture.has_flag(TFLAG_HARVESTED) && !furniture.get_harvest().is_null()
            && !furniture.get_harvest()->empty();
    }
    const auto& terrain = ter_obj();
    return !terrain.get_harvest().is_null() && !terrain.get_harvest()->empty();
}

auto abs_tile_handle::dangerous_field_at() const -> bool {
    for (const auto& pr : field()) {
        if (pr.second.is_dangerous()) { return true; }
    }
    return false;
}
...
auto abs_tile_handle::tername() const -> std::string { return ter_obj().name(); }
...
auto abs_tile_handle::fetch(
    mapbuffer& buf, const tripoint_abs_ms p,
    const mapbuffer_lookup_options options) -> std::optional<abs_tile_handle> // *NOPAD*
{
    const auto split = project_remain<coords::sm>(p);
    auto* const sm = buf.get_submap(split.quotient_tripoint, options);
    if (sm == nullptr) { return std::nullopt; }
    return abs_tile_handle(
        *sm, split.quotient_tripoint, split.remainder, buf.vehicle_part_at_loaded_tile(p));
}

auto abs_tile_handle::fetch_terrain_only(mapbuffer& buf, const tripoint_abs_ms p)
    -> std::optional<abs_tile_handle> // *NOPAD*
{
    return fetch_terrain_only(
        buf, p,
        {
            .mode = mapbuffer_lookup_mode::resident_only,
        });
...
auto abs_tile_handle::fetch_terrain_only(
    mapbuffer& buf, const tripoint_abs_ms p,
    const mapbuffer_lookup_options options) -> std::optional<abs_tile_handle> // *NOPAD*
{
    const auto split = project_remain<coords::sm>(p);
...
submap_tile_range::submap_tile_range(
    const submap& sm, tripoint_abs_sm abs_sm, const bool has_vehicles, mapbuffer& buf)
    : sm_(&sm),
      abs_sm_(abs_sm),
...
auto submap_tile_range::end() const -> iterator {
    return iterator(
        *sm_, abs_sm_, has_vehicles_, buf_, static_cast<int>(SEEX) * static_cast<int>(SEEY));
}

submap_tile_range::iterator::iterator(
    const submap& sm, tripoint_abs_sm abs_sm, const bool has_vehicles, mapbuffer* const buf,
    const int idx)
    : sm_(&sm),
      abs_sm_(abs_sm),
      has_vehicles_(has_vehicles),
...
auto submap_tile_range::iterator::operator*() const -> abs_tile_handle {
    ...
    if (has_vehicles_) {
        return abs_tile_handle(
            *sm_, abs_sm_, local,
            buf_->vehicle_part_at_loaded_tile(project_combine(abs_sm_, local)));
    } else {
...
bool simulated_island::contains(point_abs_sm p) const {
    if (p.x() < begin.x() || p.x() >= end.x() || p.y() < begin.y() || p.y() >= end.y()) {
        return false;
    }
    ...
    -> std::vector<point_abs_sm> // *NOPAD*
{
    ...
    const point_abs_sm
        scan_begin(std::max(r_begin.x(), begin.x()), std::max(r_begin.y(), begin.y()));
    const point_abs_sm scan_end(std::min(r_end.x(), end.x()), std::min(r_end.y(), end.y()));
...
auto simulated_island::size() const -> std::size_t { return bits_.size(); }
    ...
    -> std::vector<simulated_island> // *NOPAD*
{
    ...
    for (const point_abs_sm& seed : columns) {
        ...
        for (const point_abs_sm& p : component) {
            const int idx = (p.x() - island.begin.x()) + (p.y() - island.begin.y()) * w;
...
submap_tile_iterator_range::iterator::iterator(
    mapbuffer& buf, std::vector<point_abs_sm> columns, const int z, const tripoint_abs_ms center,
    const int radius, const tripoint_abs_ms rect_begin, const tripoint_abs_ms rect_end,
    const int mode)
    : buf_(&buf),
    ...
    while (column_idx_ < static_cast<int>(columns_.size())) {
        while (tile_idx_ < tiles_per_submap) {
            ...
            if (mode_ == 1) {
                ...
                if (square_dist(abs_pos.xy(), center_.xy()) <= radius_) { return; }
            } else if (mode_ == 2) {
...
auto submap_tile_iterator_range::iterator::operator*() const -> abs_tile_handle {
    ...
    if (sm) {
        ...
        if (has_veh) {
            return abs_tile_handle(
                *sm, sm_pos, local,
                buf_->vehicle_part_at_loaded_tile(project_combine(sm_pos, local)));
        } else {
...
auto submap_tile_iterator_range::begin() const -> iterator {
    return iterator(*buf_, columns_, z_, center_, radius_, rect_begin_, rect_end_, mode_);
}
...
auto simulated_tiles_in_radius(mapbuffer& buf, const tripoint_abs_ms center, const int radius)
    -> submap_tile_iterator_range // *NOPAD*
{
    // Compute bounding box in abs_ms coordinates
    const tripoint_abs_ms bb_begin(center.x() - radius, center.y() - radius, center.z());
    const tripoint_abs_ms bb_end(center.x() + radius, center.y() + radius, center.z());
    ...
    const point_abs_sm sm_end = project_to<coords::sm>(bb_end).xy() + point_rel_sm(1, 1);
    ...
    for (const simulated_island& island : buf.simulated_islands()) {
        ...
        columns.insert(columns.end(), std::make_move_iterator(island_cols.begin()),
...
auto simulated_tiles_in_rectangle(
    mapbuffer& buf, const tripoint_abs_ms begin, const tripoint_abs_ms end)
    -> submap_tile_iterator_range // *NOPAD*
{
    ...
    const point_abs_sm sm_end = project_to<coords::sm>(end).xy() + point_rel_sm(1, 1);
...
auto simulated_tiles_on_zlevel(mapbuffer& buf, const int z) -> submap_tile_iterator_range // *NOPAD*
{
...
auto mapbuffer::for_each_submap_tile(
    const submap& sm, tripoint_abs_sm abs_sm, const std::function<void(const abs_tile_handle&)>& fn)
    -> void {
    for (const point_sm_ms& local : ::submap_tiles()) {
        fn(abs_tile_handle(
            sm, abs_sm, local, vehicle_part_at_loaded_tile(project_combine(abs_sm, local))));
    }
}

auto mapbuffer::for_each_simulated_tile(
    const int zlev, const std::function<void(const abs_tile_handle&)>& fn) -> void {
    for_each_simulated_submap([&](const tripoint_abs_sm& abs_sm, submap& sm) {
        if (abs_sm.z() != zlev) { return; }
        for (const point_sm_ms& local : ::submap_tiles()) {
            fn(abs_tile_handle(
                sm, abs_sm, local, vehicle_part_at_loaded_tile(project_combine(abs_sm, local))));
        }
...
mapbuffer_bounds_view::mapbuffer_bounds_view(
    mapbuffer& buffer, const point_abs_sm& begin, const point_abs_sm& end,
...
auto mapbuffer_bounds_view::begin() const -> point_abs_sm { return begin_; }

auto mapbuffer_bounds_view::end() const -> point_abs_sm { return end_; }

auto mapbuffer_bounds_view::bounds_size() const -> point_rel_sm {
    return point_rel_sm(end_.x() - begin_.x(), end_.y() - begin_.y());
}

auto mapbuffer_bounds_view::indexed_submap_index(const point_rel_sm& offset, const int zlev) const
    -> std::optional<std::size_t> {
    const auto size = bounds_size();
    if (offset.x() < 0 || offset.y() < 0 || offset.x() >= size.x() || offset.y() >= size.y()) {
        return std::nullopt;
    }
    if (zlev < -OVERMAP_DEPTH || zlev > OVERMAP_HEIGHT) { return std::nullopt; }
...
auto mapbuffer_bounds_view::get_submap_view(const tripoint_abs_sm& pos) const
...
auto mapbuffer_bounds_view::get_submap_view(const point_rel_sm& offset, const int zlev) const
    -> std::optional<submap_ref> {
    const auto index = indexed_submap_index(offset, zlev);
    if (!index || *index >= indexed_submaps_.size()) { return std::nullopt; }
...
auto mapbuffer_bounds_view::update(
    const point_abs_sm& begin, const point_abs_sm& end, mapbuffer* buffer) -> void {
    if (buffer != nullptr) { buffer_ = buffer; }
    begin_ = begin;
    end_ = end;
    submaps_.clear();
    indexed_submaps_.clear();
    for (auto& submaps : submaps_by_zlev_) { submaps.clear(); }

    const auto size = bounds_size();
    if (buffer_ == nullptr || size.x() <= 0 || size.y() <= 0) { return; }
...
mapbuffer_load_region::mapbuffer_load_region(
    mapbuffer& buffer, const load_request_source source, const point_abs_sm& begin,
    const point_abs_sm& end, const mapbuffer_lookup_options options)
    : buffer_(&buffer),
      source_(source),
      options_(options) {
    update(begin, end);
...
mapbuffer_load_region::~mapbuffer_load_region() { release(); }
...
auto mapbuffer_load_region::update(const point_abs_sm& begin, const point_abs_sm& end) -> void {
    begin_ = begin;
    end_ = end;
    if (buffer_ == nullptr) {
    ...
    if (handle_ == 0) {
        ...
        handle_ =
            submap_loader.request_load(source_.value(), buffer_->get_dimension_id(), begin_, end_);
    } else {
...
auto mapbuffer::register_submap_vehicles(const tripoint_abs_sm& p, submap& sm) -> void {
    for (const auto& veh : sm.vehicles) {
        if (veh == nullptr || veh->part_count() <= 0) { continue; }
        veh->abs_sm_pos = p;
        ...
        const auto overlaps_active_bubble =
            std::ranges::any_of(footprints, [&](const auto& footprint) {
                return footprint
                    && vehicle_submap_footprint_overlaps_active_bubble(*this, *footprint);
            });
...
auto mapbuffer::unindex_vehicle_footprint_unlocked(const vehicle* veh) -> void {
    ...
    if (locations_iter == vehicle_footprint_locations_.end()) { return; }

    for (const auto& pos : locations_iter->second) {
        const auto footprint_iter = vehicle_footprint_by_location_.find(pos);
        if (footprint_iter == vehicle_footprint_by_location_.end()) { continue; }
...
auto mapbuffer::indexed_vehicle_part_at_unlocked(const tripoint_abs_ms& p)
    ...
    std::erase_if(entries, [&](const vehicle_footprint_entry& entry) {
        ...
        if (entry.part_index >= static_cast<std::size_t>(veh->part_count())) { return true; }
    ...
    if (selected == nullptr) { return optional_vpart_position(std::nullopt); }
...
bool mapbuffer::add_submap(const tripoint_abs_sm& p, std::unique_ptr<submap>& sm) {
    ...
    if (!submaps[p]->active_items.empty()) { submaps_with_active_items_.insert(p); }
...
auto mapbuffer::is_column_state(
    const point_abs_sm col, const submap_column_load_state min_state) const noexcept -> bool {
    std::lock_guard<std::recursive_mutex> lk(submaps_mutex_);
...
bool mapbuffer::add_submap(
    const tripoint_abs_sm& p, std::unique_ptr<submap>& sm,
...
void mapbuffer::set_simulated_submaps(const std::unordered_set<point_abs_sm>& columns) {
    auto lk = std::lock_guard<std::recursive_mutex>(submaps_mutex_);

    const bool simulation_set_changed =
        std::ranges::any_of(
            column_states_,
            [&columns](const auto& entry) {
                return (entry.second == submap_column_load_state::simulated)
                    != columns.contains(entry.first);
            })
        || std::ranges::any_of(columns, [this](const point_abs_sm& column) {
               return !column_states_.contains(column);
           });
    auto queued_sounds = std::vector<sound_event>();
    if (simulation_set_changed) {
        for (auto& island_sounds : island_sounds_) {
            for (auto& sound : island_sounds) { queued_sounds.push_back(std::move(sound)); }
        }
    }
    ...
    for (const point_abs_sm& col : newly_simulated) {
        for (const auto z : std::views::iota(-OVERMAP_DEPTH, OVERMAP_HEIGHT + 1)) {
            ...
            if (submaps.contains(sm_pos)) {
                refresh_vehicle_registry_for_submap(
                    sm_pos,
                    {
                        .mode = mapbuffer_lookup_mode::resident_only,
                    });
                refresh_active_item_submap_index(
                    sm_pos,
                    mapbuffer_lookup_options{
                        .mode = mapbuffer_lookup_mode::resident_only,
                    });
    ...
    for (const point_abs_sm& col : last_demoted_columns_) {
        for (const auto z : std::views::iota(-OVERMAP_DEPTH, OVERMAP_HEIGHT + 1)) {
            ...
            if (it != submaps.end() && it->second) { it->second->last_touched = calendar::turn; }
            if (auto* tracker = get_distribution_grid_tracker_for(dimension_id_)) {
    ...
    for (size_t idx = 0; idx < simulated_islands_.size(); ++idx) {
        ...
        for (int y = isl.begin.y(); y < isl.end.y(); ++y) {
            for (int x = isl.begin.x(); x < isl.end.x(); ++x) {
                ...
                if (isl.contains(p)) { column_to_island_[p] = idx; }
            }
        }
        island_sounds_[idx].clear();
    }

    for (auto& sound : queued_sounds) {
        const auto island = column_to_island_.find(project_to<coords::sm>(sound.origin.xy()));
        if (island != column_to_island_.end()) {
            island_sounds_[island->second].push_back(std::move(sound));
        }
    }
}
    ...
    if (g != nullptr && m_target->second) {
        ...
        if (here.inbounds(addr)) {
            debugmsg("remove_submap: skipping free of submap at %s (ptr %p) "
                     ...
                     addr.to_string(), static_cast<const void*>(doomed), dimension_id_.c_str());
...
void mapbuffer::transfer_all_to(mapbuffer& dest) {
    for (auto& kv : submaps) {
        ...
        if (!kv.second->active_items.empty()) { dest.submaps_with_active_items_.insert(kv.first); }
    ...
    for (auto& kv : column_states_) {
        ...
        if (submaps.find(tripoint_abs_sm(kv.first, 0)) == submaps.end()) { continue; }
        ...
        if (dirty_columns_.contains(kv.first)) { dest.dirty_columns_.insert(kv.first); }
    }
...
void mapbuffer::unload_omt(const tripoint_abs_omt& omt_addr) {
    ...
    const std::array<tripoint_abs_sm, 4> addrs = {
        {base, base + point_rel_sm::east(), base + point_rel_sm::south(),
    ...
    if (is_dirty) {
        ...
        if (!all_uniform && !disable_mapgen) {
            std::ostringstream buf;
            {
                ...
                for (const tripoint_abs_sm& addr : addrs) {
                    const auto it = submaps.find(addr);
                    if (it == submaps.end() || !it->second) { continue; }
                    jsout.start_object();
        ...
        for (const tripoint_abs_sm& addr : addrs) {
            if (submaps.contains(addr)) { to_delete.push_back(addr); }
        }
    } else {
        ...
        for (const auto& addr : addrs) { to_delete.push_back(addr); }
    }
    ...
    if (g != nullptr) {
        ...
        to_delete.remove_if([&](const tripoint_abs_sm& p) {
            ...
            if (it == submaps.end() || !it->second) { return false; }
            const submap* doomed = it->second.get();
            if (here.inbounds(p)) {
                debugmsg("unload_omt: skipping free of submap at %s (ptr %p) "
                         "— reality bubble still references it (dim='%s')",
                         p.to_string(), static_cast<const void*>(doomed), dimension_id_.c_str());
...
submap* mapbuffer::lookup_submap(const tripoint_abs_sm& p) {
    ...
    if (resident_sm != nullptr) { return resident_sm; }
    ...
    try {
        ...
        if (!pending_data.empty()) {
            ...
            found = g->get_active_world()->read_map_omt(
                dimension_id_.str(), omt_addr, [this, &loaded, &already_loaded](JsonIn& jsin) {
                    deserialize_into_vec(jsin, loaded, already_loaded);
                });
        }
...
auto mapbuffer::get_submap(const tripoint_abs_sm& p, const mapbuffer_lookup_options options)
    -> submap* {
    switch (options.mode) {
        case mapbuffer_lookup_mode::simulated_only:
            if (!submap_loader.is_simulated(dimension_id_, p)) { return nullptr; }
        ...
        case mapbuffer_lookup_mode::load_or_generate:
            if (auto* sm = lookup_submap(p)) { return sm; }
...
auto mapbuffer::for_each_simulated_submap_position(
    ...
    if (zlev_filter && (*zlev_filter < -OVERMAP_DEPTH || *zlev_filter > OVERMAP_HEIGHT)) { return; }
...
auto mapbuffer::simulated_submap_positions() const -> std::vector<tripoint_abs_sm> {
    ...
    const auto horizontal_positions = submap_loader.simulated_submaps(dimension_id_);
    result.reserve(
        horizontal_positions.empty()
            ? loaded_submap_count()
            : horizontal_positions.size() * static_cast<std::size_t>(OVERMAP_LAYERS));
    for_each_simulated_submap_position([&](const tripoint_abs_sm& pos) { result.push_back(pos); });
    return result;
}

auto mapbuffer::mark_submap_caches_dirty(const mapbuffer_mark_submap_caches_dirty_options& options)
    -> void {
    if (options.zlev < -OVERMAP_DEPTH || options.zlev > OVERMAP_HEIGHT) { return; }
    if (options.begin.x() >= options.end.x() || options.begin.y() >= options.end.y()) { return; }
...
auto mapbuffer::queue_sound(sound_event evt) -> void {
    ...
    if (it != column_to_island_.end()) { island_sounds_[it->second].push_back(std::move(evt)); }
    // Sound origin not in a simulated column — discard.
}

auto mapbuffer::island_sounds_for(point_abs_sm col) -> std::vector<sound_event>* {
    const auto it = column_to_island_.find(col);
    if (it != column_to_island_.end()) { return &island_sounds_[it->second]; }
    return nullptr;
}

auto mapbuffer::island_for(point_abs_sm col) const -> const simulated_island* {
    const auto it = column_to_island_.find(col);
    if (it != column_to_island_.end()) { return &simulated_islands_[it->second]; }
    return nullptr;
}
    ...
    for (const auto zlev : std::views::iota(-OVERMAP_DEPTH, OVERMAP_HEIGHT + 1)) {
        for (const auto pos : point_range<point_abs_sm>(options.begin, max)) {
            ...
            for (const auto local : ::submap_tiles()) {
                if (sm->get_trap(local) == tr_null) { continue; }
...
auto mapbuffer::run_submap_batch_turns(const mapbuffer_run_submap_batch_turns_options& options)
    -> void {
    if (options.turns <= 0) { return; }
    if (options.begin.x() >= options.end.x() || options.begin.y() >= options.end.y()) { return; }
...
auto mapbuffer::creature_tracker() -> Creature_tracker& { return creature_tracker_; }

auto mapbuffer::creature_tracker() const -> const Creature_tracker& { return creature_tracker_; }
...
auto mapbuffer::creature_at(
    ...
    const bool allow_hallucination) const -> Creature* // *NOPAD*
{
    ...
    if (const auto guy = find_active_npc(p)) { return guy.get(); }
    return nullptr;
}
...
auto mapbuffer::add_active_npc(const shared_ptr_fast<npc>& guy) -> bool {
    if (!guy || guy->is_dead()) { return false; }
    ...
    if (const auto existing = find_active_npc(guy->abs_pos())) {
        if (existing.get() != guy.get()) {
            debugmsg("Tried to add NPC %s to occupied active NPC tile %s", guy->get_name(),
                     guy->abs_pos().to_string());
            return false;
        }
        return true;
    }
    ...
    if (iter == active_npcs_.end()) { active_npcs_.push_back(guy); }
...
auto mapbuffer::remove_active_npc_from_location_map(const npc& guy) -> void {
    ...
    const auto iter = std::ranges::find_if(
        active_npcs_by_location_, [&](const decltype(active_npcs_by_location_)::value_type& v) {
            return v.second.get() == &guy;
        });
    if (iter != active_npcs_by_location_.end()) { active_npcs_by_location_.erase(iter); }
}
...
auto mapbuffer::update_active_npc_pos(const npc& guy, const tripoint_abs_ms& new_pos) -> bool {
    ...
    if (const auto existing = find_active_npc(new_pos)) {
        if (existing.get() != &guy) {
            debugmsg("Tried to move NPC %s to occupied active NPC tile %s", guy.get_name(),
                     new_pos.to_string());
            return false;
        }
    }
    ...
    if (iter == active_npcs_.end()) { return false; }
...
auto mapbuffer::remove_active_npc(const npc& guy) -> void {
    ...
    if (iter != active_npcs_.end()) { active_npcs_.erase(iter); }
}
    ...
    if (veh != nullptr && g != nullptr && g->m.get_bound_dimension() == dimension_id_) {
        ...
        for (const vpart_reference& vpr : veh->get_all_parts()) {
            if (!vpr.part().removed) {
                ...
                if (!here.inbounds_z(part_pos.z())) { continue; }
                here.clear_vehicle_point_from_cache(veh, abs_to_map_local(here, part_pos));
            }
        }
...
auto mapbuffer::refresh_vehicle_footprint(vehicle* veh) -> void {
    ...
    std::lock_guard<std::recursive_mutex> lk(submaps_mutex_);
    if (!loaded_vehicles_.contains(veh)) { return; }
    index_vehicle_footprint_unlocked(*veh);
    ...
    const auto footprints = calculate_vehicle_submap_footprints(*veh);
    const auto overlaps_active_bubble = std::ranges::any_of(footprints, [&](const auto& footprint) {
        return footprint && vehicle_submap_footprint_overlaps_active_bubble(*this, *footprint);
    });
    if (!overlaps_active_bubble) { return; }

    map& here = get_map();
    here.invalidate_max_populated_zlev(veh->abs_sm_pos.z());
    auto& cache = here.get_cache(veh->abs_sm_pos.z());
    cache.vehicle_list.insert(veh);
    if (!veh->loot_zones.empty()) { cache.zone_vehicles.insert(veh); }
...
auto mapbuffer::refresh_vehicle_registry_for_submap(
    const tripoint_abs_sm& p, const mapbuffer_lookup_options options) -> void {
    std::lock_guard<std::recursive_mutex> lk(submaps_mutex_);
    unregister_submap_vehicles(p);
...
static bool has_rail_at_abs(const mapbuffer& buf, const tripoint_abs_ms& p) {
    ...
    if (tile->has_flag_ter_or_furn("RAIL")) { return true; }
    if (!tile->has_flag_ter_or_furn("NO_FLOOR")) { return false; }
...
static bool scan_rails_from_veh_abs(
    const mapbuffer& buf, const vehicle& veh, tripoint_abs_ms scan_initial_pos,
    ...
    for (size_t rail_id = 0; rail_id < veh.rail_profile.size(); rail_id++) {
        ...
        for (int step = 0; step < 3; step++) {
            ...
            if (!has_rail_at_abs(buf, p)) { return false; }
        }
    }
    return true;
}

static bool scan_rails_at_shift_abs(
    const mapbuffer& buf, const vehicle& veh, int velocity_sign, units::angle dir, int shift_sign) {
    point ray_delta;
    {
        tileray ray(dir);
        ray.advance(1);
...
auto mapbuffer::obstructed_by_vehicle_rotation(
    const tripoint_abs_ms& from, const tripoint_abs_ms& to) const -> bool {
    if (from.xy() == to.xy() && from.z() == to.z()) { return false; }
    ...
    const auto check_vehicle_block =
        [this](const tripoint_abs_ms& part_tile, const tripoint_abs_ms& neighbor) -> bool {
    ...
    const auto direct_obstruction = [&]() {
        ...
        if (delta == tripoint_rel_ms::north_west() || delta == tripoint_rel_ms::north_east()) {
...
auto mapbuffer::vehicle_wheel_traction(
    const vehicle& veh, const bool ignore_movement_modifiers) const -> float {
    ...
    for (const int p : wheel_indices) {
        ...
        // Deep water and air
        if (tr.has_flag(TFLAG_DEEP_WATER) || tr.has_flag(TFLAG_NO_FLOOR)) { continue; }

        int move_mod = tile->move_cost_ter_furn();
        if (move_mod == 0) { return 0.0f; }
...
auto mapbuffer::vehicle_vehicle_collision(
    ...
    if (!vertical) {
        ...
        collision_axis_y.x =
            (veh.abs_ms_location().x() + x_cof1) - (veh2.abs_ms_location().x() + x_cof2);
        collision_axis_y.y =
            (veh.abs_ms_location().y() + y_cof1) - (veh2.abs_ms_location().y() + y_cof2);
...
auto mapbuffer::shake_vehicle(vehicle& veh, const int velocity_before, const units::angle direction)
    -> units::angle {
    ...
    for (const rider_data& r : riders) {
        ...
        if (rider->abs_pos() != part_pos) {
            debugmsg("throw passenger: passenger at %d,%d,%d, part at %d,%d,%d",
                     rider->abs_pos().x(), rider->abs_pos().y(), rider->abs_pos().z(), part_pos.x(),
        ...
        if (throw_from_seat) {
            if (psg) {
                psg->add_msg_player_or_npc(
                    m_bad, _("You are hurled from the %s's seat by the power of the impact!"),
...
auto mapbuffer::move_vehicle(vehicle& veh, const tripoint_rel_ms& dp, const tileray& facing)
    -> vehicle* {
    ...
    const auto old_footprints = calculate_vehicle_submap_footprints(veh);
    const auto vehicle_overlaps_active_bubble =
        std::ranges::any_of(old_footprints, [&](const auto& footprint) {
            return footprint && vehicle_submap_footprint_overlaps_active_bubble(*this, *footprint);
        });
    ...
    const auto projected_footprints =
        calculate_vehicle_submap_footprints(veh, veh.abs_ms_location() + dp1, 1);
    const auto projected_vehicle_overlaps_active_bubble =
        std::ranges::any_of(projected_footprints, [&](const auto& footprint) {
            return footprint && vehicle_submap_footprint_overlaps_active_bubble(*this, *footprint);
        });
    ...
    do {
        ...
        if (vertical && velocity_before < 0 && coll_velocity > 0) { veh.vertical_velocity = 0; }
    ...
    if (veh_veh_coll_flag) { return nullptr; }
    ...
    if (!vertical && !veh.valid_wheel_config() && !veh.is_in_water() && !veh.is_flying_in_air()
        ...
        for (const auto& p : veh.get_points()) {
            ...
            if (tile) {
                ...
                if (pter == t_dirt || pter == t_grass) { set_ter(p, t_dirtmound); }
            }
        }
    }
    ...
    if (const auto local = active_reality_bubble_local(veh.abs_ms_location())) {
        ...
        if (g->u.controlling_vehicle && veh_pointer_or_null(veh_at(g->u.abs_pos())) == &veh) {
            ...
            if (veh.skidding && can_move) { veh.possibly_recover_from_skid(); }
        }
    }
...
static bool can_place_monster_at(const monster& mon, const tripoint_abs_ms& p, mapbuffer& buf) {
    if (const auto existing = buf.creature_at(p)) {
        const auto* mon_ptr = dynamic_cast<const monster*>(existing);
        if (mon_ptr && !mon_ptr->is_hallucination()) { return false; }
        if (!mon_ptr) {
            // NPC or player
            return false;
        }
...
auto mapbuffer::place_critter_at(const shared_ptr_fast<monster>& mon, const tripoint_abs_ms& p)
    -> monster* {
    return place_critter_around(mon, p, 0);
}

auto mapbuffer::place_critter_around(
    const mtype_id& id, const tripoint_abs_ms& center, const int radius) -> monster* {
...
auto mapbuffer::place_critter_around(
    const shared_ptr_fast<monster>& mon, const tripoint_abs_ms& center, const int radius,
    const bool forced) -> monster* {
    std::optional<tripoint_abs_ms> where;

    if (forced || can_place_monster_at(*mon, center, *this)) { where = center; }
    ...
    for (int r = 1; r <= radius && !where; ++r) {
        ...
        if (!candidates.empty()) { where = random_entry(candidates); }
    }
...
auto mapbuffer::place_npc(
    const tripoint_abs_ms& p, const string_id<npc_template>& type, const bool force)
    -> character_id {
    if (!force && !get_option<bool>("STATIC_NPC")) { return character_id(); }
...
auto mapbuffer::is_sheltered(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> bool {
    if (const auto local = active_reality_bubble_local(p)) { return g->m.is_sheltered(*local); }
...
auto mapbuffer::is_suspension_valid(
    const tripoint_abs_ms& point, const mapbuffer_lookup_options options) -> bool {
    ...
    // Check: if both east and west are not open air — supported
    {
        ...
        if (te && tw && *te != t_open_air && *tw != t_open_air) { return true; }
    }
    // Check: if both southeast and northwest are not open air — supported
    {
        ...
        if (tse && tnw && *tse != t_open_air && *tnw != t_open_air) { return true; }
    }
    // Check: if both south and north are not open air — supported
    {
        ...
        if (ts && tn && *ts != t_open_air && *tn != t_open_air) { return true; }
    }
    // Check: if both northeast and southwest are not open air — supported
    {
        ...
        if (tne && tsw && *tne != t_open_air && *tsw != t_open_air) { return true; }
    }
    return false;
}

auto mapbuffer::collapse_invalid_suspension(
    const tripoint_abs_ms& point, const mapbuffer_lookup_options options) -> void {
...
auto mapbuffer::propagate_suspension_check(
    const tripoint_abs_ms& point, const mapbuffer_lookup_options options) -> void {
    for (const auto& neighbor : simulated_tiles_in_radius(*this, point, 1)) {
        if (neighbor.abs_pos() != point && has_flag(TFLAG_SUSPENDED, neighbor.abs_pos(), options)) {
            collapse_invalid_suspension(neighbor.abs_pos(), options);
        }
    }
}

auto mapbuffer::collapse_check(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> int {
    ...
    if (p.z() > -OVERMAP_DEPTH) {
        ...
        for (const auto& tbelow : points_in_radius(pbelow, 1)) {
            if (has_flag(TFLAG_SUPPORTS_ROOF, tbelow, options)) {
                num_supports += 1;
                if (has_flag(TFLAG_WALL, tbelow, options)) { num_supports += 2; }
...
auto mapbuffer::crush(const tripoint_abs_ms& p, const mapbuffer_lookup_options options) -> void {
    auto crushed_creature = creature_at(p);
    if (!crushed_creature) { return; }
...
auto mapbuffer::collapse_at(
    const tripoint_abs_ms& p, const bool silent, const bool was_supporting, const bool destroy_pos,
    ...
    if (supports && !still_supports) {
        for (const auto& t : points_in_radius(p, 1)) {
            ...
            if (!one_in(collapse_check(tz, options))) { continue; }
            ...
            if (p != t
                && (has_flag(TFLAG_SUPPORTS_ROOF, t, options)
...
auto mapbuffer::revive_corpse(item& it) -> bool {
    ...
    shared_ptr_fast<monster> newmon_ptr = make_shared_fast<monster>(it.get_mtype()->id);
...
auto mapbuffer::emit_field(
    const tripoint_abs_ms& pos, const emit_id& src, const float mul,
    const mapbuffer_lookup_options options) -> void {
...
auto mapbuffer::get_fishable_locations(
    const int radius, const tripoint_abs_ms& fish_pos,
    const mapbuffer_lookup_options options) -> std::unordered_set<tripoint_abs_ms> // *NOPAD*
{
    std::unordered_set<tripoint_abs_ms> visited;

    const tripoint_abs_ms fishing_boundary_min(fish_pos + point_rel_ms(-radius, -radius));
    const tripoint_abs_ms fishing_boundary_max(fish_pos + point_rel_ms(radius, radius));

    const inclusive_cuboid<tripoint_abs_ms>
        fishing_boundaries(fishing_boundary_min, fishing_boundary_max);
    ...
    while (!to_check.empty()) {
        ...
        if (visited.contains(current_point)) { continue; }

        if (!fishing_boundaries.contains(current_point)) { continue; }
...
auto mapbuffer::is_cornerfloor(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> bool {
    // Check if the tile itself is passable
    const auto tile_passable = passable(p, options);
    if (!tile_passable) { return false; }
    ...
    for (const auto& pt : simulated_tiles_in_radius(*this, p, 1)) {
        ...
        if (!pt_passable) { impassable_adjacent.insert(pt.abs_pos()); }
    }

    if (impassable_adjacent.empty()) { return false; }
    ...
    for (const auto& impassable_diagonal : diagonals) {
        if (impassable_adjacent.contains(impassable_diagonal)) {
            ...
            for (const auto& l : simulated_tiles_in_radius(*this, impassable_diagonal, 1)) {
                if (impassable_adjacent.contains(l.abs_pos())) { f++; }
                if (f > 2) { return true; }
            }
        }
    }
...
auto mapbuffer::mop_spills(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> bool {
    bool retval = false;
    ...
    if (fld) {
        ...
        for (const field_type_id& fid : to_check) { retval |= fld->remove_field(fid); }
    }
...
auto mapbuffer::set_ter(
    const tripoint_abs_ms& p, const ter_id terrain, const mapbuffer_lookup_options options)
    -> bool {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return false; }

    const auto old_id = tile->sm->get_ter(tile->local);
    if (old_id == terrain) { return false; }
...
auto mapbuffer::set_furn(
    const tripoint_abs_ms& p, const furn_id furn, const mapbuffer_lookup_options options) -> bool {
    return set_furn(
        p,
        {
            .furniture = furn,
...
auto mapbuffer::set_furn(const tripoint_abs_ms& p, const mapbuffer_set_furn_options& options)
    -> bool {
...
auto mapbuffer::veh_at(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> optional_vpart_position {
    const auto target_sm = project_to<coords::sm>(p);
    if (get_submap(target_sm, options) == nullptr) { return optional_vpart_position(std::nullopt); }
...
auto mapbuffer::valid_move(
    ...
    if (!options.zlevels) { return false; }
    ...
    if (!options.flying && !down_ter.has_flag(TFLAG_GOES_UP) && !down_ter.has_flag(TFLAG_RAMP)
        && !down_ter.has_flag(TFLAG_CLIMBABLE) && !up_is_ledge && !options.via_ramp) {
        return false;
    }

    if (options.bash) { return true; }

    const auto up_vehicle = veh_at(up_p, options.lookup);
    if (up_vehicle && !up_vehicle.part_with_feature(VPFLAG_NOCOLLIDEBELOW, false)) { return false; }
...
auto mapbuffer::climb_difficulty(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> std::optional<int> {
    if (p.z() > OVERMAP_HEIGHT || p.z() < -OVERMAP_DEPTH) { return std::nullopt; }

    const auto center_tile = abs_tile_handle::fetch_terrain_only(*this, p, options);
    if (!center_tile) { return std::nullopt; }
    ...
    if (has_flag(*center_tile, "LADDER")) { return 1; }
    if (has_flag(*center_tile, TFLAG_RAMP) || has_flag(*center_tile, TFLAG_RAMP_UP)
    ...
    for (const auto& tile : simulated_tiles_in_radius(*this, p, 1)) {
        ...
        if (best_difficulty > 5 && has_flag(tile, "CLIMBABLE")) { best_difficulty = 5; }
    }

    return std::max(0, best_difficulty - blocks_movement);
}

auto mapbuffer::floor_between(
    const tripoint_abs_ms& first, const tripoint_abs_ms& second,
    const mapbuffer_lookup_options options) -> bool {
    int diff = std::abs(first.z() - second.z());
    if (diff == 0) { return false; }
    if (diff != 1) {
        debugmsg(
            "mapbuffer::floor_between should only be called on tiles that are "
...
auto mapbuffer::clear_path(
    const tripoint_abs_ms& f, const tripoint_abs_ms& t, int range, int cost_min, int cost_max,
    const mapbuffer_lookup_options options) -> bool {
    if (f.z() == t.z()) {
        ...
        bresenham(
            f.xy().raw(), t.xy().raw(), 0,
            [this, &is_clear, cost_min, cost_max, &t, &last_point,
             options](const point& new_point) {
                // Exit before checking the last square, it's still reachable even if it is an
                // obstacle.
                if (new_point == t.xy().raw()) { return false; }
                ...
                if (cost < cost_min || cost > cost_max || obstructed_by_vehicle_rotation(lp, p)) {
                    is_clear = false;
                    return false;
                }
...
auto mapbuffer::find_clear_path(const tripoint_abs_ms& source, const tripoint_abs_ms& destination)
...
auto mapbuffer::get_lum(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
...
auto mapbuffer::get_temperature(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> std::optional<int> {
...
auto mapbuffer::get_field(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> field* {
    const auto tile = lookup_tile(*this, p, options);
...
auto mapbuffer::has_field_at(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> bool {
    const auto tile = lookup_tile(*this, p, options);
    return tile && tile->sm->field_count > 0;
}

auto mapbuffer::get_field_entry(
    const tripoint_abs_ms& p, const field_type_id& type, const mapbuffer_lookup_options options)
    -> field_entry* {
    if (!has_field_at(p, options)) { return nullptr; }

    return get_field(p, options)->find_field(type);
}

auto mapbuffer::get_field_age(
    const tripoint_abs_ms& p, const field_type_id& type, const mapbuffer_lookup_options options)
    -> std::optional<time_duration> {
    if (!get_field(p, options)) { return std::nullopt; }

    const auto* const field_ptr = get_field_entry(p, type, options);
    return field_ptr == nullptr ? -1_turns : field_ptr->get_field_age();
}

auto mapbuffer::get_field_intensity(
    const tripoint_abs_ms& p, const field_type_id& type, const mapbuffer_lookup_options options)
    -> std::optional<int> {
    if (!get_field(p, options)) { return std::nullopt; }
...
auto mapbuffer::passable(const tripoint_abs_ms& p, const mapbuffer_lookup_options options) -> bool {
    const auto tile = abs_tile_handle::fetch(*this, p, options);
    if (!tile) { return false; }
...
auto mapbuffer::ter_vars(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> data_vars::data_set* {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return nullptr; }
...
auto mapbuffer::furn_vars(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> data_vars::data_set* {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return nullptr; }
...
auto mapbuffer::get_trap(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> std::optional<trap_id> {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return std::nullopt; }
...
auto mapbuffer::get_radiation(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> std::optional<int> {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return std::nullopt; }
...
auto mapbuffer::set_trap(
    const tripoint_abs_ms& p, const trap_id trap, const mapbuffer_lookup_options options) -> bool {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return false; }
...
auto mapbuffer::remove_trap(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> bool {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return false; }

    const auto old_id = tile->sm->get_trap(tile->local);
    if (old_id == tr_null) { return false; }
...
auto mapbuffer::creature_on_trap(Creature& critter, const bool may_avoid) -> void {
    ...
    if (trap_here == tr_null) { trap_here = tile->trap_id(); }

    const auto& tr = trap_here.obj();
    if (tr.is_null()) { return; }
    const player* const pl = critter.as_player();
    if (pl != nullptr && pl->in_vehicle) { return; }

    if (may_avoid && critter.avoid_trap(pos, tr)) { return; }
    tr.trigger(pos, &critter);
}

auto mapbuffer::set_radiation(
    const tripoint_abs_ms& p, const int radiation, const mapbuffer_lookup_options options) -> bool {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return false; }
...
auto mapbuffer::adjust_radiation(
    const tripoint_abs_ms& p, const int delta, const mapbuffer_lookup_options options)
    -> std::optional<int> {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return std::nullopt; }
...
auto mapbuffer::set_lum(
    const tripoint_abs_ms& p, const std::uint8_t luminance, const mapbuffer_lookup_options options)
    -> bool {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return false; }

    const auto old_luminance = tile->sm->get_lum(tile->local);
    if (old_luminance == luminance) { return false; }

    tile->sm->set_lum(tile->local, luminance);
    refresh_luminous_item_submap_index(
        project_to<coords::sm>(p),
        {
            .mode = mapbuffer_lookup_mode::resident_only,
        });
    if (active_reality_bubble_local(p)) { g->m.invalidate_lightmap_caches(); }
    return true;
}

auto mapbuffer::set_temperature(
    const tripoint_abs_ms& p, const int temperature, const mapbuffer_lookup_options options)
    -> bool {
    if (is_outside_pocket_dimension_bounds(p)) { return false; }
...
auto mapbuffer::mod_field_age(const tripoint_abs_ms& p, const mapbuffer_field_age_options& options)
    -> std::optional<time_duration> {
...
auto mapbuffer::mod_field_intensity(
    const tripoint_abs_ms& p, const mapbuffer_field_intensity_options& options)
    -> std::optional<int> {
    auto set_options = options;
    set_options.isoffset = true;
...
auto mapbuffer::set_field_age(const tripoint_abs_ms& p, const mapbuffer_field_age_options& options)
    -> std::optional<time_duration> {
    if (!get_field(p, options.lookup)) { return std::nullopt; }

    auto* const field_ptr = get_field_entry(p, options.type, options.lookup);
    if (field_ptr == nullptr) { return -1_turns; }

    return field_ptr->set_field_age(
        (options.isoffset ? field_ptr->get_field_age() : 0_turns) + options.age);
}

auto mapbuffer::set_field_intensity(
    const tripoint_abs_ms& p, const mapbuffer_field_intensity_options& options)
    -> std::optional<int> {
    if (!get_field(p, options.lookup)) { return std::nullopt; }
    ...
    if (field_ptr != nullptr) {
        const auto adjusted =
            (options.isoffset ? field_ptr->get_field_intensity() : 0) + options.intensity;
        if (adjusted > 0) { return field_ptr->set_field_intensity(adjusted); }
        remove_field(p, options.type, options.lookup);
        return 0;
    }

    if (options.intensity <= 0) { return 0; }
...
auto mapbuffer::add_field(const tripoint_abs_ms& p, const mapbuffer_add_field_options& options)
    -> bool {
    if (!options.type) {
        debugmsg("Tried to add null field at %d,%d,%d", p.x(), p.y(), p.z());
        return false;
    }
...
auto mapbuffer::remove_field(
    const tripoint_abs_ms& p, const field_type_id& type, const mapbuffer_lookup_options options)
    -> bool {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return false; }

    if (!tile->sm->get_field(tile->local).remove_field(type)) { return false; }
...
auto mapbuffer::get_items(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> location_vector<item>* {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return nullptr; }
...
auto mapbuffer::add_item_or_charges(
    const tripoint_abs_ms& p, detached_ptr<item>&& new_item,
    const mapbuffer_add_item_or_charges_options& options) -> detached_ptr<item> {
    if (!new_item) { return std::move(new_item); }
    if (new_item->is_null()) {
        debugmsg("Tried to add a null item to the mapbuffer");
        return std::move(new_item);
    }
    if (new_item->has_flag(flag_NO_DROP)) { return std::move(new_item); }
    ...
    auto valid_tile = [&](const tripoint_abs_ms& target) -> std::optional<abs_tile_handle> {
        ...
        if (!tile) { return std::nullopt; }
        if (tile->has_flag(TFLAG_DESTROY_ITEM)) { return std::nullopt; }
        if (new_item->made_of(LIQUID) && tile->has_flag(TFLAG_SWIMMABLE)) { return std::nullopt; }
        return tile;
    };

    auto valid_limits = [&](const abs_tile_handle& tile) {
        const auto max_volume =
            tile.furn() != f_null ? tile.furn().obj().max_volume : tile.ter().obj().max_volume;
        auto stored_volume = 0_ml;
        const auto& items = tile.items();
        for (const auto* const existing : items) { stored_volume += existing->volume(); }
    ...
    auto route_allows_overflow = [&](const tripoint_abs_ms& target) {
        ...
        for (const auto& origin : simulated_tiles_in_radius(*this, p, 1)) {
            if (origin.abs_pos() == p || !origin.passable()) { continue; }
            if (origin.abs_pos() == target) { return true; }
            const auto abs_route =
                Pathfinding::route(*this, origin.abs_pos(), target, pf_settings, rt_settings);
            if (!abs_route.empty()) { return true; }
        }
        return false;
    };
    ...
    auto place_item = [&](const abs_tile_handle& tile) {
        ...
        if (new_item->count_by_charges()) {
            for (auto& existing : items) {
                ...
                if (existing->merge_charges(std::move(new_item))) { return; }
            }
        }
    ...
    auto try_place =
        [&](const tripoint_abs_ms& target, const bool reject_noitem,
    ...
    if (try_place(p, false, false)) { return std::move(new_item); }
    ...
    if (options.overflow) {
        ...
        for (const auto& candidate : tiles) {
            if (!route_allows_overflow(candidate)) { continue; }
            if (try_place(candidate, true, true)) { return std::move(new_item); }
        }
    }
...
auto mapbuffer::add_item(
    const tripoint_abs_ms& p, detached_ptr<item>&& new_item, const mapbuffer_lookup_options options)
    -> detached_ptr<item> {
    if (!new_item) { return std::move(new_item); }
    if (new_item->is_null()) {
        debugmsg("Tried to add a null item to the mapbuffer");
    ...
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return std::move(new_item); }
    ...
    if (new_item->made_of(LIQUID) && tile_has_flag(*tile, "SWIMMABLE")) { return reject(); }

    if (tile_has_flag(*tile, "DESTROY_ITEM")) { return reject(); }
    ...
    tile->sm->is_uniform = false;
    if (active_reality_bubble_local(p)) { g->m.invalidate_max_populated_zlev(p.z()); }
...
auto mapbuffer::process_item_at(
    const tripoint_abs_ms& p, detached_ptr<item>&& new_item, const bool activate)
    -> detached_ptr<item> {
    if (!new_item) { return std::move(new_item); }

    auto* items = get_items(p);
    if (items == nullptr) { return std::move(new_item); }
    ...
    if (new_item && new_item->saved_loc == item_location) { new_item->saved_loc = nullptr; }
    return std::move(new_item);
}

auto mapbuffer::erase_item(const tripoint_abs_ms& p, const mapbuffer_erase_item_options& options)
    -> location_vector<item>::iterator {
    const auto tile = lookup_tile(*this, p, options.lookup);
    if (!tile) { return location_vector<item>::iterator(); }
...
auto mapbuffer::remove_item(
    const tripoint_abs_ms& p, item* const to_remove, const mapbuffer_lookup_options options)
    -> detached_ptr<item> {
    if (to_remove == nullptr) { return detached_ptr<item>(); }

    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return detached_ptr<item>(); }

    auto& items = tile->sm->get_items(tile->local);
    const auto iter = std::ranges::find(items, to_remove);
    if (iter == items.end()) { return detached_ptr<item>(); }
...
auto mapbuffer::clear_items(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> std::vector<detached_ptr<item>> {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return {}; }

    auto& items = tile->sm->get_items(tile->local);
    for (item* const it : items) { tile->sm->active_items.remove(it); }
    sync_active_item_submap_index(p, *tile->sm);
...
auto mapbuffer::handle_rotten_away_item(
    const tripoint_abs_ms& p, const item& rotten_item, const mapbuffer_lookup_options options)
    -> void {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return; }
...
auto mapbuffer::make_item_active(
    const tripoint_abs_ms& p, item& target, const mapbuffer_lookup_options options) -> bool {
    if (!target.needs_processing()) { return false; }
...
auto mapbuffer::make_item_inactive(
    const tripoint_abs_ms& p, item& target, const mapbuffer_lookup_options options) -> bool {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return false; }
...
auto mapbuffer::update_item_lum(
    const tripoint_abs_ms& p, item& target, const mapbuffer_item_lum_options& options) -> bool {
    if (!target.is_emissive()) { return false; }
...
auto mapbuffer::refresh_active_item_submap_index(
    const tripoint_abs_ms& p, const mapbuffer_lookup_options options) -> bool {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return false; }
...
auto mapbuffer::refresh_active_item_submap_index(
    const tripoint_abs_sm& p, const mapbuffer_lookup_options options) -> bool {
...
auto mapbuffer::clear_active_item_submap_index() -> void { submaps_with_active_items_.clear(); }

auto mapbuffer::get_submaps_with_active_items() const -> const std::set<tripoint_abs_sm>& {
    return submaps_with_active_items_;
}

auto mapbuffer::refresh_luminous_item_submap_index(
    const tripoint_abs_ms& p, const mapbuffer_lookup_options options) -> bool {
...
auto mapbuffer::get_active_items_in_radius(
    const tripoint_abs_ms& center, const int radius, const special_item_type type)
    -> std::vector<item*> {
    ...
    for (const tripoint_abs_sm& abs_submap_loc : submaps_with_active_items_) {
        if (!submap_loader.is_simulated(dimension_id_, abs_submap_loc)) { continue; }

        const auto sm_origin = project_to<coords::ms>(abs_submap_loc);
        const auto sm_max = sm_origin.xy() + point_rel_ms(SEEX - 1, SEEY - 1);
        if (sm_origin.x() > maxp.x() || sm_origin.y() > maxp.y() || sm_max.x() < minp.x()
            || sm_max.y() < minp.y()) {
            continue;
        }
        ...
        for (item* elem : items) {
            ...
            if (rl_dist(elem->abs_pos(), center) > radius) { continue; }
...
auto mapbuffer::has_graffiti_at(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> bool {
    const auto tile = lookup_tile(*this, p, options);
    return tile && tile->sm->has_graffiti(tile->local);
}

auto mapbuffer::graffiti_at(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
...
auto mapbuffer::set_graffiti(
    const tripoint_abs_ms& p, const std::string& contents, const mapbuffer_lookup_options options)
    -> bool {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return false; }
...
auto mapbuffer::delete_graffiti(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> bool {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return false; }
...
auto mapbuffer::has_signage(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> bool {
    const auto tile = lookup_tile(*this, p, options);
    return tile && tile->sm->has_signage(tile->local);
}

auto mapbuffer::get_signage(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> std::optional<std::string> {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return std::nullopt; }
...
auto mapbuffer::set_signage(
    const tripoint_abs_ms& p, const std::string& message, const mapbuffer_lookup_options options)
    -> bool {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return false; }
...
auto mapbuffer::delete_signage(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> bool {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return false; }
...
auto mapbuffer::has_computer(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> bool {
    const auto tile = lookup_tile(*this, p, options);
    return tile && tile->sm->has_computer(tile->local);
}

auto mapbuffer::get_computer(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> computer* {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return nullptr; }
...
auto mapbuffer::set_computer(
    const tripoint_abs_ms& p, const computer& terminal, const mapbuffer_lookup_options options)
    -> bool {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return false; }
...
auto mapbuffer::add_computer(
    const tripoint_abs_ms& p, const mapbuffer_add_computer_options& options) -> computer* {
    const auto tile = lookup_tile(*this, p, options.lookup);
    if (!tile) { return nullptr; }
...
auto mapbuffer::delete_computer(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> bool {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return false; }
...
void mapbuffer::add_spawn(
    const mtype_id& type, int count, const tripoint_abs_ms& p, bool friendly, int faction_id,
    int mission_id, const std::string& name, mapbuffer_lookup_options options) const {
    add_spawn(type, count, p, spawn_point::friendly_to_spawn_disposition(friendly), faction_id,
              mission_id, name);
}

void mapbuffer::add_spawn(
    const mtype_id& type, int count, const tripoint_abs_ms& p, spawn_disposition disposition,
...
vehicle* mapbuffer::add_vehicle(
    const std::variant<vgroup_id, vproto_id>& type_, const tripoint_abs_ms& p,
    const units::angle dir, const int veh_fuel, const int veh_status, const bool merge_wrecks,
    std::optional<bool> locked, std::optional<bool> has_keys, mapbuffer_lookup_options options) {
    ...
    if (!place_on_submap) {
        debugmsg("add_vehicle triggered for nonexistent submap t=%s d=%d p=%s", type,
                 to_degrees(dir), p.to_string());
        return nullptr;
    }
...
std::unique_ptr<vehicle> mapbuffer::add_vehicle_to_mapbuffer(
    std::unique_ptr<vehicle> veh, const bool merge_wrecks, mapbuffer_lookup_options options) {
    ...
    for (std::vector<int>::const_iterator part = frame_indices.begin(); part != frame_indices.end();
        ...
        if (has_flag(TFLAG_DEEP_WATER, abs_pos, options) && !can_float) { return nullptr; }

        // Don't spawn shopping carts on top of another vehicle or other obstacle.
        if (veh->type == vproto_id("shopping_cart")) {
            if (veh_at(abs_pos, options) || !passable(abs_pos, options)) { return nullptr; }
        }
        ...
        if (other_veh != nullptr && other_veh->type != vproto_id("shopping_cart")) {
            ...
            std::unique_ptr<vehicle> new_veh =
                add_vehicle_to_mapbuffer(std::move(veh), true, options);
            if (new_veh != nullptr) {
                new_veh->smash();
                return new_veh;
            }
            ...
            if (!passable(abs_pos, options)) { return nullptr; }

            needs_smashing = true;
        }
    }

    if (needs_smashing) { veh->smash(); }

    return veh;
}

std::set<vehicle*> mapbuffer::get_vehicles(
    const tripoint_abs_sm& start, const tripoint_abs_sm& end, mapbuffer_lookup_options options) {
    auto vehs = std::set<vehicle*>{};

    if (start.x() > end.x() || start.y() > end.y() || start.z() > end.z()) { return vehs; }
    for (const auto sm_pos : tripoint_range<tripoint_abs_sm>(start, end)) {
        ...
        for (const auto& elem : current_submap->vehicles) { vehs.emplace(elem.get()); }
    }
...
std::unique_ptr<vehicle> mapbuffer::detach_vehicle(vehicle* veh, mapbuffer_lookup_options options) {
    if (veh == nullptr) {
    ...
    if (z < -OVERMAP_DEPTH || z > OVERMAP_HEIGHT) {
        debugmsg(
            "detach_vehicle got a vehicle outside allowed z-level range!  name=%s, submap:%d,%d,%d",
    ...
    for (auto const& part : veh->get_avail_parts(VPFLAG_BOARDABLE)) {
        ...
        if (passenger) { unboard_vehicle(part.abs_pos()); }
    }
...
void mapbuffer::destroy_vehicle(vehicle* veh, mapbuffer_lookup_options options) {
    detach_vehicle(veh);
}

auto mapbuffer::partial_con_at(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> partial_con* {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return nullptr; }

    const auto iter = tile->sm->partial_constructions.find(tripoint_sm_ms(tile->local, p.z()));
    if (iter == tile->sm->partial_constructions.end()) { return nullptr; }
    return iter->second.get();
}

auto mapbuffer::partial_con_set(
    const tripoint_abs_ms& p, std::unique_ptr<partial_con> con,
    const mapbuffer_lookup_options options) -> bool {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return false; }

    const auto inserted =
        tile->sm->partial_constructions.emplace(tripoint_sm_ms(tile->local, p.z()), std::move(con))
            .second;
    if (!inserted) {
...
auto mapbuffer::partial_con_remove(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> bool {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return false; }
...
auto mapbuffer::is_bashable(
    const tripoint_abs_ms& p, const bool allow_floor, const mapbuffer_lookup_options options)
    -> bool {
    if (veh_at(p, options).obstacle_at_part()) { return true; }
    ...
    if (furn != f_null && furn.obj().bash.str_max != -1) { return true; }
...
auto mapbuffer::is_bashable_ter(
    const tripoint_abs_ms& p, const bool allow_floor, const mapbuffer_lookup_options options)
    -> bool {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return false; }
    ...
    return ter_bash.str_max != -1
        && ((!ter_bash.bash_below
             && !tile->sm->get_ter(tile->local).obj().has_flag("VEH_TREAT_AS_BASH_BELOW"))
            || allow_floor);
}

auto mapbuffer::is_bashable_furn(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> bool {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return false; }
...
auto mapbuffer::bash_strength(
    ...
    const auto furn = tile->sm->get_furn(tile->local);
    if (furn != f_null && furn.obj().bash.str_max != -1) { return furn.obj().bash.str_max; }
...
auto mapbuffer::bash_resistance(
    const tripoint_abs_ms& p, const bool allow_floor, const mapbuffer_lookup_options options)
    -> int {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return -1; }
    ...
    const auto furn = tile->sm->get_furn(tile->local);
    if (furn != f_null && furn.obj().bash.str_min != -1) { return furn.obj().bash.str_min; }
...
auto mapbuffer::bash_rating(
    const int str, const tripoint_abs_ms& p, const bool allow_floor,
    ...
    if (veh != nullptr && vp && vp->obstacle_at_part()) { return 2; }
...
auto mapbuffer::is_divable(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> bool {
...
auto mapbuffer::is_water_shallow_current(
    const tripoint_abs_ms& p, const mapbuffer_lookup_options options) -> bool {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return false; }
...
auto mapbuffer::has_items(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> bool {
    const auto tile = lookup_tile(*this, p, options);
...
auto mapbuffer::has_nearby_fire(
    const tripoint_abs_ms& p, const int radius, const mapbuffer_lookup_options options) -> bool {
    for (const tripoint_abs_ms& pt : points_in_radius(p, radius)) {
        const auto field_entry = get_field_entry(pt, fd_fire, options);
        if (field_entry != nullptr) { return true; }
        {
            auto h = abs_tile_handle::fetch_terrain_only(*this, pt, options);
            if (h && h->has_flag_ter_or_furn("USABLE_FIRE")) { return true; }
        }
    }
    return false;
}

auto mapbuffer::has_nearby_table(
    const tripoint_abs_ms& p, const int radius, const mapbuffer_lookup_options options) -> bool {
    for (const tripoint_abs_ms& pt : points_in_radius(p, radius)) {
        const auto vp = veh_at(pt, options);
        {
            auto h = abs_tile_handle::fetch_terrain_only(*this, pt, options);
            if (h && h->has_flag_ter_or_furn("FLAT_SURF")) { return true; }
        }
...
auto mapbuffer::has_nearby_chair(
    const tripoint_abs_ms& p, const int radius, const mapbuffer_lookup_options options) -> bool {
    for (const tripoint_abs_ms& pt : points_in_radius(p, radius)) {
        const auto vp = veh_at(pt, options);
        {
            auto h = abs_tile_handle::fetch_terrain_only(*this, pt, options);
            if (h && h->has_flag_ter_or_furn("CAN_SIT")) { return true; }
        }
...
auto mapbuffer::can_put_items(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> bool {
    if (can_put_items_ter_furn(p, options)) { return true; }
    const auto vp = veh_at(p, options);
    return static_cast<bool>(vp.part_with_feature("CARGO", true));
}

auto mapbuffer::can_put_items_ter_furn(
    const tripoint_abs_ms& p, const mapbuffer_lookup_options options) -> bool {
    auto h = abs_tile_handle::fetch_terrain_only(*this, p, options);
    return h && !h->has_flag("NOITEM") && !h->has_flag("SEALED");
}

auto mapbuffer::dangerous_field_at(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> bool {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return false; }
...
auto mapbuffer::is_harvestable(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> bool {
    const auto& harvest_here = get_harvest(p, options);
    return !harvest_here.is_null() && !harvest_here->empty();
}

auto mapbuffer::accessible_items(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> bool {
    auto h = abs_tile_handle::fetch_terrain_only(*this, p, options);
    return h && (!h->has_flag("SEALED") || h->has_flag("LIQUIDCONT"));
}

auto mapbuffer::is_wall_adjacent(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> bool {
    for (const tripoint_abs_ms& pt : points_in_radius(p, 1)) {
        if (pt != p) {
            auto h = abs_tile_handle::fetch(*this, pt, options);
            if (h && h->move_cost() == 0) { return true; }
        }
    }
    return false;
}

auto mapbuffer::is_flammable(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> bool {
    if (flammable_items_at(p, 0, options)) { return true; }
    {
        auto h = abs_tile_handle::fetch_terrain_only(*this, p, options);
        if (h && (h->has_flag("FLAMMABLE") || h->has_flag("FLAMMABLE_ASH"))) { return true; }
    }
    if (get_field_intensity(p, fd_web, options).value_or(0) > 0) { return true; }
    return false;
}

auto mapbuffer::tinder_at(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> bool {
    auto* items = get_items(p, options);
...
auto mapbuffer::flammable_items_at(
    const tripoint_abs_ms& p, const int threshold, const mapbuffer_lookup_options options) -> bool {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile || tile->sm->get_items(tile->local).empty()) { return false; }
...
auto mapbuffer::get_harvest(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> const harvest_id& {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return harvest_id::NULL_ID(); }

    const auto furn_here = tile->sm->get_furn(tile->local);
    if (furn_here.obj().examine != iexamine::none) {
        if (furn_here.obj().has_flag(TFLAG_HARVESTED)) { return harvest_id::NULL_ID(); }
        return furn_here.obj().get_harvest();
    }
    const auto ter_here = tile->sm->get_ter(tile->local);
    if (ter_here.obj().has_flag(TFLAG_HARVESTED)) { return harvest_id::NULL_ID(); }
    return ter_here.obj().get_harvest();
}

auto mapbuffer::get_harvest_names(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> const std::set<std::string>& {
    const auto& harvest_here = get_harvest(p, options);
    static const std::set<std::string> empty_set;
    if (harvest_here.is_null()) { return empty_set; }
    return harvest_here->names();
}

auto mapbuffer::get_ter_transforms_into(
    const tripoint_abs_ms& p, const mapbuffer_lookup_options options) -> ter_id {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return t_null; }
    return tile->sm->get_ter(tile->local).obj().transforms_into.id();
}

auto mapbuffer::get_furn_transforms_into(
    const tripoint_abs_ms& p, const mapbuffer_lookup_options options) -> furn_id {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return f_null; }
    return tile->sm->get_furn(tile->local).obj().transforms_into.id();
}

auto mapbuffer::tername(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> std::string {
    const auto tile = lookup_tile(*this, p, options);
...
auto mapbuffer::name(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> std::string {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return std::string(); }

    const auto furn_here = tile->sm->get_furn(tile->local);
    if (furn_here != f_null) { return furn_here.obj().name(); }

    const auto vp = veh_at(p, options);
    if (vp) {
        const auto displayed = vp->part_displayed();
        if (displayed) { return displayed->info().name(); }
    }

    return tile->sm->get_ter(tile->local).obj().name();
}

auto mapbuffer::disp_name(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> std::string {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return std::string(); }

    const auto furn_here = tile->sm->get_furn(tile->local);
    if (furn_here != f_null) { return string_format(_("the %s"), furn_here.obj().name()); }

    const auto vp = veh_at(p, options);
    if (vp) {
        const auto displayed = vp->part_displayed();
        if (displayed) { return string_format(_("the %s"), displayed->info().name()); }
    }

    return string_format(_("the %s"), tile->sm->get_ter(tile->local).obj().name());
}

auto mapbuffer::obstacle_name(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> std::string {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return std::string(); }
...
auto mapbuffer::sees(
    const tripoint_abs_ms& F, const tripoint_abs_ms& T, const int range,
 ...
 **/
auto mapbuffer::sees(
    const tripoint_abs_ms& F, const tripoint_abs_ms& T, const int range, int& bresenham_slope,
    mapbuffer_lookup_options options) -> bool {
    if ((range >= 0 && range < rl_dist(F, T))) {
        bresenham_slope = 0;
        return false; // Out of range!
                  ...
                  [this, &visible, &T, &last_point, options](const point& new_point) {
                      ...
                      if (!is_transparent(new_tripoint, options)
                          || obstructed_by_vehicle_rotation(
                              tripoint_abs_ms(last_point, T.z()), new_tripoint)) {
                          visible = false;
                          return false;
                      }
        ...
        [this, &visible, &T, &last_point, options](const tripoint& new_point) {
            ...
            if (new_point.z == last_point.z()) {
                ...
                if ((has_floor(tripoint_abs_ms{new_point.x, new_point.y, max_z}, true, options)
                     || !is_transparent(
                         tripoint_abs_ms{new_point.x, new_point.y, last_point.z()}, options))
...
auto mapbuffer::obstacle_coverage(
    const tripoint_abs_ms& loc1, const tripoint_abs_ms& loc2,
    const mapbuffer_lookup_options options) -> int {
    const auto tile1 = lookup_tile(*this, loc1, options);
    const auto tile2 = lookup_tile(*this, loc2, options);
    if (!tile1 || !tile2) { return 100; }
    // Can't hide if you are standing on furniture, or non-flat slowing-down terrain tile.
    if (tile1->sm->get_furn(tile1->local).obj().id
        || (move_cost(loc2, nullptr, options) > 2
    ...
    if (!obst_tile) { return 100; }
    const auto obstacle_f = obst_tile->sm->get_furn(obst_tile->local);
    if (obstacle_f != f_null) { return obstacle_f->coverage; }
...
auto mapbuffer::features(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> std::string {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return std::string(); }
    ...
    if (ter.has_flag("BARRICADE")) { feats += _("barricaded") + std::string(" "); }
    if (ter.has_flag("BASHED")) { feats += _("bashed") + std::string(" "); }
    if (ter.has_flag("ROUGH")) { feats += _("rough") + std::string(" "); }
    if (ter.has_flag("SHARP")) { feats += _("sharp") + std::string(" "); }
    if (ter.has_flag("SHORT")) { feats += _("short") + std::string(" "); }
    if (ter.has_flag("RAMP_UP")) { feats += _("ramp up") + std::string(" "); }
    if (ter.has_flag("RAMP_DOWN")) { feats += _("ramp down") + std::string(" "); }

    // Gather from furniture
    if (furn_here != f_null) {
        const auto& furn = furn_here.obj();
        if (furn.has_flag("BARRICADE")) { feats += _("barricaded") + std::string(" "); }
        if (furn.has_flag("BASHED")) { feats += _("bashed") + std::string(" "); }
        if (furn.has_flag("ROUGH")) { feats += _("rough") + std::string(" "); }
        if (furn.has_flag("SHARP")) { feats += _("sharp") + std::string(" "); }
        if (furn.has_flag("SHORT")) { feats += _("short") + std::string(" "); }
    }
...
auto mapbuffer::ranged_target_size(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> double {
    auto h = abs_tile_handle::fetch(*this, p, options);
    if (h && h->move_cost() == 0) { return 1.0; }
    // No floor check in mapbuffer — return 0 for open air-like terrains
    const auto tile = lookup_tile(*this, p, options);
    if (tile && tile->sm->get_ter(tile->local).obj().has_flag(TFLAG_NO_FLOOR)) { return 0.0; }
    // TODO: Size based on furniture/terrain cover
    return 0.1;
}

auto mapbuffer::max_volume(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> units::volume {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return 0_ml; }

    const auto furn_here = tile->sm->get_furn(tile->local);
    if (furn_here != f_null) { return furn_here.obj().max_volume; }
    return tile->sm->get_ter(tile->local).obj().max_volume;
}

auto mapbuffer::free_volume(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> units::volume {
    return max_volume(p, options) - stored_volume(p, options);
}

auto mapbuffer::stored_volume(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> units::volume {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return 0_ml; }
    units::volume vol = 0_ml;
    for (const auto& i : tile->sm->get_items(tile->local)) { vol += (*i).volume(); }
    return vol;
}

// ----- Item search -----

auto mapbuffer::has_item_with(
    const tripoint_abs_ms& p, const std::function<bool(const item&)>& filter,
    const mapbuffer_lookup_options options) -> bool {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return false; }
...
auto mapbuffer::has_adjacent_item_with(
    const tripoint_abs_ms& p, const std::function<bool(const item&)>& filter,
    const mapbuffer_lookup_options options) -> bool {
...
auto mapbuffer::has_adjacent_furniture_with(
    const tripoint_abs_ms& p, const std::function<bool(const furn_t&)>& filter,
    const mapbuffer_lookup_options options) -> bool {
    for (const tripoint_abs_ms& adj : points_in_radius(p, 1)) {
        if (adj == p) { continue; }
        ...
        if (!tile) { continue; }
        const auto furn_here = tile->sm->get_furn(tile->local);
        if (furn_here != f_null && filter(furn_here.obj())) { return true; }
    }
    return false;
}

auto mapbuffer::has_adjacent_terrain_with(
    const tripoint_abs_ms& p, const std::function<bool(const ter_t&)>& filter,
    const mapbuffer_lookup_options options) -> bool {
    for (const tripoint_abs_ms& adj : points_in_radius(p, 1)) {
        if (adj == p) { continue; }
        const auto tile = lookup_tile(*this, adj, options);
        if (!tile) { continue; }
        if (filter(tile->sm->get_ter(tile->local).obj())) { return true; }
    }
    return false;
}

auto mapbuffer::sees_some_items(
    const tripoint_abs_ms& p, const tripoint_abs_ms& from, const mapbuffer_lookup_options options)
    -> bool {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile || tile->sm->get_items(tile->local).empty()) { return false; }

    // SEALED -> never visible
    if (tile->sm->get_ter(tile->local).obj().has_flag(TFLAG_SEALED)) { return false; }

    const auto furn_here = tile->sm->get_furn(tile->local);
    if (furn_here != f_null) {
        if (furn_here.obj().has_flag("SEALED")) { return false; }
        // CONTAINER -> only visible when adjacent or at same tile
        if (furn_here.obj().has_flag("CONTAINER")) { return square_dist(p.xy(), from.xy()) <= 1; }
    }

    return true;
}

auto mapbuffer::could_see_items(
    const tripoint_abs_ms& p, const tripoint_abs_ms& from, const mapbuffer_lookup_options options)
    -> bool {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return false; }
...
auto mapbuffer::move_cost_ter_furn(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> int {
    const auto tile = lookup_tile(*this, p, options);
    if (!tile) { return 2; }

    const auto& terrain = tile->sm->get_ter(tile->local).obj();
    if (terrain.movecost == 0) { return 0; }

    const auto furn_here = tile->sm->get_furn(tile->local);
    if (furn_here != f_null && furn_here.obj().movecost < 0) { return 0; }
...
auto mapbuffer::impassable_ter_furn(
    const tripoint_abs_ms& p, const mapbuffer_lookup_options options) -> bool {
    return move_cost_ter_furn(p, options) == 0;
}

auto mapbuffer::passable_ter_furn(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> bool {
    return move_cost_ter_furn(p, options) > 0;
}
...
auto mapbuffer::ter(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> std::optional<ter_id> {
...
auto mapbuffer::furn(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> std::optional<furn_id> {
    const auto tile = abs_tile_handle::fetch_terrain_only(*this, p, options);
    if (!tile) { return std::nullopt; }
    return tile->furn();
}

auto mapbuffer::furnname(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> std::string {
    const auto tile = abs_tile_handle::fetch_terrain_only(*this, p, options);
    if (!tile) { return std::string(); }
...
auto mapbuffer::has_flag(
    const std::string& flag, const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> bool {
    const auto tile = abs_tile_handle::fetch_terrain_only(*this, p, options);
    if (!tile) { return false; }
...
auto mapbuffer::has_flag(
    ter_bitflags flag, const tripoint_abs_ms& p, const mapbuffer_lookup_options options) -> bool {
    const auto tile = abs_tile_handle::fetch_terrain_only(*this, p, options);
    if (!tile) { return false; }
    return tile->has_flag(flag);
}

auto mapbuffer::has_flag_ter(
    const std::string& flag, const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> bool {
    const auto tile = abs_tile_handle::fetch_terrain_only(*this, p, options);
    if (!tile) { return false; }
...
auto mapbuffer::has_flag_ter_or_furn(
    const std::string& flag, const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> bool {
    const auto tile = abs_tile_handle::fetch_terrain_only(*this, p, options);
    if (!tile) { return false; }
...
auto mapbuffer::has_floor_or_support(
    const tripoint_abs_ms& p, const mapbuffer_lookup_options options) -> bool {
    const auto tile = abs_tile_handle::fetch_terrain_only(*this, p, options);
    if (!tile) { return false; }
    if (!tile->ter_obj().has_flag(TFLAG_NO_FLOOR)) { return true; }
    if (p.z() <= -OVERMAP_DEPTH) { return false; }

    const auto below = p + tripoint_rel_ms::below();
    const auto below_tile = abs_tile_handle::fetch_terrain_only(*this, below, options);
    if (!below_tile) { return false; }
    if (below_tile->ter_obj().movecost == 0) { return true; }
    if (below_tile->furn_obj().movecost < 0) { return true; }
    return veh_at(below, options).has_value();
}

auto mapbuffer::has_floor(
    const tripoint_abs_ms& p, bool visible_only, const mapbuffer_lookup_options options) -> bool {
...
auto mapbuffer::is_transparent(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> bool {
    const auto tile = abs_tile_handle::fetch_terrain_only(*this, p, options);
    if (!tile) { return false; }
    // Transparency determined by terrain/furniture opacity
    return tile->ter_obj().transparent && tile->furn_obj().transparent;
}

auto mapbuffer::is_outside(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> bool {
    if (const auto local = active_reality_bubble_local(p)) { return g->m.is_outside(*local); }
    const auto sm_pos = project_to<coords::sm>(p);
    const auto sm = get_submap(sm_pos, options);
    if (!sm) { return false; }
    ensure_roof_above_cache(*this, *sm, options);
...
auto mapbuffer::combined_movecost(
    const tripoint_abs_ms& from, const tripoint_abs_ms& to, const vehicle* ignored_vehicle,
    ...
    if (flying || from.z() == to.z()) { return (cost1 + cost2) * mults[match] / 2; }
...
auto mapbuffer::move_cost(
    const tripoint_abs_ms& p, const vehicle* ignored_vehicle,
    const mapbuffer_lookup_options options) -> int {
    const auto tile = abs_tile_handle::fetch(*this, p, options);
    if (!tile) { return 0; }
...
auto mapbuffer::hit_with_acid(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> bool {
    const auto tile = abs_tile_handle::fetch(*this, p, options);
    if (!tile) { return false; }
    if (tile->passable()) {
    ...
    if (t == t_wall_glass || t == t_wall_glass_alarm || t == t_vat) {
        ...
        if (one_in(3)) { set_ter(p, t_door_b, options); }
...
auto mapbuffer::hit_with_fire(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> bool {
    const auto tile = abs_tile_handle::fetch(*this, p, options);
    if (!tile) { return false; }
    if (tile->passable()) {
...
auto mapbuffer::can_open_door(
    const tripoint_abs_ms& p, bool inside, const mapbuffer_lookup_options options) -> bool {
    const auto tile = abs_tile_handle::fetch_terrain_only(*this, p, options);
    if (!tile) { return false; }
    const auto& ter = tile->ter_obj();
    if (ter.open) { return !has_flag(str_OPENCLOSE_INSIDE, p, options) || inside; }
    const auto& furn = tile->furn_obj();
    if (furn.open) { return !has_flag(str_OPENCLOSE_INSIDE, p, options) || inside; }
    const auto vp = veh_at(p, options);
    if (vp) {
...
auto mapbuffer::open_door(const tripoint_abs_ms& p, const mapbuffer_open_door_options& options)
    -> bool {
    ...
    const auto& ter = tile->ter_obj();
    if (ter.open) {
        if (has_flag(str_OPENCLOSE_INSIDE, p, options.lookup) && !options.inside) { return false; }
        if (!set_ter(p, ter.open, options.lookup)) { return false; }
        sound_event se;
        se.origin = p;
        se.volume = 50;
        se.category = sounds::sound_t::movement;
    ...
    if (furn.open) {
        if (has_flag(str_OPENCLOSE_INSIDE, p, options.lookup) && !options.inside) { return false; }
        if (!set_furn(p, furn.open, options.lookup)) { return false; }
        sound_event se;
        se.origin = p;
        se.volume = 50;
        se.category = sounds::sound_t::movement;
    ...
    if (options.who == nullptr && vp->vehicle().is_locked) { return false; }
    ...
    if (has_locked_door && (!is_owner || vp->vehicle().is_locked)) { return false; }

    vp->vehicle().open_all_at(openable);
    return true;
}

auto mapbuffer::close_door(
    const tripoint_abs_ms& p, bool inside, bool check_only, const mapbuffer_lookup_options options)
    -> bool {
    const auto tile = abs_tile_handle::fetch_terrain_only(*this, p, options);
    if (!tile) { return false; }

    if (has_flag(str_OPENCLOSE_INSIDE, p, options) && !inside) { return false; }
...
auto mapbuffer::forced_door_closing(
    const tripoint_abs_ms& p, const ter_id& door_type, int bash_dmg,
    const mapbuffer_lookup_options options) -> bool {
    const auto valid_location = [&](const tripoint_abs_ms& p) {
        return (passable(p, options) || has_flag("LIQUID", p, options))
    ...
    if (monster* const mon_ptr = creature ? creature->as_monster() : nullptr) {
        ...
        if (!critter.is_dead() && critter.type->size >= creature_size::huge) { return false; }
        if (!critter.is_dead()) {
            if (cannot_push) { return false; }
            g->knockback(kbp, p, std::max(1, bash_dmg / 10), -1, 1, nullptr);
            if (g->critter_at(p)) { return false; }
        }
    }
    ...
    if (const optional_vpart_position vp = veh_at(p, options)) {
        ...
        if (veh_at(p, options)) { return false; }
    }

    // Item checks — use mapbuffer
    if (bash_dmg < 0 && !has_items(p, options)) { return false; }
    if (bash_dmg == 0) {
        auto* items = get_items(p, options);
        if (items) {
...
ter_id mapbuffer::get_roof(
    const tripoint_abs_ms& p, const bool allow_air, mapbuffer_lookup_options options) {
    if (p.z() <= -OVERMAP_DEPTH) {
...
auto mapbuffer::place_items(
    const item_group_id& loc, int chance, const tripoint_abs_ms& p1, const tripoint_abs_ms& p2,
    bool ongrass, const time_point& turn, int magazine, int ammo,
...
static bool furn_is_supported(mapbuffer& m, const tripoint_abs_ms& p) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (furn && furn->obj().has_flag("BLOCKSDOOR")) { return true; }
    }
...
static void set_bash_sound_source(sound_event& se, const bash_params& params) {
    if (!params.caused_by_player) { return; }
...
static auto release_avatar_grabbed_furniture_if_destroyed(
    const tripoint_abs_ms& p, const furn_t& old_furniture, const furn_id& new_furniture) -> void {
    avatar& you = get_avatar();
    if (you.get_grab_type() == OBJECT_FURNITURE && you.abs_pos() + you.grab_point == p
...
bash_results mapbuffer::bash_ter_success(
    const tripoint_abs_ms& p, const bash_params& params, mapbuffer_lookup_options options) {
    bash_results result;
    const auto maybe_ter = ter(p, options);
    if (!maybe_ter) { return result; }
    result.success = true;
    const ter_t& ter_before = maybe_ter->obj();
    const map_bash_info& bash = ter_before.bash;
    if (has_flag_ter("FUNGUS", p, options)) { fungal_effects(*g, *this).create_spores(p); }
    ...
    if (!params.bashing_from_above && (follow_below || ter(p, options) == t_open_air)) {
        ...
        if (follow_below && params.do_recurse) {
            ...
            do {
                ...
                if (encountered_types.contains(ter_now)) {
                    ...
                    do {
                        ...
                        ter(below, options)->obj().movecost == 0 && impassable_bash_tries-- > 0);
                    if (impassable_bash_tries <= 0) {
                        debugmsg("Loop in terrain bashing for type %s", ter_before.id.str());
                    }
                } else if (ter_now == t_open_air) {
                    const ter_id& roof = get_roof(
                        below, params.bash_floor && ter(below, options)->obj().movecost != 0,
                        options);
                    if (roof != t_open_air) { set_ter(p, roof, options); }
                } else {
            ...
            const ter_id& roof = get_roof(
                below, params.bash_floor && ter(below, options)->obj().movecost != 0, options);
...
bash_results mapbuffer::bash_furn_success(
    const tripoint_abs_ms& p, const bash_params& params, mapbuffer_lookup_options options) {
    bash_results result;
    const auto tile = lookup_tile(*this, p, options);
    ...
    if (has_flag(TFLAG_FUNGUS, p, options)) { fungal_effects(*g, *this).create_spores(p); }
    if (has_flag("MIGO_NERVE", p, options)) { map_funcs::migo_nerve_cage_removal(*this, p, true); }
    ...
    if (tent) {
            ...
            for (const auto& pt : simulated_tiles_in_radius(*this, tentp->first, rad)) {
                ...
                for (const auto& cur_id : recur_bash.tent_centers) {
                    if (centers.contains(cur_id.id())) {
                        ...
                        spawn_items(
                            p, item_group::items_from(recur_bash.drop_group, calendar::turn),
                            options);
                        release_avatar_grabbed_furniture_if_destroyed(
                            pt.abs_pos(), furn_obj, recur_bash.furn_set);
        ...
        release_avatar_grabbed_furniture_if_destroyed(p, furnid, bash.furn_set);
        set_furn(p, bash.furn_set, options);
        for (auto it : *get_items(p, options)) { it->on_drop(); }
        // HACK: Hack alert.
    ...
    if (!tent) { spawn_items(p, item_group::items_from(bash.drop_group, calendar::turn), options); }
...
bash_results mapbuffer::bash_ter_furn(
    const tripoint_abs_ms& p, const bash_params& params, mapbuffer_lookup_options options) {
    bash_results result;
    ...
    if (!result.success) {
        ...
        if (!smash_ter && smax > 0) {
            ...
            if (flipped_version != furn_str_id::NULL_ID()) {
                ...
                if (rng(1, 100) <= damage_percent) { set_furn(p, flipped_version, options); }
            }
        }
        ...
        if (params.strength >= smin / 2 && one_in(smin / 2)) {
            ...
            if (has_flag(TFLAG_UNSTABLE, above, options) && character != nullptr) {
                character->add_msg_if_player(
                    m_warning, _("You feel the ground beneath you shake from the impact!"));
...
bash_results mapbuffer::bash(
    const tripoint_abs_ms& p, const int str, bool silent, bool destroy, bool bash_floor,
    const vehicle* bashing_vehicle, mapbuffer_lookup_options options) {
...
bash_results mapbuffer::bash(
    const tripoint_abs_ms& p, const bash_params& bsh, const vehicle* bashing_vehicle,
    ...
    if (!bashed_sealed) { result |= bash_items(p, bsh, options); }
    // Don't bash the vehicle doing the bashing
    const vehicle* veh = veh_pointer_or_null(veh_at(p, options));
    if (veh != nullptr && veh != bashing_vehicle) { result |= bash_vehicle(p, bsh, options); }

    // If we still didn't bash anything solid (a vehicle) or a tile with SEALED flag, bash ter/furn
    if (!result.bashed_solid && !bashed_sealed) { result |= bash_ter_furn(p, bsh, options); }

    return result;
}

bash_results mapbuffer::bash_items(
    const tripoint_abs_ms& p, const bash_params& params, mapbuffer_lookup_options options) {
    bash_results result;
    if (!has_items(p, options)) { return result; }
...
bash_results mapbuffer::bash_vehicle(
    const tripoint_abs_ms& p, const bash_params& params, mapbuffer_lookup_options options) {
    bash_results result;
...
bash_results mapbuffer::bash_field(
    const tripoint_abs_ms& p, const bash_params&, mapbuffer_lookup_options options) {
    bash_results result;
...
auto mapbuffer::destroy(
    const tripoint_abs_ms& p, bool silent, const mapbuffer_lookup_options options) -> void {
    // Dimension bounds cannot be destroyed
    if (is_outside_pocket_dimension_bounds(p)) { return; }
    ...
    int count = 0;
    while (count <= 25 && bash(p, 999, silent, true, false, nullptr, options).success
           && (!was_impassable || !passable(p, options))) {
        count++;
    }
}

auto mapbuffer::board_vehicle(
    const tripoint_abs_ms& p, Character& who, const mapbuffer_lookup_options options) -> bool {
    ...
    if (vp->part().has_flag(vehicle_part::passenger_flag)) {
        ...
        if (existing) { unboard_vehicle(p, false, options); }
    }
...
auto mapbuffer::unboard_vehicle(
    const tripoint_abs_ms& p, bool dead_passenger, const mapbuffer_lookup_options options) -> void {
    unboard_vehicle(
        p,
        {
...
auto mapbuffer::unboard_vehicle(
    const tripoint_abs_ms& p, const mapbuffer_unboard_vehicle_options& options) -> void {
    ...
    for (const auto& candidate : veh.get_any_parts(VPFLAG_BOARDABLE)) {
        if (!candidate.part().has_flag(vehicle_part::passenger_flag)) { continue; }
        if (options.passenger == nullptr
    ...
    if (!passenger_part) { return; }

    auto& part = passenger_part->part();
    auto* passenger =
        options.passenger != nullptr
            ? options.passenger
    ...
    if (passenger && !options.dead_passenger) {
        ...
        if (passenger->controlling_vehicle) { veh.skidding = true; }
        passenger->controlling_vehicle = false;
    }
}

auto mapbuffer::creature_in_field(Creature& critter, const mapbuffer_lookup_options options)
    -> void {
    const auto pos = critter.abs_pos();
    ...
    if (const auto* u = critter.as_player()) {
        ...
        if (in_vehicle) {
            if (const auto vp = veh_at(pos, options)) {
                if (vp->part_with_feature(VPFLAG_NOFIELDS, true)) { return; }
            }
        }
    }
    ...
    for (auto& field_entry_it : *curfield) {
        ...
        for (const auto& fe : cur.field_effects()) {
            ...
            critter.add_effect(
                field_fx.get_id(), field_fx.get_duration(), field_fx.get_bp(),
                field_fx.get_intensity());
        }
    }
}
...
auto mapbuffer::cheap_light_at(const tripoint_abs_ms& p, const mapbuffer_lookup_options options)
    -> float {
    // Skip pocket dimension bounds
    if (is_outside_pocket_dimension_bounds(p)) { return 0.0f; }

    // Step 1: Natural light level (coordinate-independent, based on z-level + time)
    const float natural_light =
        g != nullptr ? g->natural_light_level(p.z()) : LIGHT_AMBIENT_MINIMAL;
    ...
    if (tile) {
        ...
        if (lum > 0) { ambient = std::max(ambient, static_cast<float>(lum)); }
    }

    return ambient;
}

auto mapbuffer::get_heat_radiation(
    const tripoint_abs_ms& location, const bool direct, const mapbuffer_lookup_options options)
    -> int {
    int temp_mod = 0;
    int best_fire = 0;
    ...
    const auto visit_tile = [&](const abs_tile_handle& tile) {
        ...
        if (heat_intensity == 0 || !sees(location, dest, -1, options)) { return; }
    ...
    if (options.mode == mapbuffer_lookup_mode::simulated_only) {
        for (const auto& tile : simulated_tiles_in_radius(*this, location, 6)) { visit_tile(tile); }
    } else {
...
auto mapbuffer::get_convection_temperature(
    const tripoint_abs_ms& location, const mapbuffer_lookup_options options) -> int {
    ...
    if (const auto* fields = get_field(location, options)) {
        for (const auto& entry : *fields) {
            if (entry.first.obj().has_fire) { lava_mod = 0; }
...
auto mapbuffer::add_splatter(
    const field_type_id& type, const tripoint_abs_ms& where, const int intensity,
    const mapbuffer_lookup_options options) -> void {
...
auto mapbuffer::add_splatter_trail(
    const field_type_id& type, const tripoint_abs_ms& from, const tripoint_abs_ms& to,
    const mapbuffer_lookup_options options) -> void {
    const auto trail = line_to(from.xy(), to.xy());
    for (const auto& p : trail) {
        const tripoint_abs_ms pos(p, from.z());
        if (is_column_state(project_to<coords::sm>(pos).xy(), submap_column_load_state::resident)) {
            add_splatter(type, pos, 1, options);
        }
    }
}

auto mapbuffer::add_splash(
    const field_type_id& type, const tripoint_abs_ms& center, const int radius, const int intensity,
    const mapbuffer_lookup_options options) -> void {
...
auto mapbuffer::propagate_field(
    const tripoint_abs_ms& center, const field_type_id& type, int amount, const int max_intensity,
    const mapbuffer_lookup_options options) -> void {
    using gas_blast = std::pair<float, tripoint_abs_ms>;
    auto cmp = [](const gas_blast& a, const gas_blast& b) { return a.first > b.first; };
    ...
    while (amount > 0 && !open.empty()) {
        ...
        while (!open.empty()) {
            ...
            if (!closed.contains(next_pos)) { gas_front.push_back(open.top()); }
            open.pop();
        }
        ...
        while (!gas_front.empty()) {
            ...
            for (const auto& pt : simulated_tiles_in_radius(*this, gp_pos, 1)) {
                ...
                if (pt_pos == gp_pos || closed.contains(pt_pos)) { continue; }
...
auto mapbuffer::spawn_item(
    const tripoint_abs_ms& p, const itype_id& type_id, const unsigned quantity, const int charges,
    const time_point& birthday, const int damlevel, const mapbuffer_lookup_options options)
    -> void {
    for (unsigned i = 0; i < quantity; ++i) {
        auto new_item = item::spawn(type_id, birthday, charges > 0 ? (i == 0 ? charges : 0) : 0);
        if (damlevel > 0) { new_item->set_damage(damlevel); }
        add_item(p, std::move(new_item), options);
    }
}

auto mapbuffer::spawn_items(
    const tripoint_abs_ms& p, std::vector<detached_ptr<item>> new_items,
    const mapbuffer_lookup_options options) -> std::vector<detached_ptr<item>> // *NOPAD*
{
    std::vector<detached_ptr<item>> remaining;
    for (auto& it : new_items) {
        detached_ptr<item> leftover = add_item_or_charges(
            p, std::move(it),
            {
                .lookup = options,
            });
        if (!!leftover) { remaining.emplace_back(std::move(leftover)); }
    }
    return remaining;
}
...
auto mapbuffer::make_rubble(
    const tripoint_abs_ms& p, const furn_id& rubble_type, const ter_id& floor_type,
    const bool overwrite, const mapbuffer_lookup_options options) -> void {
    if (overwrite) {
        set_ter(p, floor_type, options);
        set_furn(p, rubble_type, options);
    } else {
        // First see if there is existing furniture to destroy
        if (is_bashable_furn(p, options)) { set_furn(p, f_null, options); }
        ...
        // Check again for new terrain after potential destruction
        {
            ...
            if (h_mc2 && h_mc2->move_cost() == 0) { set_ter(p, floor_type, options); }
        }
...
std::optional<tripoint_bub_ms> mapbuffer::active_reality_bubble_local(
    ...
    if (!g->m.inbounds(local)) { return std::nullopt; }

    return local;
}

auto mapbuffer::invalidate_active_terrain_set_caches(
    const tripoint_abs_ms& p, const ter_id& old_id, const ter_id& new_id) const -> void {
    ...
    if (new_terrain.has_flag(TFLAG_CONNECT_TO_WALL)
        != old_terrain.has_flag(TFLAG_CONNECT_TO_WALL)) {
        here.set_absorption_cache_dirty(*local);
    }
...
auto mapbuffer::sync_furniture_change_side_tables(
    const tripoint_abs_ms& p, submap& sm, const point_sm_ms& local, const furn_id& old_id,
    const furn_id& new_id, const cata::poly_serialized<active_tile_data>* new_active) const
    -> void {
    ...
    if (old_furniture.active) {
        ...
        if (tracker != nullptr) { tracker->on_changed(p); }
    }
...
auto mapbuffer::invalidate_active_furniture_set_caches(
    const tripoint_abs_ms& p, const furn_id& old_id, const furn_id& new_id) const -> void {
    const auto local = active_reality_bubble_local(p);
    if (!local) { return; }
    ...
    if (old_furniture.has_flag(TFLAG_NO_FLOOR) != new_furniture.has_flag(TFLAG_NO_FLOOR)
        || old_furniture.has_flag(TFLAG_Z_TRANSPARENT)
...
auto mapbuffer::sync_active_trap_change_side_tables(
    const tripoint_abs_ms& p, const point_sm_ms& local_tile, const trap_id& old_id,
    const trap_id& new_id) const -> void {
    const auto local = active_reality_bubble_local(p);
    if (!local) { return; }
    ...
    if (new_id.obj().is_funnel()) { here.funnel_locations_.emplace_back(sm_abs, local_tile); }
}

auto mapbuffer::invalidate_active_field_add_caches(
    const tripoint_abs_ms& p, const field_type_id& type) const -> void {
    const auto local = active_reality_bubble_local(p);
    if (!local) { return; }
    ...
    if (field_type.is_dangerous()) { here.set_pathfinding_cache_dirty(*local); }

    if (field_type.accelerated_decay) { here.support_dirty(*local); }
}

auto mapbuffer::invalidate_active_field_remove_caches(
    const tripoint_abs_ms& p, const field_type_id& type) const -> void {
    const auto local = active_reality_bubble_local(p);
    if (!local) { return; }
...
void mapbuffer::sync_active_item_submap_index(const tripoint_abs_ms& p, const submap& sm) {
...
void mapbuffer::save(bool delete_after_save, bool notify_tracker, bool show_progress) {
    ...
    std::vector<omt_entry> omts_to_process;
    {
        ...
        if (show_progress) { popup = std::make_unique<static_popup>(); }
        ...
        for (auto& [pos, sm_ptr] : submaps) {
            if (show_progress) {
                ...
                if (last_update + update_interval < now) {
                    popup->message(_("Please wait as the map saves [%d/%d]"), num_processed,
                                   num_total_submaps);
                    ui_manager::redraw();
                    refresh_display();
                    inp_mngr.pump_events();
            ...
            if (!seen_omts.insert(omt_addr).second) { continue; }
    ...
    for (const auto& pos : submaps_to_delete) { remove_submap(pos); }
    ...
    std::ranges::for_each(pending_snapshot, [&](auto& entry) {
        ...
        const bool in_memory =
            submaps.contains(base) || submaps.contains(base + point_east)
            || submaps.contains(base + point_south) || submaps.contains(base + point_south_east);
        if (!in_memory) {
            g->get_active_world()
                ->write_map_omt(dimension_id_.str(), omt_addr, [&data](std::ostream& fout) {
                    fout << data;
                });
        }
    });
...
void mapbuffer::save_omt(
    const tripoint_abs_omt& omt_addr, std::list<tripoint_abs_sm>& submaps_to_delete,
    ...
    if (all_uniform) {
        ...
        if (delete_after_save) {
            for (const tripoint_abs_sm& submap_addr : submap_addrs) {
                const auto it = submaps.find(submap_addr);
                if (it != submaps.end() && it->second) { submaps_to_delete.push_back(submap_addr); }
            }
        }
        return;
    }

    if (disable_mapgen) { return; }
    ...
    g->get_active_world()->write_map_omt(dimension_id_.str(), omt_addr, [&](std::ostream& fout) {
        ...
        for (const tripoint_abs_sm& submap_addr : submap_addrs) {
            ...
            if (delete_after_save) { submaps_to_delete.push_back(submap_addr); }
        }
...
void mapbuffer::deserialize_into_vec(
    JsonIn& jsin, std::vector<std::pair<tripoint_abs_sm, std::unique_ptr<submap>>>& out,
    ...
    while (!jsin.end_array()) {
        ...
        while (!jsin.end_object()) {
            ...
            if (submap_member_name == "version") {
                ...
                if (!sm) { // This whole thing is a nasty hack that relys on coordinates coming
                           // first...
                    debugmsg("coordinates was not at the top of submap json");
                }
                sm->load(jsin, submap_member_name, version,
                         project_to<coords::ms>(submap_coordinates), get_dimension_id());
            }
        }
        if (!skip) { out.emplace_back(submap_coordinates, std::move(sm)); }
    }
}
...
bool mapbuffer::preload_omt(const tripoint_abs_omt& omt_addr) {
    ...
    if (!pending_data.empty()) {
        ...
        g->get_active_world()->read_map_omt(
            dimension_id_.str(), omt_addr, [this, &loaded, &already_loaded](JsonIn& jsin) {
                deserialize_into_vec(jsin, loaded, already_loaded);
            });
    }
    ...
    for (auto& [pos, sm] : loaded) {
        if (!add_submap(pos, sm)) {
            DebugLog(DL::Warn, DC::Map) << string_format(
                "preload_omt: submap %d,%d,%d already loaded; keeping in-memory version", pos.x(),
                pos.y(), pos.z());
...
auto mapbuffer::generate_omt(
    const tripoint_abs_omt& omt_addr, const mapbuffer_generate_omt_options& options)
    -> mapgen_result {
    ZoneScopedN("mapbuffer_generate_omt");
    const auto base = project_to<coords::sm>(omt_addr);
    const auto all_loaded =
        lookup_submap_in_memory(base) && lookup_submap_in_memory(base + point_east)
    ...
    if (const auto uniform_terrain = uniform_terrain_for_omt(dimension_id_, omt_addr)) {
        ...
        if (generated) { run_omt_pillar_post_pass_if_complete(omt_addr.xy()); }
        return {
            .status =
...

    {
        ...
        const auto generate_result = constructor.generate(
            omt_addr, calendar::turn,
            {
        ...
        if (generate_result.needs_main_thread()) { return generate_result; }
        if (!generate_result.is_generated()) { return generate_result; }
    }
...
auto mapbuffer::run_omt_pillar_post_pass(const point_abs_omt& omt_pos) -> void {
    ...
    const auto ensure_vertical_transition_link =
        [&](const vertical_transition_link_request& request) {
            auto* const target_sm = lookup_submap_in_memory(request.target_pos);
            if (target_sm == nullptr
                || !can_replace_with_vertical_transition(
                    *target_sm, request.local, request.desired)) {
                return;
            }
    ...
    for (const auto zlev : std::views::iota(-OVERMAP_DEPTH, OVERMAP_HEIGHT + 1)) {
        ...
        for (const auto& offset : offsets) {
            ...
            if (sub_here == nullptr) { continue; }

            auto* const sub_below =
                zlev > -OVERMAP_DEPTH ? lookup_submap_in_memory(sm_pos + tripoint_below) : nullptr;

            auto changed = false;
            for (const auto local : ::submap_tiles()) {
                const auto terrain_here = sub_here->get_ter(local);
                /* DO NOT UNCOMMENT THIS UNTIL IT IS MADE TOGGLEABLE BY OMT OR TILE
                   BADLY EFFECTED MAPGENS INCLUDE: REF CENTER, LMOE SHELTER, and multiple modded
                OMTs if( const auto target = vertical_transition_target_below( terrain_here );
                ...
                */
                if (terrain_here != t_open_air) { continue; }
                if (zlev <= -OVERMAP_DEPTH) {
                    sub_here->set_ter(local, t_rock_floor);
                    changed = true;
                    continue;
                }
                if (sub_below == nullptr) { continue; }
            ...
            if (changed) { mark_post_pass_changed(*this, *sub_here); }
        }
    }
}
...
auto mapbuffer::actualize_submap(const tripoint_abs_sm& pos) -> void {
    ...
    for (const auto p : ::submap_tiles()) {
        ...
        if (!items.empty()) {
            ...
            if (!furn.has_flag("DONT_REMOVE_ROTTEN")) { remove_rotten_items(options, items); }
        }
