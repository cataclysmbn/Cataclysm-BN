...
struct memorized_terrain_tile {
    ...
    bool operator!=(const memorized_terrain_tile& rhs) const { return !(*this == rhs); }
};
...
public:
    ...
    bool is_empty() const { return tiles.empty() && symbols.empty() && terrain_tiles.empty(); }
...
private:
    ...
    std::vector<memorized_terrain_tile> terrain_tiles; // base terrain layer             — 0 or
                                                       // SEEX*SEEY
...
public:
     ...
     */
    void memorize_tile(
        const tripoint_abs_ms& pos, const std::string& ter, int subtile, int rotation);
    /**
    ...
    /**
     * Clears memorized overlay tile (furniture/vpart/trap) and symbol, leaving terrain memory
     * intact. Use this when a vehicle or furniture leaves a tile so the ground beneath is
     * preserved.
     * @param pos tile position, in global ms coords.
     */
