...
public:
     ...
     */
    auto update_request(
        load_request_handle handle, const point_abs_sm& begin, const point_abs_sm& end) -> void;
    ...
    auto update_lazy_border_focus(const dimension_id& dim_id, const tripoint_abs_ms& pos) -> void;
     ...
     */
    auto is_stably_requested(const dimension_id& dim_id, const point_abs_sm& pos) const -> bool;
    auto is_stably_requested(const dimension_id& dim_id, const tripoint_abs_sm& pos) const -> bool {
     ...
     */
    auto is_properly_requested(const dimension_id& dim_id, const tripoint_abs_sm& pos) const
        -> bool;
    /**
     * Return true if submap at @p pos in @p dim_id is loaded in memory.
     */
    auto is_loaded(const dimension_id& dim_id, const point_abs_sm& pos) const -> bool;
    auto is_loaded(const dimension_id& dim_id, const tripoint_abs_sm& pos) const -> bool {
     ...
     */
    auto is_simulated(const dimension_id& dim_id, const point_abs_sm& pos) const -> bool;
    auto is_simulated(const dimension_id& dim_id, const tripoint_abs_sm& pos) const -> bool {
...
private:
    ...
    template <typename CoordType> struct coord_pair_hash {
        auto operator()(const std::pair<dimension_id, CoordType>& k) const noexcept -> std::size_t {
    ...
    using horizontal_omt_set = std::unordered_set<omt_column_key, coord_pair_hash<point_abs_omt>>;
    ...
    struct lazy_omt_load_result {
        ...
        auto generated() const -> bool { return generation.is_generated(); }
    };
    ...
    std::unordered_map<vehicle*, vehicle_footprint_request> vehicle_footprint_requests_;
    ...
    std::unordered_map<omt_column_key, retained_omt_list::iterator, coord_pair_hash<point_abs_omt>>
        retained_omt_index_;

    /** OMT z-levels waiting for amortized lazy-border preload. */
    lazy_omt_job_list lazy_omt_jobs_;
    std::unordered_map<omt_key, lazy_omt_job_list::iterator, coord_pair_hash<tripoint_abs_omt>>
        lazy_omt_job_index_;
    ...
    auto add_lazy_border_into(key_set& target, const horizontal_omt_set& border_omts) const -> void;
    ...
    static auto load_lazy_omt_zlevel_data(
        mapbuffer& mb, const tripoint_abs_omt& omt_addr, const lazy_omt_load_options& options)
        -> lazy_omt_load_result;
    auto complete_lazy_omt_result_on_main_thread(const omt_key& key, lazy_omt_load_result result)
        -> lazy_omt_load_result;
    auto erase_lazy_omt_job(const omt_key& key) -> void;
    auto apply_lazy_omt_result(const omt_key& key, const lazy_omt_load_result& result) -> bool;
