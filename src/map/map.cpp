    ...
    if (myorigin != nullptr) {
        {
            ...
            if (h && h->furn() != f_null) { return h->furn_obj().max_volume; }
        }
        {
            auto h = abs_tile_handle::fetch_terrain_only(*myorigin, location);
            if (h) { return h->ter_obj().max_volume; }
        }
        return 0_ml;
    }
...
auto map::validate_active_submap_view_complete(const char* context) const -> void {
    ...
    if (view.is_complete()) { return; }

    for (const auto p : bubble_submaps()) {
        const auto local = point_rel_sm(p.x(), p.y());
        if (view.get_submap_view(local, p.z())) { continue; }
...
void map::reset_vehicle_cache() {
    ...
    for (vehicle* const veh : get_mapbuffer().get_vehicles()) {
        if (veh == nullptr || !get_mapbuffer().has_loaded_vehicle(veh)) { continue; }
        const auto footprints = get_mapbuffer().get_vehicle_submap_footprints(*veh);
        const auto overlaps_bubble = std::ranges::any_of(footprints, [&](const auto& footprint) {
            return footprint && footprint->max.x() >= bubble_min.x()
                && footprint->min.x() <= bubble_max.x() && footprint->max.y() >= bubble_min.y()
                && footprint->min.y() <= bubble_max.y() && footprint->max.z() >= bubble_min.z()
                && footprint->min.z() <= bubble_max.z();
        });
        if (!overlaps_bubble) { continue; }
        if (!inbounds_z(veh->abs_sm_pos.z())) { continue; }
        auto& ch = get_cache(veh->abs_sm_pos.z());
        ch.vehicle_list.insert(veh);
        if (!veh->loot_zones.empty()) { ch.zone_vehicles.insert(veh); }
...
static bool sees_veh(const Creature& c, vehicle& veh, bool force_recalc) {
    ...
    return std::ranges::any_of(veh_points, [&c](const tripoint_abs_ms& pt) { return c.sees(pt); });
}


bool map::obstructed_by_vehicle_rotation(
    const tripoint_bub_ms& from, const tripoint_bub_ms& to) const {
    return get_mapbuffer().obstructed_by_vehicle_rotation(::bub_to_abs(from), ::bub_to_abs(to));
}
...
VehicleList map::get_vehicles(const tripoint_bub_sm& start, const tripoint_bub_sm& end) {
    ...
    for (const auto zlev : std::views::iota(-OVERMAP_DEPTH, OVERMAP_HEIGHT + 1)) {
        for (vehicle* const veh : get_cache(zlev).vehicle_list) {
            ...
            const auto footprints = get_mapbuffer().get_vehicle_submap_footprints(*veh);
            const auto overlaps_window =
                std::ranges::any_of(footprints, [&](const auto& footprint) {
                    return footprint && footprint->max.x() >= abs_start.x()
                        && footprint->min.x() <= abs_end.x() && footprint->max.y() >= abs_start.y()
                        && footprint->min.y() <= abs_end.y() && footprint->max.z() >= abs_start.z()
                        && footprint->min.z() <= abs_end.z();
                });
            if (!overlaps_window) { continue; }
...
bool map::is_outside(const tripoint_abs_ms& p) const { return get_mapbuffer().is_outside(p); }
...
void map::smash_items(
    ...
    items.remove_top_items_with([&](detached_ptr<item>&& it) {
        ...
        if (will_explode_on_impact(power) && it->will_explode_in_fire()) {
            return item::detonate(std::move(it), contents);
        }
        if (it->is_corpse()) {
...
bash_results map::bash_ter_success(const tripoint_bub_ms& p, const bash_params& params) {
    ...
    if (has_flag_ter("FUNGUS", p)) {
        fungal_effects(*g, get_mapbuffer()).create_spores(bub_to_abs(p));
    }
...
bash_results map::bash_furn_success(const tripoint_bub_ms& p, const bash_params& params) {
    ...
    if (tent) {
        ...
        release_avatar_grabbed_furniture_if_destroyed(p, furnid, bash.furn_set);
        furn_set(p, bash.furn_set);
        for (item* const& it : i_at(p)) { it->on_drop(); }
        // HACK: Hack alert.
...
bash_results map::bash(
    const tripoint_bub_ms& p, const bash_params& bsh, const vehicle* bashing_vehicle) {
    bash_results result;
...
void map::shoot(
    ...
    if (furn.bash.ranged) {
        ...
        if (!hit_items
            ...
            if (get_avatar().sees(abs_pos)) { add_msg(_("The shot strikes the %s!"), furnname(p)); }
    ...
    if (ter.bash.ranged) {
        ...
        if (!hit_items
            ...
            if (get_avatar().sees(abs_pos)) { add_msg(_("The shot strikes the %s!"), tername(p)); }
            if (phys) { bash(p, dam, false); }
            dam = 0;
...
std::vector<tripoint_abs_sm> map::check_submap_active_item_consistency() {
    ...
    for (const tripoint_abs_sm& p : active_item_submaps) {
        ...
        if (!submap_loader.is_simulated(bound_dimension_, p)) { continue; }
...
void map::process_items(int turns) {
    ...
    // Uses for_each_simulated_submap to cover both in-bubble and off-bubble regions.
    {
        ...
        get_mapbuffer().for_each_simulated_submap([&](const tripoint_abs_sm& pos, submap& sm) {
            ...
            for (const auto& veh : sm.vehicles) {
                ...
                has_vehicle_with_items =
                    has_vehicle_with_items || counts.total > 0 || veh->has_cargo_recharge;
            }
            if (has_vehicle_with_items) { process_items_in_vehicles(sm, turns); }
        });
    }
...
std::vector<detached_ptr<item>> use_charges_from_stack(
    ...
    stack.remove_top_items_with([&quantity, &filter, &type, &pos, &ret](detached_ptr<item>&& it) {
        ...
        detached_ptr<item> new_it = item::use_charges(std::move(it), type, quantity, ret, filter);
        // NOLINTNEXTLINE(bugprone-use-after-move)
        if (it && !new_it) { ret.push_back(std::move(it)); }
        return new_it;
    });
...
static void use_charges_from_furn(
    ...
    for (const itype& itt : item_list) {
        if (itt.has_flag(json_flag_USES_GRID_POWER)) {
            ...
            if (filter(*furn_item)) {
                item::use_charges(std::move(furn_item), type, quantity, ret);
            ...
            if (iter != stack.end()) {
                ...
                (*iter)->attempt_detach(
                    [&filter, &ammo, &quantity, &ret, &p](detached_ptr<item>&& it) {
                        if (filter(*it)) {
                            return item::use_charges(std::move(it), ammo, quantity, ret);
                        }
                        return std::move(it);
                    });
            }
...
void map::add_splatter_trail(
    ...
    for (tripoint_bub_ms& elem : trail) {
        ...
        remainder--;
        if (get_mapbuffer()
                .obstructed_by_vehicle_rotation(bub_to_abs(last_point), bub_to_abs(elem))) {
            if (one_in(2)) {
                elem.x() = last_point.x();
        ...
        bresenham(
            ...
            [this, &is_clear, cost_min, cost_max, &t, &last_point](point new_point) {
                ...
                if (cost < cost_min || cost > cost_max
                    || get_mapbuffer().obstructed_by_vehicle_rotation(
                        bub_to_abs(tripoint_bub_ms(last_point, t.z())),
                        bub_to_abs(tripoint_bub_ms(point_bub_ms(new_point), t.z())))) {
                    is_clear = false;
                    return false;
                }
    ...
    bresenham(f.raw(), t.raw(), 0, 0,
              [this, &is_clear, cost_min, cost_max, t, &last_point](const tripoint& new_point) {
                  ...
                  if (new_point.z == last_point.z()) {
                      ...
                      if (cost < cost_min || cost > cost_max
                          || get_mapbuffer().obstructed_by_vehicle_rotation(
                              bub_to_abs(last_point), bub_to_abs(tripoint_bub_ms(new_point)))) {
                          is_clear = false;
                          return false;
                      }
                  } else {
...
void map::spawn_monsters_submap_group(
    ...
    const auto current_submap = active_load_region_.view().get_submap_view(
    ...
    for (const auto sm_ms : submap_tiles()) {
        ...
        const auto vehicle_at_tile =
            group.horde ? get_mapbuffer().veh_at(abs_fp) : optional_vpart_position();
        const bool vehicle_owned =
            vehicle_at_tile && vehicle_at_tile->vehicle().is_owned_by(get_avatar());
        ...
        if (vehicle_tile_passable && !vehicle_owned && (outside || spawnable_vehicle_tile)) {
    ...
    for (auto& i : current_submap->spawns) {
        ...
        for (int j = 0; j < i.count; j++) {
            ...
            const auto valid_location = [&](const tripoint_bub_ms& p) {
                ...
                return (!g || &get_map() != this || !g->critter_at(p))
                    && tmp.can_move_to(bub_to_abs(p));
            };
                ...
                if (above != nullptr) {
                    for (const auto dx : std::views::iota(-1, 2)) {
                        for (const auto dy : std::views::iota(-1, 2)) {
                            ...
                            if (!above->inbounds(above_pos)) { continue; }
                            const auto above_idx = above->idx(above_pos.x(), above_pos.y());
                            outside =
                                outside
                                || (above->outside_cache[above_idx]
    ...
    for (int smx = min_submap.x(); smx <= max_submap.x(); ++smx) {
        for (int smy = min_submap.y(); smy <= max_submap.y(); ++smy) {
            ...
            const auto sm_view = active_load_region_.view().get_submap_view(
                point_rel_sm(gridp.x(), gridp.y()), gridp.z());
            if (!sm_view) {
                for (const auto sm_ms : submap_tiles()) {
...
void map::build_map_cache(const int zlev, bool skip_lightmap) {
    ...
    auto level_has_vehicle_obscuration = [](const level_cache& ch) {
        const auto has_block = [](const diagonal_blocks& blocks) { return blocks.nw || blocks.ne; };
...

    {
        ...
        std::ranges::
            for_each(std::views::iota(-OVERMAP_DEPTH, OVERMAP_HEIGHT + 1), [this](const int z) {
                get_cache(z).vehicle_obstruction_cache_dirty = false;
            });
        std::ranges::
            for_each(std::views::iota(-OVERMAP_DEPTH, OVERMAP_HEIGHT + 1), [&](const int z) {
...
auto map::function_over(const tripoint_bub_ms& start, const tripoint_bub_ms& end, Functor fun) const
    ...
    for (const auto z : std::views::iota(min.z(), max.z() + 1)) {
        ...
        for (const auto smp : submap_range) {
            ...
            const auto sm_view =
                active_load_region_.view().get_submap_view(point_rel_sm(smp.x(), smp.y()), z);
            if (!sm_view) {
...
void map::scent_blockers(
    ...
    for (auto& wrapped_veh : vehs) {
        ...
        for (const vpart_reference& vp : veh.get_any_parts(VPFLAG_OPENABLE)) {
            if (!vp.part().open) { mark_vehicle_obstruction(vp.bub_pos()); }
        }
    }
}
