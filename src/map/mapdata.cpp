...
auto add_fluid_grid_variant(
    std::map<furn_str_id, furn_str_id>& variants, const furn_str_id& from, const furn_str_id& to,
    const char* label) -> void {
    const auto iter = variants.find(from);
    if (iter != variants.end() && iter->second != to) {
        debugmsg("fluid grid %s variant conflict for %s: %s vs %s", label, from.c_str(),
                 iter->second.c_str(), to.c_str());
        return;
    }
    variants[from] = to;
}
...
auto build_fluid_grid_variant_maps() -> void {
    ...
    std::ranges::for_each(furniture_data.get_all(), [&](const furn_t& furn) {
        if (!furn.fluid_grid || furn.fluid_grid->role != fluid_grid_role::tank) { return; }
        if (furn.fluid_grid->connected_variant) {
            add_fluid_grid_variant(
                connected, furn.id, *furn.fluid_grid->connected_variant, "connected");
        }
        if (furn.fluid_grid->disconnected_variant) {
            add_fluid_grid_variant(
                disconnected, furn.id, *furn.fluid_grid->disconnected_variant, "disconnected");
        }
    });
...
auto parse_fluid_grid_role(const std::string& role) -> std::optional<fluid_grid_role> {
    ...
    if (iter == role_map.end()) { return std::nullopt; }
    return iter->second;
}
...
static const std::unordered_map<std::string, ter_bitflags> ter_bitflags_map = {{
    ...
    {"COLLAPSES", TFLAG_COLLAPSES}, // This tile includes a ceiling. If the ceiling drops, this tile
                                    // is destroyed.
    ...
    {"NO_SCENT", TFLAG_NO_SCENT},     // cannot have scent values, which prevents scent diffusion
                                      // through this tile
    ...
    {"CONNECT_TO_WALL", TFLAG_CONNECT_TO_WALL},   // superseded by ter_connects, retained for json
    ...
    {"SEEN_FROM_ABOVE", TFLAG_SEEN_FROM_ABOVE},   // This should be visible if the tile above has no
                                                  // floor
    {"HIDE_PLACE", TFLAG_HIDE_PLACE}, // Creature on this tile can't be seen by other creature not
                                      // standing on adjacent tiles
    ...
    {"THIN_OBSTACLE", TFLAG_THIN_OBSTACLE},   // Passable by players and monsters. Vehicles destroy
                                              // it.
    {"SMALL_PASSAGE", TFLAG_SMALL_PASSAGE},   // A small passage, that large or huge things cannot
                                              // pass through
    {"Z_TRANSPARENT", TFLAG_Z_TRANSPARENT},   // Doesn't block vision passing through the z-level
    {"SUN_ROOF_ABOVE", TFLAG_SUN_ROOF_ABOVE}, // This furniture has a "fake roof" above, that blocks
                                              // sunlight (see #44421).
    ...
    {"SUSPENDED", TFLAG_SUSPENDED}, // This furniture is suspended between other terrain, and will
                                    // cause a cascading failure on break.
    ...
    {"ROAD", TFLAG_ROAD}, // Some floors have this flag, as do some passable transformation of
                          // otherwise impassible terrain/furniture. Very notably, open doors.
    {"BASH_TRANSFORM", TFLAG_BASH_TRANSFORM}, // Bashing this terrain/furniture but failing to
                                              // destroy it has a chance to transform it, if it's
                                              // capable of transforming.
}};

static const std::unordered_map<std::string, ter_connects> ter_connects_map = {{
    {"WALL", TERCONN_WALL}, // implied by TFLAG_CONNECT_TO_WALL, TFLAG_AUTO_WALL_SYMBOL or
                            // TFLAG_WALL
...
static void load_map_bash_tent_centers(const JsonArray& ja, std::vector<furn_str_id>& centers) {
    for (const std::string& line : ja) { centers.emplace_back(line); }
}

static void correct_if_magic(std::optional<units::sound>& val) {
    if (val && *val < 0_dB) { val.reset(); }
}
...
bool map_deconstruct_info::load(
    ...
    if (!is_furniture) { ter_set = ter_str_id(j.get_string("ter_set")); }
    can_do = true;
...
furn_workbench_info::furn_workbench_info()
    : multiplier(1.0f),
      allowed_mass(units::mass_max),
...
plant_data::plant_data()
    : transform(furn_str_id::NULL_ID()),
      base(furn_str_id::NULL_ID()),
      growth_multiplier(1.0f),
      harvest_multiplier(1.0f) {}
...
pry_result::pry_result()
    : pry_quality(-1),
      ...
      break_ter_type(ter_str_id::NULL_ID()),
      break_furn_type(furn_str_id::NULL_ID()),
...
ter_t::ter_t()
    : open(ter_str_id::NULL_ID()),
      close(ter_str_id::NULL_ID()),
...
void load_season_array(const JsonObject& jo, const std::string& key, C& container, F load_func) {
    if (!jo.has_member(key)) {
        ...
        if (arr.size() == 1) {
            ...
            for (auto& e : container) { e = load_func(arr.next_string()); }
...
void load_furniture(const JsonObject& jo, const std::string& src) {
    if (furniture_data.empty()) { furniture_data.insert(null_furniture_t()); }
...
void map_data_common_t::set_flag(const std::string& flag) {
    ...
    if (it != ter_bitflags_map.end()) {
        ...
        if (!transparent && it->second == TFLAG_TRANSPARENT) { transparent = true; }
...
ter_id t_null,
    // Ground
    t_dirt, t_sand, t_clay, t_alluvial_deposit, t_dirtmound, t_pit_shallow, t_pit, t_grave,
    t_grave_new, t_pit_corpsed, t_pit_covered, t_pit_spiked, t_pit_spiked_covered, t_pit_glass,
    t_pit_glass_covered, t_rock_floor, t_grass, t_grass_long, t_grass_tall, t_grass_golf,
    t_grass_dead, t_grass_white, t_moss, t_moss_underground, t_metal_floor, t_pavement,
    t_pavement_y, t_sidewalk, t_concrete, t_thconc_floor, t_thconc_floor_olight, t_strconc_floor,
    t_floor, t_floor_waxed,
    t_dirtfloor, // Dirt floor(Has roof)
    t_carpet_red, t_carpet_yellow, t_carpet_purple, t_carpet_green, t_linoleum_white,
    t_linoleum_gray, t_grate, t_slime, t_bridge, t_covered_well,
    // Lighting related
    t_utility_light,
    // Walls
    t_wall_log_half, t_wall_log, t_wall_log_chipped, t_wall_log_broken, t_palisade, t_palisade_gate,
    t_palisade_gate_o, t_wall_half, t_wall_wood, t_wall_wood_chipped, t_wall_wood_broken, t_wall,
    t_concrete_wall, t_brick_wall, t_wall_metal, t_wall_glass, t_wall_glass_alarm,
    t_reinforced_glass, t_reinforced_glass_shutter, t_reinforced_glass_shutter_open,
    t_laminated_glass, t_ballistic_glass, t_reinforced_door_glass_o, t_reinforced_door_glass_c,
    t_bars, t_reb_cage, t_wall_r, t_wall_w, t_wall_b, t_wall_g, t_wall_p, t_wall_y, t_door_c,
    ...
    t_door_frame, t_chaingate_l, t_fencegate_c, t_fencegate_o, t_chaingate_c, t_chaingate_o,
    t_door_boarded, t_door_boarded_damaged, t_door_boarded_peep, t_rdoor_boarded,
    t_rdoor_boarded_damaged, t_door_boarded_damaged_peep, t_door_metal_c, t_door_metal_o,
    t_door_metal_locked, t_door_metal_pickable, t_mdoor_frame, t_door_bar_c, t_door_bar_o,
    t_door_bar_locked, t_door_glass_c, t_door_glass_o, t_door_glass_frosted_c,
    t_door_glass_frosted_o, t_portcullis, t_recycler, t_window, t_window_taped, t_window_domestic,
    t_window_domestic_taped, t_window_open, t_curtains, t_window_bars_curtains,
    t_window_bars_domestic, t_window_alarm, t_window_alarm_taped, t_window_empty, t_window_frame,
    t_window_boarded, t_window_boarded_noglass, t_window_reinforced, t_window_reinforced_noglass,
    t_window_enhanced, t_window_enhanced_noglass, t_window_bars_alarm, t_window_bars,
    t_window_stained_green, t_window_stained_red, t_window_stained_blue, t_window_no_curtains,
    t_window_no_curtains_open, t_window_no_curtains_taped, t_rock, t_fault, t_paper, t_rock_wall,
    t_rock_wall_half,
    // Tree
    t_tree, t_tree_young, t_tree_apple, t_tree_apple_harvested, t_tree_coffee,
    t_tree_coffee_harvested, t_tree_pear, t_tree_pear_harvested, t_tree_cherry,
    t_tree_cherry_harvested, t_tree_peach, t_tree_peach_harvested, t_tree_apricot,
    t_tree_apricot_harvested, t_tree_plum, t_tree_plum_harvested, t_tree_pine, t_tree_blackjack,
    t_tree_birch, t_tree_willow, t_tree_maple, t_tree_maple_tapped, t_tree_hickory,
    t_tree_hickory_dead, t_tree_hickory_harvested, t_tree_deadpine, t_underbrush, t_shrub,
    t_shrub_blueberry, t_shrub_strawberry, t_trunk, t_stump, t_root_wall, t_wax, t_floor_wax,
    t_fence, t_chainfence, t_chainfence_posts, t_fence_post, t_fence_wire, t_fence_barbed,
    t_fence_rope, t_railing,
    // Nether
    ...
    t_water_cube, t_lake_bed, t_lake_moss, t_water_pool, t_sewage, t_lava,
    // More embellishments than you can shake a stick at.
    t_sandbox, t_slide, t_monkey_bars, t_backboard, t_gas_pump, t_gas_pump_smashed, t_diesel_pump,
    t_diesel_pump_smashed, t_atm, t_generator_broken, t_missile, t_missile_exploded, t_radio_tower,
    t_radio_controls, t_console_broken, t_console, t_gates_mech_control, t_gates_control_concrete,
    t_gates_control_brick, t_barndoor, t_palisade_pulley, t_gates_control_metal, t_sewage_pipe,
    t_sewage_pump, t_centrifuge, t_column, t_vat, t_rootcellar, t_cvdbody, t_cvdmachine, t_nanofab,
    t_nanofab_body, t_water_pump, t_conveyor, t_machinery_light, t_machinery_heavy, t_machinery_old,
    t_machinery_electronic, t_improvised_shelter,
    // Staircases etc.
    t_stairs_down, t_stairs_up, t_manhole, t_ladder_up, t_ladder_down, t_slope_down, t_slope_up,
    t_rope_up, t_manhole_cover,
    // Special
    t_card_science, t_card_military, t_card_industrial, t_card_reader_broken, t_slot_machine,
    t_elevator_control, t_elevator_control_off, t_elevator, t_pedestal_wyrm, t_pedestal_temple,
    // Temple tiles
    t_rock_red, t_rock_green, t_rock_blue, t_floor_red, t_floor_green, t_floor_blue, t_switch_rg,
    t_switch_gb, t_switch_rb, t_switch_even, t_open_air, t_plut_generator, t_pavement_bg_dp,
    t_pavement_y_bg_dp, t_sidewalk_bg_dp, t_guardrail_bg_dp, t_rad_platform,
    // Railroad and subway
    t_railroad_rubble, t_buffer_stop, t_railroad_crossing_signal, t_crossbuck_wood,
    t_crossbuck_metal, t_railroad_tie, t_railroad_tie_h, t_railroad_tie_v, t_railroad_tie_d,
    t_railroad_track, t_railroad_track_h, t_railroad_track_v, t_railroad_track_d,
    t_railroad_track_d1, t_railroad_track_d2, t_railroad_track_on_tie, t_railroad_track_h_on_tie,
    t_railroad_track_v_on_tie, t_railroad_track_d_on_tie, t_pd_border, t_rock_border;
...
furn_id f_null, f_hay, f_rubble, f_rubble_rock, f_wreckage, f_ash, f_barricade_road, f_sandbag_half,
    f_sandbag_wall, f_bulletin, f_indoor_plant, f_indoor_plant_y, f_bed, f_toilet, f_makeshift_bed,
    f_straw_bed, f_sink, f_oven, f_woodstove, f_fireplace, f_bathtub, f_chair, f_armchair, f_sofa,
    f_cupboard, f_trashcan, f_desk, f_exercise, f_ball_mach, f_bench, f_lane, f_table, f_pool_table,
    f_counter, f_fridge, f_fridge_on, f_minifreezer_on, f_glass_fridge, f_freezer, f_dresser,
    f_locker, f_rack, f_bookcase, f_washer, f_dryer, f_vending_c, f_vending_o, f_dumpster,
    f_dive_block, f_crate_c, f_crate_o, f_coffin_c, f_coffin_o, f_gunsafe_ml, f_large_canvas_wall,
    f_canvas_wall, f_canvas_door, f_canvas_door_o, f_groundsheet, f_fema_groundsheet,
    f_large_groundsheet, f_large_canvas_door, f_large_canvas_door_o, f_center_groundsheet,
    f_skin_wall, f_skin_door, f_skin_door_o, f_skin_groundsheet, f_mutpoppy, f_flower_fungal,
    f_fungal_mass, f_fungal_clump, f_dahlia, f_datura, f_dandelion, f_cattails, f_bluebell,
    f_lake_pondweed, f_lake_detritus, f_lake_liverwort, f_lake_eelgrass, f_lake_hornwort,
    f_cave_mushrooms, f_safe_c, f_safe_l, f_safe_o, f_plant_seed, f_plant_seedling, f_plant_mature,
    f_plant_harvest, f_fvat_empty, f_fvat_full, f_wood_keg, f_standing_tank, f_statue, f_egg_sackbw,
    f_egg_sackcs, f_egg_sackws, f_egg_sacke, f_flower_marloss, f_floor_canvas, f_tatami,
    f_kiln_empty, f_kiln_full, f_kiln_metal_empty, f_kiln_metal_full, f_arcfurnace_empty,
    f_arcfurnace_full, f_smoking_rack, f_smoking_rack_active, f_metal_smoking_rack,
    f_metal_smoking_rack_active, f_water_mill, f_water_mill_active, f_wind_mill, f_wind_mill_active,
    f_robotic_arm, f_vending_reinforced, f_brazier, f_firering, f_tourist_table, f_camp_chair,
    f_sign;
    ...
    if (jo.has_array("harvest_by_season")) {
        for (JsonObject harvest_jo : jo.get_array("harvest_by_season")) {
            ...
            std::ranges::transform(
                season_strings, std::inserter(seasons, seasons.begin()),
            ...
            if (harvest_jo.has_array("entries")) {
                ...
                hl = harvest_list::load(
                    harvest_jo, src, string_format("harvest_inline_%d", static_cast<int>(num)));
            ...
            for (season_type s : seasons) { harvest_by_season[s] = hl; }
        }
    }
...
void ter_t::load(const JsonObject& jo, const std::string& src) {
    ...
    if (jo.has_member("connects_to")) { set_connects(jo.get_string("connects_to")); }
    ...
    if (jo.has_object("oxytorch")) { oxytorch->load(jo.get_object("oxytorch")); }

    boltcut = cata::make_value<activity_data_ter>();
    if (jo.has_object("boltcut")) { boltcut->load(jo.get_object("boltcut")); }

    hacksaw = cata::make_value<activity_data_ter>();
    if (jo.has_object("hacksaw")) { hacksaw->load(jo.get_object("hacksaw")); }
...
static void check_bash_items(const map_bash_info& mbi, const std::string& id, bool is_terrain) {
    if (!item_group::group_is_defined(mbi.drop_group)) {
        debugmsg("%s: bash result item group %s does not exist", id.c_str(),
    ...
    if (mbi.str_max != -1) {
        ...
        if (!mbi.ter_set.is_valid()) {
            debugmsg("bash result terrain %s of %s does not exist", mbi.ter_set.c_str(),
                     id.c_str());
        }
        if (!mbi.furn_set.is_valid()) {
            debugmsg("bash result furniture %s of %s does not exist", mbi.furn_set.c_str(),
                     id.c_str());
        }
    }
...
static void check_decon_items(
    const map_deconstruct_info& mbi, const std::string& id, bool is_terrain) {
    if (!mbi.can_do) { return; }
    if (!item_group::group_is_defined(mbi.drop_group)) {
    ...
    if (!mbi.ter_set.is_valid()) {
        debugmsg("deconstruct result terrain %s of %s does not exist", mbi.ter_set.c_str(),
                 id.c_str());
    }
    if (!mbi.furn_set.is_valid()) {
...
static void check_pry_items(const pry_result& pry, const std::string& id, bool is_terrain) {
    if (pry.pry_quality == -1) { return; }
    ...
    if (is_terrain) {
        ...
        if (!pry.new_ter_type.is_valid()) { // Some tiles specify t_null explicitly
            debugmsg("pry result terrain of %s does not exist", pry.new_ter_type.c_str(),
                     id.c_str());
        }
        if (pry.breakable && !pry.break_ter_type.is_valid()) {
            debugmsg("pry breakage result terrain %s of %s does not exist",
                     pry.new_ter_type.c_str(), id.c_str());
        }
    } else {
        ...
        if (!pry.new_furn_type.is_valid()) { // Some tiles specify t_null explicitly
            debugmsg("pry result furniture of %s does not exist", pry.new_furn_type.c_str(),
                     id.c_str());
        }
        if (pry.breakable && !pry.break_furn_type.is_valid()) {
            debugmsg("pry breakage result furniture %s of %s does not exist",
                     pry.new_furn_type.c_str(), id.c_str());
        }
    }
}
...
void ter_t::check() const {
    ...
    if (examine == iexamine_function_from_string("locked_object_pickable")
        ...
        throw JsonError(string_format(
            "Terrain %s has iexamine `locked_object_pickable`, without a non-null `lockpick_result`",
    ...
    if (id && open == id) { debugmsg("%s has \"open\" set to itself", id.c_str()); }
    if (!close.is_valid()) {
        debugmsg("invalid terrain %s for closing %s", close.c_str(), id.c_str());
    }
    if (id && close == id) { debugmsg("%s has \"close\" set to itself", id.c_str()); }
    ...
    if (roof && roof->roof) {
        debugmsg("%s has roof %s, which has its own roof %s", id.str(), roof.str(),
                 roof->roof.str());
    }
    if (roof && !roof->bash.bash_below) {
        debugmsg("%s has roof %s, with \"bash_below\": false", id.str(), roof.str());
    }
    ...
    if (json_report_strict && deconstruct.ter_set == t_open_air.id()) {
        debugmsg("%s deconstructs into \"t_open_air\", but \"t_null\" is preferred", id.str());
    }
        ...
        if (!role) {
            ...
            if (fluid_grid_obj.has_member("transformer")) {
                ...
                if (fluid_grid_entry.role == fluid_grid_role::rain_collector) {
                    optional(transformer_obj, was_loaded, "collector_area_m2",
                             transformer.collector_area_m2, 0.0);
                } else {
                    optional(transformer_obj, was_loaded, "collector_area_m2",
                             transformer.collector_area_m2, 0.0);
                    ...
                    for (JsonObject transform_obj : transforms) {
                        ...
                        auto parse_io =
                            [&](const JsonArray& array, std::vector<fluid_grid_transform_io>& out) {
                                for (JsonObject io_obj : array) {
                                    ...
                                    if (!assign(io_obj, "amount", io.amount, true)) {
                                        debugmsg(
                                            "fluid grid transformer entry missing amount in %s",
                                            id.c_str());
                                    }
                                    out.push_back(io);
                                }
                        ...
                        if (recipe.inputs.empty() && recipe.outputs.empty()) {
                            debugmsg("fluid grid transformer entry missing inputs/outputs in %s",
                                     id.c_str());
                        }
...
void furn_t::check() const {
    ...
    if (has_flag("EMITTER")) {
        if (emissions.empty()) {
            ...
            for (const emit_id& e : emissions) {
                if (!e.is_valid()) {
                    debugmsg("furn %s has the EMITTER flag, but invalid emission %s was set",
    ...
    if (fluid_grid) {
        ...
        if (fluid_grid_data.role == fluid_grid_role::tank) {
            ...
            if (fluid_grid_data.capacity && fluid_grid_data.use_keg_capacity) {
                debugmsg("furn %s has both fluid grid capacity and use_keg_capacity set",
                         id.c_str());
            }
            if (!fluid_grid_data.connected_variant && !fluid_grid_data.disconnected_variant) {
                debugmsg(
                    "furn %s has fluid grid tank role but no connected/disconnected variant configured",
                    id.c_str());
            }
        } else {
            if (fluid_grid_data.capacity || fluid_grid_data.use_keg_capacity) {
                debugmsg("furn %s has fluid grid non-tank role with capacity configured",
                         id.c_str());
            }
        }
        if ((fluid_grid_data.role == fluid_grid_role::transformer
            ...
            debugmsg(
                "furn %s has fluid grid transformer or rain_collector role but no transformer configured",
                id.c_str());
        }
        if (fluid_grid_data.role == fluid_grid_role::rain_collector && fluid_grid_data.transformer
        ...
        if (fluid_grid_data.disconnected_variant
            && !fluid_grid_data.disconnected_variant->is_valid()) {
...
auto fluid_grid_connected_variant(const furn_id& id) -> std::optional<furn_id> {
    ...
    if (iter == connected.end() || !iter->second.is_valid()) { return std::nullopt; }
...
auto fluid_grid_disconnected_variant(const furn_id& id) -> std::optional<furn_id> {
    ...
    if (iter == disconnected.end() || !iter->second.is_valid()) { return std::nullopt; }
    return furn_id(iter->second);
}
...
void activity_byproduct::load(const JsonObject& jo) {
    ...
    if (jo.has_int("count")) {
        ...
        if (random_min > random_max) { std::swap(random_min, random_max); }
    }
}
