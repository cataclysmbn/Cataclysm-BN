...
map_memory::map_memory() { clear_cache(); }
...
bool map_memory::has_memory_for_autodrive(const tripoint_abs_ms& pos) {
    ...
    //       Problem is, many cars' headlights won't cover every ground tile in front of them at
    //       night, and these dark tiles would be considered as possible obstacles. To work around
    //       it, we check for whether map memory has any data associated with the tile and then
    //       assume it's up to date, which works in 99% cases. Oh, and we don't want to use
    //       get_tile() and get_symbol() to avoid looking up the mm_submap twice.
    const auto p = project_remain<coords::sm>(pos);
...
void map_memory::memorize_tile(
    const tripoint_abs_ms& pos, const std::string& ter, const int subtile, const int rotation) {
    const auto p = project_remain<coords::sm>(pos);
...
void map_memory::memorize_terrain_tile(
    const tripoint_abs_ms& pos, const std::string& ter, const int subtile, const int rotation) {
    const auto p = project_remain<coords::sm>(pos);
    mm_submap& sm = get_submap(p.quotient_tripoint);
...
bool map_memory::prepare_region(const tripoint_abs_ms& p1, const tripoint_abs_ms& p2) {
    ...
    if (rect.contains(sm_p1.xy()) && rect.contains(sm_p2.xy())) { return false; }
    ...
    for (int z = -OVERMAP_DEPTH; z <= OVERMAP_HEIGHT; z++) {
        for (int dy = 0; dy < cache_size.y(); dy++) {
            for (int dx = 0; dx < cache_size.x(); dx++) {
                cached.push_back(
                    fetch_submap(tripoint_abs_sm(cache_pos.xy(), z) + point_rel_sm(dx, dy)));
            }
        }
    }
    return true;
...
static void temp_remove_open_air(const shared_ptr_fast<mm_submap>& sm) {
    ...
    for (const auto sm_ms : submap_tiles()) {
        ...
        if (!t.tile.empty()
            && (t.tile == "t_open_air" || t.tile == "t_open_air_rooved"
...
shared_ptr_fast<mm_submap> map_memory::load_submap(const tripoint_abs_sm& sm_pos) {
    if (test_mode) { return nullptr; }

    const auto reg = project_to<coords::mmr>(sm_pos);

    mm_region mmr;
    const auto loader = [&](JsonIn& jsin) { mmr.deserialize(jsin); };
    ...
    try {
        ...
        debugmsg("Failed to load memory map region (%d,%d,%d): %s", reg.x(), reg.y(), reg.z(),
                 err.what());
        return nullptr;
    }
...
void map_memory::load(const tripoint_abs_ms& pos) {
    ...
    for (int dy = 0; dy < mm_size; dy++) {
        for (int dx = 0; dx < mm_size; dx++) { fetch_submap(start + tripoint(dx, dy, 0)); }
    }
...
bool map_memory::save(const tripoint_abs_ms& pos) {
    ...
    half_open_rectangle<point_abs_sm>
        rect_keep(sm_center.xy() - mm_hsize_p, sm_center.xy() + mm_hsize_p);
    ...
    for (auto& it : regions) {
        ...
        if (!reg.is_empty()) {
            const auto writer = [&](std::ostream& fout) -> void {
                fout << serialize_wrapper([&](JsonOut& jsout) { reg.serialize(jsout); });
            };
        ...
        half_open_rectangle<point_abs_sm>
            rect_reg(regp_sm.xy(), regp_sm.xy() + point_rel_sm(MM_REG_SIZE, MM_REG_SIZE));
