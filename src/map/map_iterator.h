...
public:
    ...
    public:
        ...
        point_generator(const Tripoint& _p, const tripoint_range* _range): p(_p) {
            if (_range) {
        ...
        auto operator++() -> point_generator& { // *NOPAD*
            traits::x(p)++;
            if (traits::x(p) <= traits::x(range_max)) { return *this; }

            traits::y(p)++;
            traits::x(p) = traits::x(range_min);
            if (traits::y(p) <= traits::y(range_max)) { return *this; }
        ...
        auto operator==(const point_generator& other) const -> bool { return p == other.p; }
    };
    ...
    tripoint_range(const Tripoint& _minp, const Tripoint& _maxp): minp(_minp), maxp(_maxp) {}

    auto begin() const -> point_generator { return point_generator(minp, this); }
    ...
    const Tripoint& min() const { return minp; }
    const Tripoint& max() const { return maxp; }
};

template <typename Tripoint>
inline tripoint_range<Tripoint> points_in_radius(
    const Tripoint& center, const int radius, const int radiusz = 0) {
    static_assert(Tripoint::dimension == 3, "Requires tripoint type");
...
public:
    ...
    public:
        ...
        point_generator(const Point& _p, const point_range* _range): p(_p) {
            if (_range) {
                range_min = _range->minp;
                range_max = _range->maxp;
            }
    ...
    point_range(const Point& _minp, const Point& _maxp): minp(_minp), maxp(_maxp) {}
