...
submap::submap(const tripoint_abs_sm& position, const dimension_id& dim)
    : maptile_soa<SEEX, SEEY>(position, dim) {
    dim_ = dim;
    pos_ = position;
...
auto submap::set_position(const tripoint_abs_sm& position) -> void {
    if (pos_ == position) { return; }
    const auto offset = project_to<coords::ms>(position) - project_to<coords::ms>(pos_);
    for (const auto& p : submap_tiles()) { itm[p.x()][p.y()].move_by(offset); }
    pos_ = position;
}
