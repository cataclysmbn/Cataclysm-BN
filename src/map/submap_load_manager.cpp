...
auto submap_load_manager::evict_omt_column(const omt_column_key& key) -> void {
    ...
    if (was_dirty) { dirty_omts_.erase(key); }
...
auto submap_load_manager::apply_lazy_omt_result(
    ...
    if (result.dirty || result.generated()) { dirty_omts_.insert({key.first, key.second.xy()}); }
...
auto submap_load_manager::update(const bool defer_lazy_border_work) -> void {
    ...
    for (const auto& [dim_id, omt_xy] : new_omts) { dirty_omts_.insert({dim_id, omt_xy}); }
