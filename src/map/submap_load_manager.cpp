...
auto signum(const int value) -> int { return (value > 0) - (value < 0); }
...
auto steps_to_omt_edge_axis(const int local, const int direction) -> std::size_t {
    ...
    if (direction < 0) { return clamp_steps(local + 1); }
...
auto omt_edge_deadlines_for(const tripoint_abs_ms& pos, const point& direction)
    -> omt_edge_deadlines {
    if (direction == point_zero) { return {}; }
...
auto is_omt_zlevel_loaded(mapbuffer& mb, const tripoint_abs_omt& omt_addr) -> bool {
    ...
    return mb.lookup_submap_in_memory(sm_base) && mb.lookup_submap_in_memory(sm_base + point_east)
...
auto contains_request_pos(const submap_load_request& req, const point_abs_sm& pos) -> bool {
    return pos.x() >= req.begin.x() && pos.x() < req.end.x() && pos.y() >= req.begin.y()
        && pos.y() < req.end.y();
}
...
auto submap_load_manager::request_load(
    load_request_source source, const dimension_id& dim_id, const point_abs_sm& begin,
...
auto submap_load_manager::update_request(
    const load_request_handle handle, const point_abs_sm& begin, const point_abs_sm& end) -> void {
...
auto submap_load_manager::update_lazy_border_focus(
    const dimension_id& dim_id, const tripoint_abs_ms& pos) -> void {
    if (lazy_omt_focus_ && lazy_omt_focus_->dim_id == dim_id) {
        const auto delta = point{
            signum(pos.x() - lazy_omt_focus_->pos.x()), signum(pos.y() - lazy_omt_focus_->pos.y())};
        if (delta != point_zero) {
            if (delta != lazy_omt_preload_direction_) { lazy_omt_budget_credit_ = 0.0; }
    ...
    lazy_omt_focus_ = lazy_omt_focus{.dim_id = dim_id, .pos = pos};
}
...
auto submap_load_manager::compute_desired_set() const -> key_set {
    ...
    std::ranges::for_each(requests_, [&](const auto& kv) {
        ...
        if (req.source == load_request_source::lazy_border) { return; }
...
auto submap_load_manager::rebuild_simulated_submaps_by_dimension(const key_set& simulated) -> void {
...
auto submap_load_manager::compute_lazy_border_omts() const -> horizontal_omt_set {
    ...
    std::ranges::for_each(requests_, [&](const auto& kv) {
        ...
        std::ranges::
            for_each(cata::views::cartesian_product(x_range, y_range), [&](const auto pair) {
                auto [x, y] = pair;
...
auto submap_load_manager::add_lazy_border_into(
    key_set& target, const horizontal_omt_set& border_omts) const -> void {
    ZoneScoped;
...
auto submap_load_manager::erase_retained_omt(const omt_column_key& key) -> void {
    ...
    if (it == retained_omt_index_.end()) { return; }
...
auto submap_load_manager::evict_omt_column(const omt_column_key& key) -> void {
    ...
    std::ranges::for_each(std::views::iota(-OVERMAP_DEPTH, OVERMAP_HEIGHT + 1), [&](const auto z) {
...
auto submap_load_manager::process_retained_omt_eviction() -> void {
    ...
    TracyPlot("Retained OMT Lazy Border Cap Add", static_cast<int64_t>(lazy_border_cap_add));
    ...
    if (retained > panic_cap) {
        ...
        if (retained > hard_cap) {
            const auto scale =
                std::min(retained_omt_max_budget_scale, divide_round_up_size(retained, hard_cap));
            budget *= scale;
        }
...
auto submap_load_manager::load_lazy_omt_zlevel_data(
    mapbuffer& mb, const tripoint_abs_omt& omt_addr, const lazy_omt_load_options& options)
    -> lazy_omt_load_result {
    ZoneScopedN("slm_lazy_load_omt_zlevel_data");
    auto result = lazy_omt_load_result{};
    if (is_omt_zlevel_loaded(mb, omt_addr)) { return result; }
    ...
    if (!is_omt_zlevel_loaded(mb, omt_addr)) {
        ...
        if (is_pool_worker_thread() && is_any_omt_zlevel_loaded(mb, omt_addr)) { return result; }
        result.generation = mb.generate_omt(
            omt_addr,
            {
...
auto submap_load_manager::complete_lazy_omt_result_on_main_thread(
    const omt_key& key, lazy_omt_load_result result) -> lazy_omt_load_result {
    if (!result.generation.needs_main_thread()) { return result; }

    auto& mb = MAPBUFFER_REGISTRY.get(key.first);
    auto completed = load_lazy_omt_zlevel_data(
        mb, key.second,
        {
...
auto submap_load_manager::erase_lazy_omt_job(const omt_key& key) -> void {
    ...
    if (it == lazy_omt_job_index_.end()) { return; }
    lazy_omt_jobs_.erase(it->second);
    lazy_omt_job_index_.erase(it);
}

auto submap_load_manager::apply_lazy_omt_result(
    const omt_key& key, const lazy_omt_load_result& result) -> bool {
...
auto submap_load_manager::finish_lazy_omt_job(const omt_key& key) -> bool {
    ...
    if (it == lazy_omt_futures_.end()) { return false; }
...
auto submap_load_manager::reap_lazy_omt_jobs() -> void {
    ...
    if (generated) { run_deferred_mapgen_hooks_and_omt_post_passes(generated_omt_columns); }
...
auto submap_load_manager::start_lazy_omt_job(const omt_key& key) -> lazy_omt_start_result {
    if (lazy_omt_futures_.contains(key)) { return {}; }

    auto& mb = MAPBUFFER_REGISTRY.get(key.first);
    if (is_omt_zlevel_loaded(mb, key.second)) { return {}; }

    if (get_thread_pool().num_workers() == 0 || is_any_omt_zlevel_loaded(mb, key.second)) {
        auto result = load_lazy_omt_zlevel_data(
            mb, key.second,
            {
                .defer_postprocess_hooks = true,
                .worker_safe = false,
...
auto submap_load_manager::lazy_omt_priority(const omt_column_key& key) const -> int {
    if (lazy_omt_preload_direction_ == point_zero) { return 0; }

    auto prioritize_x = lazy_omt_preload_direction_.x != 0;
    auto prioritize_y = lazy_omt_preload_direction_.y != 0;
    if (lazy_omt_focus_) {
        const auto deadlines =
            omt_edge_deadlines_for(lazy_omt_focus_->pos, lazy_omt_preload_direction_);
    ...
    std::ranges::for_each(requests_, [&](const auto& kv) {
        const auto& req = kv.second;
        if (req.source != load_request_source::lazy_border || req.dim_id != dim_id) { return; }
        const auto size = request_size(req);
        if (size.x() <= 0 || size.y() <= 0) { return; }
        const auto min_omt = project_to<coords::omt>(req.begin);
        const auto max_omt = project_to<coords::omt>(request_inclusive_end(req));
...
auto submap_load_manager::queue_lazy_border_omts(const horizontal_omt_set& border_omts) -> void {
    ...
    std::ranges::sort(candidates, [&](const omt_column_key& lhs, const omt_column_key& rhs) {
    ...
    std::ranges::for_each(candidates, [&](const omt_column_key& key) {
        ...
        auto& mb = MAPBUFFER_REGISTRY.get(dim_id);
        std::ranges::
            for_each(std::views::iota(-OVERMAP_DEPTH, OVERMAP_HEIGHT + 1), [&](const auto z) {
...
auto submap_load_manager::process_lazy_border_preload() -> void {
    ...
    const auto urgent = static_cast<
        std::size_t>(std::ranges::count_if(lazy_omt_jobs_, [&](const omt_key& key) {
    ...
    const auto load_budget_matching = [&](std::size_t budget, const auto& can_load) -> std::size_t {
        ...
        while (budget > 0 && !lazy_omt_jobs_.empty()) {
            ...
            if (!can_load(key)) { return started; }
            erase_lazy_omt_job(key);
            auto& mb = MAPBUFFER_REGISTRY.get(key.first);
            if (is_omt_zlevel_loaded(mb, key.second)) { continue; }
            const auto result = start_lazy_omt_job(key);
            if (result.started) {
                --budget;
                ++started;
                if (result.generated) { generated_omt_columns.emplace(key.first, key.second.xy()); }
            }
        }
        return started;
    };
    const auto has_priority_work_at_front = [&]() {
        return !lazy_omt_jobs_.empty()
            && lazy_omt_priority({lazy_omt_jobs_.front().first, lazy_omt_jobs_.front().second.xy()})
                   > 0;
    };
    const auto background_budget = [&]() {
        if (lazy_omt_jobs_.empty() || has_priority_work_at_front()) { return std::size_t{0}; }
        return std::
            max(std::size_t{1},
    ...
    if (lazy_omt_preload_direction_ == point_zero) {
        ...
        TracyPlot("Lazy Border OMT Deadline", static_cast<int64_t>(lazy_border_steps_to_cross_omt));
        TracyPlot("Lazy Border Credit x1000", int64_t{0});
        TracyPlot("Lazy Border OMT Budget", static_cast<int64_t>(budget));
        const auto started = load_budget_matching(budget, [](const omt_key&) { return true; });
        if (started > 0) { run_deferred_mapgen_hooks_and_omt_post_passes(generated_omt_columns); }
    ...
    if (urgent == 0) {
        ...
        const auto priority_budget =
            std::min(urgent, static_cast<std::size_t>(lazy_omt_budget_credit_));
    ...
    started += load_budget_matching(remaining_budget, [](const omt_key&) { return true; });

    TracyPlot("Lazy Border OMT Deadline", static_cast<int64_t>(deadline));
    TracyPlot("Lazy Border Credit x1000", static_cast<int64_t>(lazy_omt_budget_credit_ * 1000.0));
    TracyPlot("Lazy Border OMT Budget", static_cast<int64_t>(budget));
...
auto submap_load_manager::process_deferred_lazy_border_work() -> void {
    ...
    if (!lazy_border_work_deferred_) { return; }
    if (!has_lazy_border_work_pending()) {
        lazy_border_work_deferred_ = false;
        TracyPlot("Lazy Border Work Deferred", int64_t{0});
        return;
...
auto submap_load_manager::update(const bool defer_lazy_border_work) -> void {
    ...
    // set construction, diffing, loading, and retention work.
    {
        ...
        std::ranges::for_each(requests_, [&](const auto& kv) {
            ...
            const auto& req = kv.second;
            if (req.source != load_request_source::reality_bubble) { return; }
            const auto prev = std::ranges::find_if(prev_requests_, [&](const auto& entry) {
                return entry.first == kv.first;
            });
            if (prev == prev_requests_.end()) { return; }
            const auto center = request_center(req);
            const auto prev_center = request_center(prev->second);
            bubble_delta =
                point{signum(center.x() - prev_center.x()), signum(center.y() - prev_center.y())};
        });
        if (bubble_delta != point_zero) { lazy_omt_preload_direction_ = bubble_delta; }
    ...
    auto lazy_border_omts = horizontal_omt_set{};
    {
        ...
        std::erase_if(lazy_border_omts, [&](const omt_column_key& key) {
            const auto& [dim_id, omt_xy] = key;
            const auto sm_base = project_to<coords::sm>(omt_xy);
            const auto omt_offsets =
                std::array{point_zero, point_south, point_east, point_south_east};
    ...
    TracyPlot("Border Submaps", static_cast<int64_t>(all_desired.size() - simulated.size()));
    ...
    // Must happen on the main thread (safe_reference remains main-thread-only).
    {
        ...
        std::ranges::transform(
            new_omts, std::inserter(drained_dims, drained_dims.end()),
        ...
        for (const auto& [dim_id, omt_xy] : new_omts) {
            ...
            for (const auto z : std::views::iota(-OVERMAP_DEPTH, OVERMAP_HEIGHT + 1)) {
                ...
                const auto all_loaded =
                    ...
                    && mb.lookup_submap_in_memory((sm_base + point_south_east));
                if (!all_loaded) {
                    const auto result = mb.generate_omt(
                        omt_addr,
                        {
                            .defer_postprocess_hooks = true,
                            .worker_safe = false,
...

    {
        ...
        for (const desired_key& key : simulated) { by_dim[key.first].insert(key.second); }
    ...
    // z-level loop both work in terms of the 2-D desired set.
    {
        ...
        for (const desired_key& key : previous_all_desired_) {
            ...
            if (!any_still_desired) { retain_omt({key.first, omt_xy}); }
        }
    }
...
auto submap_load_manager::is_requested(const dimension_id& dim_id, const point_abs_sm& pos) const
    -> bool {
...
auto submap_load_manager::is_stably_requested(
    const dimension_id& dim_id, const point_abs_sm& pos) const -> bool {
    return std::ranges::any_of(requests_, [&](const auto& kv) {
        const submap_load_request& req = kv.second;
...
auto submap_load_manager::release_vehicle_footprint_requests() -> void {
    for (const auto& [veh, request] : vehicle_footprint_requests_) { release_load(request.handle); }
...
auto submap_load_manager::update_vehicle_footprint_requests() -> void {
    ...
    MAPBUFFER_REGISTRY.for_each([&](const dimension_id& dim_id, mapbuffer& buffer) {
        buffer.for_each_vehicle([&](vehicle& veh) {
            ...
            if (request_it == vehicle_footprint_requests_.end()) {
                const auto handle = request_load(
                    load_request_source::vehicle_footprint, dim_id, request_begin, request_end);
                vehicle_footprint_requests_.emplace(
                    &veh,
                    vehicle_footprint_request{
                        .dim_id = dim_id,
                        .begin = request_begin,
                        .end = request_end,
                        .handle = handle,
    ...
    std::erase_if(vehicle_footprint_requests_, [&](const auto& entry) {
        if (active_vehicles.contains(entry.first)) { return false; }
        release_load(entry.second.handle);
        return true;
    });
}

auto submap_load_manager::is_properly_requested(
    const dimension_id& dim_id, const tripoint_abs_sm& pos) const -> bool {
    const point_abs_sm p = pos.xy();
    return std::ranges::any_of(requests_, [&](const auto& kv) {
        const submap_load_request& req = kv.second;
        if (req.source != load_request_source::reality_bubble) { return false; }
        if (req.dim_id != dim_id) { return false; }
        return contains_request_pos(req, p);
    });
}

auto submap_load_manager::is_simulated(const dimension_id& dim_id, const point_abs_sm& pos) const
    -> bool {
    if (!is_loaded(dim_id, pos)) { return false; }
    bool covered_by_lazy_only = false;
    for (const auto& [handle, req] : requests_) {
        if (req.dim_id != dim_id) { continue; }
        if (!contains_request_pos(req, pos)) { continue; }
        if (req.source != load_request_source::lazy_border) { return true; }
        covered_by_lazy_only = true;
    }
    if (covered_by_lazy_only) { return false; }
...
auto submap_load_manager::is_loaded(const dimension_id& dim_id, const point_abs_sm& pos) const
    -> bool {
...
auto submap_load_manager::active_dimensions() const -> std::vector<dimension_id> {
    ...
    for (const auto& kv : requests_) { dims.insert(kv.second.dim_id); }
...
auto submap_load_manager::non_bubble_requests() const -> std::vector<submap_load_request> {
    ...
    auto to_request = [](const auto& kv) -> const submap_load_request& { return kv.second; };
    auto view = requests_ | std::views::filter(is_non_bubble) | std::views::transform(to_request);
