...
map_item_stack::map_item_stack(): example(nullptr), totalcount(0) { vIG.emplace_back(); }

map_item_stack::map_item_stack(const item* const it, const tripoint_rel_ms& pos)
    : example(it),
      totalcount(it->count()) {
...
std::vector<map_item_stack> filter_item_stacks(
    ...
    std::ranges::copy_if(stack, std::back_inserter(ret), [z](const map_item_stack& a) {
        if (a.example != nullptr) { return z(*a.example); }
        return false;
    });
...
int list_filter_high_priority(std::vector<map_item_stack>& stack, const std::string& priorities) {
    ...
    int id = stack.size();
    for (auto& elem : tempstack) { stack.push_back(elem); }
    return id;
}

int list_filter_low_priority(
    std::vector<map_item_stack>& stack, const int start, const std::string& priorities) {
    // TODO:optimize if necessary
    std::vector<map_item_stack> tempstack;
    const auto filter_fn = item_filter_from_string(priorities);
