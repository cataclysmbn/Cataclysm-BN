...
auto strength_penalty(const creature_size thrower_size) -> int {
    return std::max(0, static_cast<int>(creature_size::large) - static_cast<int>(thrower_size))
         * smaller_than_large_strength_penalty;
}
...
auto throw_range(const int throw_strength, const int strength_requirement) -> int {
    if (throw_strength < strength_requirement) { return 0; }
...
auto shove_velocity(const int throw_strength, const int strength_requirement) -> int {
    return std::clamp(
        base_shove_velocity
            + shove_velocity_per_excess_strength
                  * std::max(0, throw_strength - strength_requirement),
