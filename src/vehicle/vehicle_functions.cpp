...
auto check_reload_timing(
    item& gun, const item& ammo_item, bool is_ammo_refill, const vehicle& veh,
    ...
    const auto effective_weight =
        is_ammo_refill ? (ammo_item.weight() / ammo_item.count()) : ammo_item.weight();
        ...
        if (cycle_start == 0LL) {
            ...
            if (get_avatar().sees(veh.abs_ms_location())) {
                ...
                sfx::play_variant_sound(
                    "reload", "start", sfx::get_heard_volume(veh.bub_ms_location(), 85));
            }
...
void perform_reload(
    vehicle& veh, vehicle_part& cargo_part, item& gun, item& source_item,
    ...
    gun.item_vars()
        .set("last_autoload_turn", to_turns<int64_t>(calendar::turn - calendar::turn_zero));
...
auto try_autoload_turret(vehicle& veh, vehicle_part& pt) -> bool {
    ...
    if (pt.info().has_flag("USE_TANKS")) { return false; }

    item& gun = pt.get_base();
    const auto ammo_capacity = gun.ammo_capacity();
    const auto needs_reload =
        gun.magazine_integral()
    ...
    if (autoloader_it == parts_here.end()) { return false; }

    const vehicle_part& autoloader_part = veh.cpart(*autoloader_it);
    const int autoloader_power = -autoloader_part.info().epower;

    if (veh.fuel_left(fuel_type_battery, true) < autoloader_power) { return false; }
...
void process_autoloaders(vehicle& veh) {
    ...
    for (const int i : std::views::iota(0, veh.part_count())) {
        ...
        if (autoloader.is_unavailable() || !autoloader.info().has_flag("AUTOLOADER")) { continue; }

        const int power_cost = -autoloader.info().epower;
        if (veh.fuel_left(fuel_type_battery, true) < power_cost) { continue; }

        // Find associated turret
        const auto parts_here = veh.parts_at_relative(autoloader.mount, false);
        auto turret_it = std::ranges::find_if(parts_here, [&veh](int idx) {
            const auto& p = veh.cpart(idx);
            return !p.is_broken() && p.is_turret();
        });

        if (turret_it != parts_here.end()) { try_autoload_turret(veh, veh.part(*turret_it)); }
    }
}
