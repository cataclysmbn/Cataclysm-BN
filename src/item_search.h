...
{
    std::string sRule;
    std::function<bool(const itype_id &)> ruleFunction = [](const itype_id& item) { return true; };
    bool bActive = false;
    bool bExclude = false;
    ...
    bool operator()(const itype_id& item_type) const;
    bool operator()(const itype& item_type) const;
