...
auto get_current_mod_path( lua_State *L ) -> std::string
{
    ...
    if( lua_istable( L, -1 ) ) {
        ...
        if( lua_isstring( L, -1 ) ) {
            auto const path = std::string{ lua_tostring( L, -1 ) };
...
auto resolve_relative_path( std::string_view modname ) -> std::optional<fs::path>
{
    ...
    auto const &current = loading_stack.back();
...
auto search_module( lua_State *L, std::string_view modname ) -> std::optional<fs::path>
{
    ...
    if( modname.starts_with( "./" ) || modname.starts_with( "../" ) ) {
        ...
        for( auto const &suffix : { ".lua", "/init.lua" } ) {
            auto path = *resolved;
            path += suffix;
    ...
    auto const normalized = normalize_module_name( modname );
    auto const mod_path_str = get_current_mod_path( L );
...
auto module_loader( lua_State *L ) -> int
{
    ...
    auto const path = fs::path{ lua_tostring( L, lua_upvalueindex( 1 ) ) };

    // Push to loading stack BEFORE execution
    loading_stack.push_back( path );

    // Load the file
    auto const status = luaL_loadfile( L, path.string().c_str() );
    if( status != LUA_OK ) {
...
{
    auto const modname = std::string_view{ lua_tostring( L, 1 ) };

    auto const found = search_module( L, modname );
    if( !found ) {
...
auto register_searcher( lua_State *L ) -> void
{
    ...
    auto const len = static_cast<int>( lua_rawlen( L, -1 ) );
