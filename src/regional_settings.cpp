...
template <typename T>
void read_and_set_or_throw(
    const JsonObject &jo, const std::string &member, T &target, bool required )
{
    T tmp;
    if( !jo.read( member, tmp ) ) {
        if( required ) { jo.throw_error( string_format( "%s required", member ) ); }
    } else {
        target = tmp;
    }
}
    ...
    const JsonObject &jo, forest_biome_component &forest_biome_component, const bool overlay )
{
    ...
    if( forest_biome_component.clear_types ) { forest_biome_component.unfinalized_types.clear(); }

    if( !jo.has_object( "types" ) ) {
        if( !overlay ) { jo.throw_error( "types required" ); }
    } else {
...
static void load_forest_biome_terrain_dependent_furniture(
    const JsonObject &jo,
    forest_biome_terrain_dependent_furniture &forest_biome_terrain_dependent_furniture,
    const bool overlay )
{
    read_and_set_or_throw <
    int > ( jo, "chance", forest_biome_terrain_dependent_furniture.chance, !overlay );
    read_and_set_or_throw<bool>(
        jo, "clear_furniture", forest_biome_terrain_dependent_furniture.clear_furniture, !overlay );
    ...
    if( !jo.has_object( "furniture" ) ) {
        if( !overlay ) { jo.throw_error( "furniture required" ); }
    } else {
        for( const JsonMember member : jo.get_object( "furniture" ) ) {
            if( member.is_comment() ) { continue; }
            forest_biome_terrain_dependent_furniture.unfinalized_furniture[member.name()] =
                member.get_int();
        }
    }
}

static void load_forest_biome(
    const JsonObject &jo, forest_biome &forest_biome, const bool overlay )
{
    read_and_set_or_throw <
    int > ( jo, "sparseness_adjacency_factor", forest_biome.sparseness_adjacency_factor, !overlay );
    ...
    read_and_set_or_throw <
    int > ( jo, "item_spawn_iterations", forest_biome.item_spawn_iterations, !overlay );
    ...
    read_and_set_or_throw <
    bool > ( jo, "clear_terrain_furniture", forest_biome.clear_terrain_furniture, !overlay );

    if( forest_biome.clear_components ) { forest_biome.unfinalized_biome_components.clear(); }

    if( !jo.has_object( "components" ) ) {
        if( !overlay ) { jo.throw_error( "components required" ); }
    } else {
        for( const JsonMember member : jo.get_object( "components" ) ) {
            if( member.is_comment() ) { continue; }
            JsonObject component_jo = member.get_object();
            load_forest_biome_component(
                component_jo, forest_biome.unfinalized_biome_components[member.name()], overlay );
        }
    }

    if( forest_biome.clear_groundcover ) { forest_biome.unfinalized_groundcover.clear(); }

    if( !jo.has_object( "groundcover" ) ) {
        if( !overlay ) { jo.throw_error( "groundcover required" ); }
    } else {
    ...
    if( !jo.has_object( "terrain_furniture" ) ) {
        if( !overlay ) { jo.throw_error( "terrain_furniture required" ); }
    } else {
        for( const JsonMember member : jo.get_object( "terrain_furniture" ) ) {
            if( member.is_comment() ) { continue; }
            JsonObject terrain_furniture_jo = member.get_object();
            load_forest_biome_terrain_dependent_furniture(
                terrain_furniture_jo,
                forest_biome.unfinalized_terrain_dependent_furniture[member.name()], overlay );
        }
    }
}

static void load_forest_mapgen_settings(
    const JsonObject &jo, forest_mapgen_settings &forest_mapgen_settings, const bool strict,
    const bool overlay )
{
    if( !jo.has_object( "forest_mapgen_settings" ) ) {
        if( strict ) { jo.throw_error( "\"forest_mapgen_settings\": { … } required for default" ); }
    } else {
        for( const JsonMember member : jo.get_object( "forest_mapgen_settings" ) ) {
            if( member.is_comment() ) { continue; }
            JsonObject forest_biome_jo = member.get_object();
            load_forest_biome(
                forest_biome_jo, forest_mapgen_settings.unfinalized_biomes[member.name()], overlay );
        }
    }
}

static void load_forest_trail_settings(
    const JsonObject &jo, forest_trail_settings &forest_trail_settings, const bool strict,
    const bool overlay )
{
    if( !jo.has_object( "forest_trail_settings" ) ) {
        if( strict ) { jo.throw_error( "\"forest_trail_settings\": { … } required for default" ); }
    } else {
        JsonObject forest_trail_settings_jo = jo.get_object( "forest_trail_settings" );
        read_and_set_or_throw <
        int > ( forest_trail_settings_jo, "chance", forest_trail_settings.chance, !overlay );
        read_and_set_or_throw <
        int > ( forest_trail_settings_jo, "border_point_chance",
                forest_trail_settings.border_point_chance, !overlay );
        read_and_set_or_throw <
        int > ( forest_trail_settings_jo, "minimum_forest_size",
                forest_trail_settings.minimum_forest_size, !overlay );
        read_and_set_or_throw <
        int > ( forest_trail_settings_jo, "random_point_min",
                forest_trail_settings.random_point_min, !overlay );
        read_and_set_or_throw <
        int > ( forest_trail_settings_jo, "random_point_max",
                forest_trail_settings.random_point_max, !overlay );
        read_and_set_or_throw <
        int > ( forest_trail_settings_jo, "random_point_size_scalar",
                forest_trail_settings.random_point_size_scalar, !overlay );
        read_and_set_or_throw <
        int > ( forest_trail_settings_jo, "trailhead_chance",
                forest_trail_settings.trailhead_chance, !overlay );
        read_and_set_or_throw <
        int > ( forest_trail_settings_jo, "trailhead_road_distance",
                forest_trail_settings.trailhead_road_distance, !overlay );
        read_and_set_or_throw <
        int > ( forest_trail_settings_jo, "trail_center_variance",
                forest_trail_settings.trail_center_variance, !overlay );
        read_and_set_or_throw <
        int > ( forest_trail_settings_jo, "trail_width_offset_min",
                forest_trail_settings.trail_width_offset_min, !overlay );
        read_and_set_or_throw <
        int > ( forest_trail_settings_jo, "trail_width_offset_max",
                forest_trail_settings.trail_width_offset_max, !overlay );
        read_and_set_or_throw <
        bool > ( forest_trail_settings_jo, "clear_trail_terrain",
        ...
        if( !forest_trail_settings_jo.has_object( "trail_terrain" ) ) {
            if( !overlay ) { forest_trail_settings_jo.throw_error( "trail_terrain required" ); }
        } else {
        ...
        if( !forest_trail_settings_jo.has_object( "trailheads" ) ) {
            if( !overlay ) { forest_trail_settings_jo.throw_error( "trailheads required" ); }
        } else {
            for( const JsonMember member : forest_trail_settings_jo.get_object( "trailheads" ) ) {
                if( member.is_comment() ) { continue; }
                forest_trail_settings.trailheads
                .add( overmap_special_id( member.name() ), member.get_int() );
            }
        }
    }
}

static void load_overmap_feature_flag_settings(
    const JsonObject &jo, overmap_feature_flag_settings &overmap_feature_flag_settings,
    const bool strict, const bool overlay )
{
    if( !jo.has_object( "overmap_feature_flag_settings" ) ) {
        ...
        JsonObject overmap_feature_flag_settings_jo = jo.get_object( "overmap_feature_flag_"
                "settings" );
        read_and_set_or_throw <
        bool > ( overmap_feature_flag_settings_jo, "clear_blacklist",
                 overmap_feature_flag_settings.clear_blacklist, !overlay );
        read_and_set_or_throw <
        bool > ( overmap_feature_flag_settings_jo, "clear_whitelist",
        ...
        if( !overmap_feature_flag_settings_jo.has_array( "blacklist" ) ) {
            if( !overlay ) { overmap_feature_flag_settings_jo.throw_error( "blacklist required" ); }
        } else {
        ...
        if( !overmap_feature_flag_settings_jo.has_array( "whitelist" ) ) {
            if( !overlay ) { overmap_feature_flag_settings_jo.throw_error( "whitelist required" ); }
        } else {
    ...
    const bool overlay )
{
    if( !jo.has_object( "overmap_forest_settings" ) ) {
        if( strict ) { jo.throw_error( "\"overmap_forest_settings\": { … } required for default" ); }
    } else {
        JsonObject overmap_forest_settings_jo = jo.get_object( "overmap_forest_settings" );
        read_and_set_or_throw<double>(
            overmap_forest_settings_jo, "noise_threshold_forest",
            overmap_forest_settings.noise_threshold_forest, !overlay );
        read_and_set_or_throw<double>(
            overmap_forest_settings_jo, "noise_threshold_forest_thick",
            overmap_forest_settings.noise_threshold_forest_thick, !overlay );
        read_and_set_or_throw<double>(
            overmap_forest_settings_jo, "noise_threshold_swamp_adjacent_water",
            overmap_forest_settings.noise_threshold_swamp_adjacent_water, !overlay );
        read_and_set_or_throw<double>(
            overmap_forest_settings_jo, "noise_threshold_swamp_isolated",
            overmap_forest_settings.noise_threshold_swamp_isolated, !overlay );
        read_and_set_or_throw <
        int > ( overmap_forest_settings_jo, "river_floodplain_buffer_distance_min",
                overmap_forest_settings.river_floodplain_buffer_distance_min, !overlay );
        read_and_set_or_throw <
        int > ( overmap_forest_settings_jo, "river_floodplain_buffer_distance_max",
                overmap_forest_settings.river_floodplain_buffer_distance_max, !overlay );
    }
}

static void load_overmap_lake_settings(
    const JsonObject &jo, overmap_lake_settings &overmap_lake_settings, const bool strict,
    const bool overlay )
{
    if( !jo.has_object( "overmap_lake_settings" ) ) {
        if( strict ) { jo.throw_error( "\"overmap_lake_settings\": { … } required for default" ); }
    } else {
        JsonObject overmap_lake_settings_jo = jo.get_object( "overmap_lake_settings" );
        read_and_set_or_throw<double>(
            overmap_lake_settings_jo, "noise_threshold_lake",
            overmap_lake_settings.noise_threshold_lake, !overlay );
        read_and_set_or_throw <
        int > ( overmap_lake_settings_jo, "lake_size_min", overmap_lake_settings.lake_size_min,
                !overlay );
        read_and_set_or_throw<int>(
            overmap_lake_settings_jo, "lake_depth", overmap_lake_settings.lake_depth, !overlay );
        ...
        if( !overmap_lake_settings_jo.has_array( "shore_extendable_overmap_terrain" ) ) {
            ...
            const std::vector<std::string> from_json = overmap_lake_settings_jo.get_string_array(
                        "shore_extendable_overmap_terrain" );
            overmap_lake_settings.unfinalized_shore_extendable_overmap_terrain
            .insert( overmap_lake_settings.unfinalized_shore_extendable_overmap_terrain.end(),
                     from_json.begin(), from_json.end() );
        }

        if( !overmap_lake_settings_jo.has_array( "shore_extendable_overmap_terrain_aliases" ) ) {
            if( !overlay ) {
                overmap_lake_settings_jo.throw_error( "shore_extendable_overmap_terrain_aliases "
                                                      "required" );
            }
        } else {
            for( JsonObject alias_entry : overmap_lake_settings_jo.get_array( "shore_extendable_"
                    "overmap_terrain_"
                    "aliases" ) ) {
                ...
                alias.match_type = alias_entry.get_enum_value <
...
static void load_region_terrain_and_furniture_settings(
    const JsonObject &jo,
    region_terrain_and_furniture_settings &region_terrain_and_furniture_settings, const bool strict,
    const bool overlay )
{
    if( !jo.has_object( "region_terrain_and_furniture" ) ) {
        ...
        JsonObject region_terrain_and_furniture_settings_jo = jo.get_object( "region_terrain_and_"
                "furniture" );
        ...
        if( !region_terrain_and_furniture_settings_jo.has_object( "terrain" ) ) {
            ...
            for( const JsonMember region :
                 region_terrain_and_furniture_settings_jo.get_object( "terrain" ) ) {
                if( region.is_comment() ) { continue; }
                for( const JsonMember terrain : region.get_object() ) {
                    if( terrain.is_comment() ) { continue; }
                    region_terrain_and_furniture_settings
                    .unfinalized_terrain[region.name()][terrain.name()] = terrain.get_int();
                }
            }
        }
        ...
        if( !region_terrain_and_furniture_settings_jo.has_object( "furniture" ) ) {
            ...
            for( const JsonMember template_furniture :
                ...
                if( template_furniture.is_comment() ) { continue; }
                for( const JsonMember furniture : template_furniture.get_object() ) {
                    if( furniture.is_comment() ) { continue; }
                    region_terrain_and_furniture_settings.unfinalized_furniture
                    [template_furniture.name()][furniture.name()] = furniture.get_int();
                }
            }
        }
    }
...
void load_region_settings( const JsonObject &jo )
{
    ...
    if( !jo.read( "id", new_region.id ) ) { jo.throw_error( "No 'id' field." ); }
    bool strict = new_region.id == "default";
    if( !jo.read( "default_oter", new_region.default_oter ) && strict ) {
        jo.throw_error( "default_oter required for default ( though it should probably remain "
                        "'field' )" );
    }
    ...
    if( jo.has_array( "default_groundcover" ) ) {
        ...
        for( JsonArray inner : jo.get_array( "default_groundcover" ) ) {
            if( new_region.default_groundcover_str
                ->add( ter_str_id( inner.get_string( 0 ) ), inner.get_int( 1 ) )
                == nullptr ) {
                jo.throw_error( "'default_groundcover' must be a weighted list: an array of pairs [ "
                                "\"id\", weight ]" );
            }
        }
    } else if( strict ) {
    ...
    if( !jo.has_object( "field_coverage" ) ) {
        ...
        if( pjo.read( "boost_chance", tmpval ) && tmpval != 0.0f ) {
            ...
            new_region.field_coverage.boosted_mpercent_coverage = static_cast<int>(
                        tmpval * 10000.0 );
            ...
            if( pjo.has_object( "boosted_other" ) ) {
                for( const JsonMember member : pjo.get_object( "boosted_other" ) ) {
                    if( member.is_comment() ) { continue; }
                    new_region.field_coverage.boosted_percent_str[member.name()] =
                        member.get_float();
                }
            } else {
    ...
    if( !jo.has_object( "map_extras" ) ) {
        if( strict ) { jo.throw_error( "\"map_extras\": { … } required for default" ); }
    } else {
        for( const JsonMember zone : jo.get_object( "map_extras" ) ) {
            if( zone.is_comment() ) { continue; }
            ...
            if( !zjo.has_object( "extras" ) ) {
                if( strict ) { zjo.throw_error( "\"extras\": { … } required for default" ); }
            } else {
    ...
    if( !jo.has_object( "city" ) ) {
        ...
        if( !cjo.read( "apartment_radius", new_region.city_spec.apartment_radius ) && strict ) {
            jo.throw_error( "city: apartment_radius required for default" );
        }
        if( !cjo.read( "apartment_sigma", new_region.city_spec.apartment_sigma ) && strict ) {
            jo.throw_error( "city: apartment_sigma required for default" );
        }
        ...
        const auto load_building_types =
        [&jo, &cjo, strict]( const std::string & type, building_bin & dest ) {
    ...
    if( jo.has_string( "base_weather" ) ) {
        jo.read( "base_weather", new_region.weather_base );
    } else if( jo.has_string( "weather" ) ) {
        jo.read( "weather", new_region.weather_base );
    } else if( jo.has_object( "weather" ) ) {
        JsonObject wjo = jo.get_object( "weather" );
        new_region.weather.load( wjo, "" );
        new_region.weather_base = base_weather_id();
    } else if( strict ) {
        jo.throw_error( "\"base_weather\": \"id\" required for default" );
    } else {
        new_region.weather_base = base_weather_id();
    }
    ...
    if( jo.has_array( "effects" ) ) {
        ...
        for( JsonObject effect_object : effects ) {
            ...
            int one_in = 0;
            if( effect_object.has_int( "one_in" ) ) { one_in = effect_object.get_int( "one_in" ); }
    ...
    load_region_terrain_and_furniture_settings(
        jo, new_region.region_terrain_and_furniture, strict, false );
...
void reset_region_settings() { region_settings_map.clear(); }
...
void load_region_overlay( const JsonObject &jo )
{
    if( jo.has_array( "regions" ) ) {
        ...
        for( const std::string regionid : regions ) {
            if( regionid == "all" ) {
                if( regions.size() != 1 ) {
                    jo.throw_error( "regions: More than one region is not allowed when \"all\" is "
                                    "used" );
                }

                for( auto &itr : region_settings_map ) { apply_region_overlay( jo, itr.second ); }
            } else {
...
void apply_region_overlay( const JsonObject &jo, regional_settings &region )
{
    ...
    if( jo.has_array( "default_groundcover" ) ) {
        ...
        for( JsonArray inner : jo.get_array( "default_groundcover" ) ) {
            if( region.default_groundcover_str
                ->add( ter_str_id( inner.get_string( 0 ) ), inner.get_int( 1 ) )
                == nullptr ) {
                jo.throw_error( "'default_groundcover' must be a weighted list: an array of pairs [ "
                                "\"id\", weight ]" );
    ...
    for( const JsonMember zone : jo.get_object( "map_extras" ) ) {
        ...
        int tmpval = 0;
        if( zonejo.read( "chance", tmpval ) ) { region.region_extras[zone.name()].chance = tmpval; }

        for( const JsonMember member : zonejo.get_object( "extras" ) ) {
            if( member.is_comment() ) { continue; }
            region.region_extras[zone.name()]
            .values.add_or_replace( member.name(), member.get_int() );
        }
    }
    ...
    cityjo.read( "apartment_radius", region.city_spec.apartment_radius );
    cityjo.read( "apartment_sigma", region.city_spec.apartment_sigma );
    ...
    if( cityjo.has_member( "finales" ) ) { load_building_types( "finales", region.city_spec.finales ); }
    ...
    if( jo.has_string( "base_weather" ) ) {
        jo.read( "base_weather", region.weather_base );
    } else if( jo.has_string( "weather" ) ) {
        jo.read( "weather", region.weather_base );
    } else if( jo.has_object( "weather" ) ) {
        JsonObject wjo = jo.get_object( "weather" );
        region.weather.load( wjo, "" );
        region.weather_base = base_weather_id();
    }
...
void groundcover_extra::finalize() // FIXME: return bool for failure
{
    ...
    if( btotal > 1000000 ) {
        ...
        for( auto it = boosted_percent_str.begin(); it != boosted_percent_str.end(); ++it ) {
            if( it != boosted_percent_str.begin() ) { ss << '+'; }
            ss << it->second;
        }
...
{
    if( boosted ) { return boosted_weightlist.lower_bound( rng( 0, 1000000 ) )->second; }
...
{
    // Iterate through the biome components (which have already been put into sequence), roll for
    // the one_in chance that component contributes a feature, and if so pick that feature and
    // return it. If a given component does not roll as success, proceed to the next feature in
    // sequence until a feature is picked or none are picked, in which case an empty feature is
    // returned.
    ...
    if( result == nullptr ) { return ter_furn_id(); }
...
void forest_biome::finalize()
{
    ...
    std::sort( biome_components.begin(), biome_components.end(),
    []( const forest_biome_component & a, const forest_biome_component & b ) {
...
void forest_trail_settings::finalize()
{
    for( const std::pair<const std::string, int> &pr : unfinalized_trail_terrain ) {
        const ter_str_id tid( pr.first );
        if( !tid.is_valid() ) {
            debugmsg( "Tried to add invalid terrain %s to forest_trail_settings trail_terrain.",
                      tid.c_str() );
            continue;
        }
...
void overmap_lake_settings::finalize()
{
    ...
    for( const std::string &oid : unfinalized_shore_extendable_overmap_terrain ) {
        ...
        if( !ot.is_valid() ) {
            debugmsg( "Tried to add invalid overmap terrain %s to overmap_lake_settings "
                      "shore_extendable_overmap_terrain.",
                      ot.c_str() );
            continue;
        }
    ...
    for( shore_extendable_overmap_terrain_alias &alias : shore_extendable_overmap_terrain_aliases ) {
        if( std::find( shore_extendable_overmap_terrain.begin(),
                       shore_extendable_overmap_terrain.end(), alias.alias )
            == shore_extendable_overmap_terrain.end() ) {
            debugmsg( " %s was referenced as an alias in overmap_lake_settings "
                      "shore_extendable_overmap_terrain_alises, but the value is not present in the "
                      "shore_extendable_overmap_terrain.",
                      alias.alias.c_str() );
            continue;
        }
...
void region_terrain_and_furniture_settings::finalize()
{
    for( auto const &template_pr : unfinalized_terrain ) {
        ...
        if( !template_tid.is_valid() ) {
            debugmsg( "Tried to add invalid regional template terrain %s to "
                      "region_terrain_and_furniture terrain.",
                      template_tid.c_str() );
            continue;
        }
        for( auto const &actual_pr : template_pr.second ) {
            ...
            if( !tid.is_valid() ) {
                debugmsg( "Tried to add invalid regional terrain %s to region_terrain_and_furniture "
                          "terrain template %s.",
    ...
    for( auto const &template_pr : unfinalized_furniture ) {
        ...
        if( !template_fid.is_valid() ) {
            debugmsg( "Tried to add invalid regional template furniture %s to "
                      "region_terrain_and_furniture furniture.",
                      template_fid.c_str() );
            continue;
        }
        for( auto const &actual_pr : template_pr.second ) {
            ...
            if( !fid.is_valid() ) {
                debugmsg( "Tried to add invalid regional furniture %s to "
                          "region_terrain_and_furniture furniture template %s.",
...
void regional_settings::finalize()
{
    if( default_groundcover_str != nullptr ) {
        if( weather_base.is_valid() ) {
            weather = base_weathers::get( weather_base );
        } else if( weather.id.is_null() ) {
            debugmsg( "Region %s references invalid base weather %s", id, weather_base.c_str() );
        }
...
overmap_special_id city_settings::pick_house() const { return houses.pick()->id; }

overmap_special_id city_settings::pick_urban_house() const { return urban_houses.pick()->id; }

overmap_special_id city_settings::pick_shop() const { return shops.pick()->id; }

overmap_special_id city_settings::pick_urban_shop() const { return urban_shops.pick()->id; }

overmap_special_id city_settings::pick_park() const { return parks.pick()->id; }

overmap_special_id city_settings::pick_finale() const { return finales.pick()->id; }
...
void city_settings::finalize()
{
    ...
    parks.finalize();
    if( !finales.unfinalized_buildings.empty() ) { finales.finalize(); }
}
...
overmap_special_id building_bin::pick() const
{
    ...
    if( !result ) { return overmap_special_id( "null" ); }
    return *result;
}
...
{
    if( finalized ) {
        debugmsg( "Tried to finalize a finalized bin (that's a code-side error which can't be fixed "
                  "with jsons)" );
        return;
    }
    ...
    for( const std::pair<const overmap_special_id, int> &pr : unfinalized_buildings ) {
        ...
        if( !current_id.is_valid() ) {
            ...
            if( !converted_id.is_valid() ) {
                debugmsg( "Tried to add city building %s, but it is neither a special nor a terrain "
                          "type",
                          pr.first.c_str() );
                continue;
            } else {
    ...
    for( auto const& [region_id, region] : region_settings_map ) {
        ...
        for( auto const& [extras_group, extras] : region.region_extras ) {
            for( auto const& [extra_id, extra_weight] : extras.values ) {
                ...
                if( !id.is_valid() ) { rep.warn( "defines unknown map extra '%s'", id ); }
            }
        }

        check_isolated_city_settings( rep, region );

        if( !rep.is_empty() ) { debugmsg( rep.format( "region_settings", region_id ) ); }
    }
}
