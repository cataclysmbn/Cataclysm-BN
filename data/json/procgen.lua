...
local function filter_facts_by_tag(facts, wanted)
  ...
  local out = {}
  for _, fact in ipairs(facts or {}) do
    if fact_has_tag(fact, wanted) then out[#out + 1] = fact end
  end
  return out
end
...
---@param wanted string
---@return boolean
local function fact_has_id(fact, wanted) return (fact.id or "") == wanted end

---@param fact ProcPartFact
---@param wanted string
---@return boolean
local function fact_id_contains(fact, wanted) return string.find(fact.id or "", wanted, 1, true) ~= nil end
...
local function facts_for_role(params, wanted)
  ...
  if #picks > 0 then
    for index, pick in ipairs(picks) do
      if pick.role == wanted and facts[index] ~= nil then out[#out + 1] = facts[index] end
    end
    return out
  end

  local fallback = fallback_tags_by_role[wanted]
  if fallback == nil then return out end
...
local function trail_mix_validate(params)
  ...
  if not has_non_chocolate then return { err = "Trail mix can not be made from only chocolate." } end
  ...
  if not has_nut then return { err = "Trail mix needs at least one nut ingredient." } end
  if not has_dried then return { err = "Trail mix needs at least one dried fruit ingredient." } end
  return { ok = true }
end
...
local function sandwich_condiment_name(facts)
  local has_peanut_butter = any_fact(
    facts,
    function(fact) return fact_has_id(fact, "peanutbutter") or fact_has_id(fact, "peanutbutter_imitation") end
  )
  local has_jam = any_fact(facts, function(fact) return fact_has_id(fact, "jam_fruit") end)
  local has_honey = any_fact(facts, function(fact) return fact_has_material(fact, "honey") end)
  local has_syrup = any_fact(facts, function(fact) return fact_has_id(fact, "syrup") end)
...
local function sandwich_name(params)
  ...
  local has_fish = any_fact(
    meat_facts,
    function(fact) return fact_has_material(fact, "fish") or fact_id_contains(fact, "fish") end
  )
  local has_blt = any_fact(meat_facts, function(fact) return fact_has_id(fact, "bacon") end)
    and any_fact(
      veg_facts,
      function(fact) return fact_has_id(fact, "lettuce") or fact_has_id(fact, "irradiated_lettuce") end
    )
    and any_fact(
      veg_facts,
      function(fact) return fact_has_id(fact, "tomato") or fact_has_id(fact, "irradiated_tomato") end
    )

  if has_fish then return "fish sandwich" end
  if #bread_facts >= 3 and #meat_facts > 0 and #veg_facts > 0 and #cond_facts > 0 then return "club sandwich" end
  if #meat_facts > 0 and #cheese_facts > 0 and #veg_facts > 0 and #cond_facts > 0 then return "deluxe sandwich" end
  ...
  if #veg_facts > 0 then
    if any_fact(veg_facts, function(fact) return fact_has_id(fact, "cucumber") end) then return "cucumber sandwich" end
    return "vegetable sandwich"
  end
  if #cond_facts > 0 then return sandwich_condiment_name(cond_facts) end
  return "sandwich"
end
...
local function stew_name(facts)
  if any_fact(facts, function(fact) return fact_has_material(fact, "fish") or fact_id_contains(fact, "fish") end) then
    return "fish stew"
  end
  if any_fact(facts, function(fact) return fact_has_tag(fact, "meat") end) then return "meat stew" end
...
local function trail_mix_name(params)
  ...
  local has_chocolate = any_fact(sweet_facts, function(fact) return is_chocolate_id(fact.id or "") end)
...
local function trail_mix_description(facts)
  ...
  if #ids == 0 then return "A hand-mixed trail snack." end
...
---@param params ProcParams
---@return ProcBlob
function procgen.food.name(params)
  if params.schema_id == "sandwich" then return { name = sandwich_name(params) } end
  if params.schema_id == "stew" then return { name = stew_name(params.facts or {}) } end
...
---@param params ProcParams
---@return ProcValidateResult
function procgen.food.validate(params)
