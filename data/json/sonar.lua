...
local function oter_id_to_string( oter_id )
  ...
  if oter_id.str_id then
    return oter_id:str_id():str()
  end
...
local function is_underwater_oter( oter_id )
  ...
  for _, pattern in ipairs( underwater_patterns ) do
    if string.find( id_str, pattern, 1, true ) then
      return true
    end
  end
  return false
...
sonar.register = function( mod )
  mod.sonar_scan = function( who, item, pos )
    ...
    for depth_index = 0, depth_steps - 1 do
      ...
      if not is_underwater_oter( overmapbuffer.ter( scan_omt ) ) then
        break
      end
      if overmapbuffer.reveal( scan_omt, radius, is_underwater_oter ) then
        any_revealed = true
      end
    end
