...
TEST_CASE("full backpack drop", "[activity][drop_token]") {
    ...
    GIVEN("a character with a backpack full of items and no other containers") {
        ...
        WHEN("he considers dropping the backpack") {
            ...
            THEN("he will try to drop all carried items") {
                ...
                // stacks
                AND_THEN(
                    "all of them will have identical drop tokens, marking the backpack as "
                    "parent") {
                ...
                AND_THEN(
                    "the backpack will have non-zero drop cost, while all contents will have "
                    "zero drop cost") {
                ...
                AND_THEN(
                    "total volume dropped will equal volume of backpack and all items in "
                    "inventory") {
    ...
    GIVEN("a character with two duffel bags and two backpacks full of items and no other "
          "containers") {
        ...
        WHEN("he considers dropping one duffel bag and one backpack") {
            ...
            THEN("he will try to drop some, but not all of the carried items") {
                ...
                AND_THEN(
                    "dropped duffel bag and backpack will have non-zero drop cost, while all "
                    "contents will have zero drop cost") {
                    ...
                    AND_THEN(
                        ...
                        AND_THEN(
                            "when actually dropping items, each container will be followed by "
                            "enough items of same token to fill it, "
...
TEST_CASE("full backpack pickup", "[drop_token]") {
    clear_all_state();
    const tripoint_bub_ms pos = bub_test_origin();
...
TEST_CASE("pickup_ui_stacking", "[activity][drop_token]") {
    ...
    GIVEN("Multiple identical items with tokens with different turns and parents set, but with no "
          "parents on the item stack") {
        testing_stack the_stack;
