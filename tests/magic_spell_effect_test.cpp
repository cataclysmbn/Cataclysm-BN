...
TEST_CASE( "line_attack", "[magic]" )
{
    ...
    SECTION( "aoe=0" ) {
        const std::set<tripoint_abs_ms> reference( { tripoint_abs_ms::east(), tripoint_abs_ms( 2, 0, 0 ), tripoint_abs_ms( 3, 0, 0 ), tripoint_abs_ms( 4, 0, 0 ), tripoint_abs_ms( 5, 0, 0 ) } );

        std::set<tripoint_abs_ms> targets = calculate_spell_effect_area( sp, target,
