...
struct zlope_layout {
    ...
    tripoint_bub_ms zlope_pos;
    tripoint_bub_ms wall_pos;
    tripoint_bub_ms roof_pos;
    tripoint_bub_ms open_above_zlope;
};

auto set_test_terrain( map &here, const tripoint_bub_ms &pos, const ter_id &terrain ) -> void
{
                              ...
                              const ter_id &upper_exit_terrain ) -> zlope_layout
{
    ...
    const auto layout = zlope_layout{
        .start = tripoint_bub_ms( 60, 60, 0 ),
        .zlope_pos = tripoint_bub_ms( 61, 60, 0 ),
        .wall_pos = tripoint_bub_ms( 62, 60, 0 ),
        .roof_pos = tripoint_bub_ms( 62, 60, 1 ),
        .open_above_zlope = tripoint_bub_ms( 61, 60, 1 ),
    };
...
TEST_CASE( "player_can_use_single_tile_zlope" )
{
    SECTION( "floor exit" ) {
        ...
        REQUIRE( avatar_action::move( dummy, here, point_rel_ms::east() ) );
        CHECK( dummy.bub_pos() == layout.zlope_pos );

        REQUIRE( avatar_action::move( dummy, here, point_rel_ms::east() ) );
        CHECK( dummy.bub_pos() == layout.roof_pos );

        REQUIRE( avatar_action::move( dummy, here, point_rel_ms::west() ) );
        CHECK( dummy.bub_pos() == layout.zlope_pos );
    }
...
TEST_CASE( "player_does_not_auto_climb_blocked_single_tile_zlope" )
{
    ...
    CHECK_FALSE( avatar_action::move( dummy, here, point_rel_ms::east() ) );
    CHECK( dummy.bub_pos() == layout.zlope_pos );

    REQUIRE( avatar_action::move( dummy, here, point_rel_ms::west() ) );
    CHECK( dummy.bub_pos() == layout.start );
}
